...
class ConversionMonitor:
    ...
    def __init__(
        self, status_file: Path = Path("data/journal_research/conversion_status.json")
    ):
        """
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        in_progress = sum(
            1
            for s in self.conversion_status.values()
            if s.get("status") == "in_progress"
        )
        completed = sum(
            1 for s in self.conversion_status.values() if s.get("status") == "completed"
        )
        failed = sum(
            1 for s in self.conversion_status.values() if s.get("status") == "failed"
        )
