...
```bash
python -m ai.sourcing.journal.cli.cli config set "acquisition.encryption_enabled" "true"
python -m ai.sourcing.journal.cli.cli config set "acquisition.encryption_key" "your-encryption-key"
```
...
# Configuration file
python -m ai.sourcing.journal.cli.cli config set "acquisition.storage_base_path" "/path/to/datasets"
```
