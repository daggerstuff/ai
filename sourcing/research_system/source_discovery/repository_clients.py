...
class ZenodoClient(BaseAPIClient):
    ...
    def search_records(
        self, query: str, record_type: str = "dataset", max_results: int = 100
    ) -> list[dict[str, Any]]:
        """
...
class ClinicalTrialsClient(BaseAPIClient):
    ...
    def search_studies(
        self, query: str, status: str = "COMPLETED", max_results: int = 100
    ) -> list[dict[str, Any]]:
        """
    ...
    def search_mental_health_studies(
        ...
        logger.info(
            f"Converted {len(sources)} clinical trials to DatasetSource objects"
        )
        return sources
