        ...
        try:
            ...
            if e.response and 500 <= e.response.status_code < 600:
                return self._retry_request(
                    endpoint, params, method, use_cache, cache_ttl, attempt, e
                )
    ...
    def _retry_request(
        ...
        if attempt < max_attempts:
            ...
            return self._make_request(
                endpoint, params, method, use_cache, cache_ttl, attempt + 1
            )

        self._stats["errors"] += 1
        logger.error(
            f"{self.service_name} request failed after {max_attempts} attempts: {error}"
        )
        raise APIError(
            f"{self.service_name} request failed after {max_attempts} attempts: {error}"
        )
