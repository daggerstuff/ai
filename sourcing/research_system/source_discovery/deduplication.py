...
class DatasetDeduplicator:
    ...
    def _is_duplicate_by_title_and_authors(
        ...
        title_similarity = self.parser.calculate_text_similarity(
            normalized_title1, normalized_title2
        )
    ...
    def _merge_duplicate_metadata(
        self, primary: DatasetSource, duplicate: DatasetSource
    ) -> None:
        """
        ...
        existing_authors = set(
            self.parser.normalize_author_name(author) for author in primary.authors
        )
        ...
        if availability_priority.get(
            duplicate.data_availability, 0
    ...
    def find_duplicates_by_doi(
        self, sources: list[DatasetSource], doi: str
    ) -> list[DatasetSource]:
        """
    ...
    def find_similar_sources(
        ...
        for source in sources:
            ...
            if target.authors and source.authors:
                source_last_names = self.parser.extract_author_last_names(
                    source.authors
                )
    ...
    def create_unified_source(
        self, duplicates: list[DatasetSource]
    ...
    def get_deduplication_stats(
        self, original_count: int, deduplicated_count: int
    ) -> dict[str, any]:
        """
