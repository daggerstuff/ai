...
Provides therapeutic memory ingestion controls following Mem0 cookbook best practices:
...
class InferenceMode(str, Enum):
    """Memory inference modes from Mem0."""
...
class MemoryCategory(str, Enum):
    """Categories for generalist and therapeutic memories."""
...
# Default generalist-friendly custom instructions for Mem0
DEFAULT_MEMORY_INSTRUCTIONS = """
General Memory Guidelines for High-Fidelity Context.

STORE (High Confidence):
- Confirmed facts about the user's current project or workspace
- Long-term goals and stated preferences
- Validated insights from the conversation
- Core identity attributes (role, skills, communication style)
- Crisis indicators and safety-critical signals (Priority 1)
- Verified coping strategies and interaction patterns

STORE WITH CAUTION (Moderate Confidence):
- Self-reported states or opinions (tag as self-reported)
- Mentioned events or temporary context
- Fluid goals and aspirations

NEVER STORE (Security & Privacy):
- Social Security Numbers or government IDs
- Insurance policy numbers or financial details
- Credit card or banking information
- Full addresses or exact location data
- Personal contact details (phone numbers, private emails)
- PII of third parties (anonymize relationships)
- Explicit credentials or secrets

IGNORE (Do Not Extract):
- Speculation (words like "might", "maybe", "I think I could have")
- Transient chatter or technical noise
- Gossip about third parties
- Information that is already redundantly stored

SPECIAL HANDLING:
- Crisis signals: Always flag and store with high priority (CRITICAL)
- Narrative Dissonance: Flag if new info contradicts established facts
- Sensitive Topics: Request explicit consent for traumatic or highly private data
"""
...
class TherapeuticMemoryConfig:
    ...
    Attributes:
        custom_instructions: Instructions for Mem0 on what to store/ignore
