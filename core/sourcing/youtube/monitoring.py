...
from ai.core.sourcing.youtube.models import Channel, ChannelStatus
...
class ChannelMonitor:
    ...
    def _calculate_health_score(self, channel: Channel) -> float:
        ...
        if channel.last_updated:
            days_inactive = (datetime.now() - channel.last_updated).days
            if days_inactive < 7:
                score += 0.2
    ...
    def _generate_notes(
        ...
        if channel.last_updated:
            days_ago = (datetime.now() - channel.last_updated).days
