...
try:
    from ai.core.sourcing.journal.integration.mcp_pipeline_bridge import (
        MCPPipelineBridge,
    )
...
from ai.core.sourcing.journal.mcp.tools.sessions import (
    CreateSessionTool,
    DeleteSessionTool,
    GetSessionTool,
    ListSessionsTool,
            ...
            if self.config.rate_limits.enabled and self.rate_limiter:
                try:
                    ...
                    self.audit_logger.log_rate_limit_exceeded(
                        identifier=(
                            getattr(e, "data", {}).get("identifier", "unknown")
                            if hasattr(e, "data")
                            else "unknown"
                        ),
                        user_id=user_id,
