    ...
    def extract(self, article: dict[str, Any]) -> Optional[ArticleMetadata]:
        ...
        try:
            ...
            if pub_date_str:
                try:
                    pub_date = datetime(
                        int(pub_date_str), self.DEFAULT_MONTH, self.DEFAULT_DAY
                    )
                except (ValueError, TypeError):
                    logger.debug(
                        f"Invalid publication date '{pub_date_str}', using None"
                    )
            ...
            logger.error(
                f"Unexpected error extracting article metadata: {e}", exc_info=True
            )
            return None
...
class DOAJClient(BaseAPIClient):
    ...
    REQUIRED_METADATA_FIELDS = frozenset(
        ["title", "authors", "publication_date", "url", "abstract", "keywords"]
    )
    ...
    def __init__(
        self, enable_cache: bool = True, extractor: Optional[MetadataExtractor] = None
    ):
        """
    ...
    def _validate_subject(self, subject: Optional[str]) -> str:
        ...
        if not isinstance(subject, str):
            logger.warning(
                f"Invalid subject type: {type(subject)}, converting to string"
            )
            subject = str(subject)
    ...
    def _validate_keywords(
        self, keywords: Any, param_name: str = "keywords"
    ) -> list[str]:
        """
        ...
        if not isinstance(keywords, list):
            logger.warning(
                f"Invalid {param_name} type: {type(keywords)}, expected list"
            )
            return []
    ...
    def _build_term_query(
        self, field: str, value: str, size: int, sort_field: str = "created_date"
    ) -> dict[str, Any]:
        """
    ...
    def _build_keyword_queries(
        self, keywords: list[str], search_fields: Optional[list[str]] = None
    ) -> list[dict[str, Any]]:
        """
    ...
    def search_journals(
        self, subject: Optional[str] = None, max_results: int = DEFAULT_JOURNAL_RESULTS
    ) -> list[dict[str, Any]]:
        """
        ...
        logger.info(
            f"Searching DOAJ journals: subject={subject}, max_results={max_results}"
        )
        ...
        return self._execute_search(
            self.ENDPOINT_JOURNALS, query, f"DOAJ {subject} journal search"
        )
    ...
    def search_articles(
        ...
        logger.info(
            f"Searching DOAJ articles: keywords={keywords}, subject={subject}, max_results={max_results}"
        )
    ...
    def get_journal_articles(
        ...
        """
        logger.info(
            f"Fetching DOAJ articles: journal_issn={journal_issn}, max_results={max_results}"
        )
        ...
        query = self._build_term_query(
            self.FIELD_JOURNAL_ISSN, journal_issn, max_results
        )

        return self._execute_search(
            self.ENDPOINT_ARTICLES, query, f"DOAJ articles from journal {journal_issn}"
        )
    ...
    def _create_source_from_metadata(
        ...
        try:
            ...
            logger.error(
                f"Failed to create DatasetSource from metadata: {e}", exc_info=True
            )
            return None
    ...
    def search_therapeutic_content(
        ...
        logger.info(
            f"Searching therapeutic content: keywords={keywords}, max_results={max_results}"
        )

        # Search articles
        articles = self.search_articles(
            keywords=keywords, subject=self.DEFAULT_SUBJECT, max_results=max_results
        )
    ...
    def investigate_journal(
        ...
        """
        # Validate keywords
        therapeutic_keywords = self._validate_keywords(
        ...
        def has_therapeutic_content(metadata: ArticleMetadata) -> bool:
            ...
            return any(
                keyword in keywords_text for keyword in therapeutic_keywords_lower
            )

        # Convert and filter articles
        therapeutic_sources = self._convert_articles_to_sources(
            articles, has_therapeutic_content
        )
