"""
Unit tests for ai.pipelines.local_loader
"""

import pandas as pd
from ai.core.pipelines.orchestrator import local_loader
...
def test_load_local_csv_success(tmp_path):
    ...
    progress_messages = []
    loaded = local_loader.load_local_csv(
        str(csv_path), progress_callback=progress_messages.append
    )
...
def test_load_local_csv_not_found(tmp_path):
    ...
    error_messages = []
    loaded = local_loader.load_local_csv(
        str(missing_path), error_callback=error_messages.append
    )
