...
import pytest
from ai.core.pipelines.gcs_connector import GCSConfig, GCSConnector
from ai.core.pipelines.ingestion_deduplication import IngestionDeduplicator
from ai.core.pipelines.ingestion_interface import (
    IngestRecord,
    LocalFileConnector,
)
from ai.core.pipelines.ingestion_queue import (
    IngestionQueue,
    QueueItem,
    QueueType,
)
from ai.core.pipelines.s3_connector import S3Config, S3Connector
from ai.core.pipelines.youtube_connector import YouTubeConfig, YouTubeConnector
...
@patch("ai.pipelines.youtube_connector.subprocess.run")
@patch("ai.pipelines.youtube_processor.YouTubePlaylistProcessor")
class TestYouTubeConnector:
    ...
    def test_url_validation_allow_list(self, mock_processor, mock_subprocess):
        ...
        assert not connector._validate_youtube_url(
            "https://malicious.com/watch?v=abc123"
        )
...
@patch("ai.pipelines.s3_connector.boto3.Session")
...
@patch("ai.pipelines.gcs_connector.storage.Client")
...
class TestConnectorIntegration:
    ...
    def test_local_connector_with_deduplication(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            from ai.core.pipelines.ingestion_deduplication import (
        ...
        async def run_test():
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                queue = await IngestionQueue(
                    queue_type=QueueType.INTERNAL_ASYNC, max_size=10
