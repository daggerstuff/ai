...
import pytest
from ai.core.pipelines.quarantine import (
    QuarantineRecord,
    QuarantineStatus,
    QuarantineStore,
)
...
def test_quarantine_store_connection(mocker):
    mocker.patch("ai.pipelines.quarantine.MongoClient")
    mocker.patch("ai.pipelines.quarantine.MongoClient.admin.command")
    store = QuarantineStore(mongo_uri="mock://localhost")
    assert store.collection is not None  # Connection succeeds


@patch("ai.pipelines.quarantine.MongoClient")
...
def test_quarantine_store_reprocess_fail(mock_client, mocker):
    ...
    mocker.patch("ai.pipelines.quarantine.IngestRecord")
    mocker.patch(
        "ai.pipelines.quarantine.validate_record",
