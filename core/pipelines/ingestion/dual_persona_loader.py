...
@dataclass
class DualPersonaConfig:
    """Configuration for DualPersonaLoader"""

    output_path: str = "ai/pipelines/dual_persona/output"
...
class DualPersonaLoader:
    ...
    def __init__(
        ...
        if file_path:
            ...
            if path.is_dir():
                candidates = [
                    path / "output/dual_persona_training_data.jsonl",
                    path / "dual_persona_training_data.jsonl",
                ]
                self.training_file = candidates[0]
                for candidate in candidates:
                    if candidate.exists():
                        self.training_file = candidate
                        break
            else:
        ...
        # `self.stats = {config_file = self.pipeline_dir / "training_config.json"`
        # from the snippet is a syntax error and is not included.
    ...
    def convert_to_training_format(
        self, dialogues: list[DualPersonaDialogue] | None = None
    ) -> list[dict]:
...
def load_dual_persona_training_data(pipeline_dir: str | None = None) -> list[dict]:
    """
...
if __name__ == "__main__":
    ...
    logger.info("Dual Persona Training Data Loader")
    logger.info("=" * 60)
    ...
    logger.info(f"   Avg consistency: {stats['avg_consistency_score']:.2f}")
    ...
    if training_data:
        ...
        logger.info(
            f"   Personas: {sample['metadata']['persona_1']} + {sample['metadata']['persona_2']}"
        )
        logger.info(f"   Scenario: {sample['metadata']['scenario']}")
