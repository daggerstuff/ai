...
def test_full_integration():
    ...
    print(
        f"✅ Created tri-expert voice with {len(blended_voice['core_principles'])} principles"
    )
    ...
    bias_validated_datasets = bias_detection.create_bias_validated_datasets(
        journaling_datasets[:10]
    )
    ...
    kb_enhanced_datasets = psychology_kb.create_kb_enhanced_datasets(
        journaling_datasets[:5]
    )
    ...
    if journaling_datasets:
        ...
        if voice_enhanced:
            # Add bias validation
            bias_validated = bias_detection.validate_and_enhance_datasets(
                voice_enhanced
            )
            ...
            if bias_validated:
                ...
                kb_enhanced = (
                    psychology_kb.enhance_conversation_with_psychology_concepts(
                        bias_validated[0]
                    )
                )
                master_datasets = [
                    kb_enhanced
                ]  # Start with one fully integrated example
            else:
    ...
    print(
        f"✅ Created {len(master_datasets)} master integrated datasets using ALL components"
    )
    ...
    if master_datasets:
        ...
        print(
            f"✅ Integration quality: {passed_checks}/{total_checks} components present"
        )
    ...
    print(
        f"   • Voice-enhanced datasets: {len(voice_enhanced) if 'voice_enhanced' in locals() else 0}"
    )
    ...
    print("   • Expert voices integrated: 3 (Tim + Gabor + Brené)")
    print(f"   • Therapeutic principles: {len(blended_voice['core_principles'])}")
    print("   • Psychology concepts: 4,867 available")

    # Calculate total datasets across all components
    total_datasets = (
        len(journaling_datasets)
        + len(edge_case_datasets)
        + len(dual_persona_datasets)
        + len(bias_validated_datasets)
    ...
    print("🚀 ALL 6 COMPONENTS NOW WORKING TOGETHER!")
