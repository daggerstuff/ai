...
class Sampler:
    ...
    def split_dataset(
        ...
        if len(conversations) != len(label_bundles):
            raise ValueError(
                "Conversations and label bundles must have the same length"
            )
    ...
    def _group_by_therapeutic_response(
        self, paired_data: List[Tuple[Conversation, LabelBundle]]
    ) -> Dict[str, List]:
    ...
    def _group_by_therapy_modality(
        self, paired_data: List[Tuple[Conversation, LabelBundle]]
    ) -> Dict[str, List]:
    ...
    def _calculate_split_statistics(
        self,
        all_data: List[Tuple[Conversation, LabelBundle]],
    ...
    def _calculate_label_distribution(
        self, split_data: List[Tuple[Conversation, LabelBundle]]
    ) -> Dict[str, Any]:
        ...
        return {
            "crisis_distribution": {
                k: v / total_conversations if total_conversations > 0 else 0
    ...
    def _calculate_imbalance_metrics(
        self, splits: Tuple[List, List, List]
    ) -> Dict[str, float]:
        ...
        max_crisis_diff = 0.0
        all_crisis_levels = (
            set(train_dist.keys()).union(val_dist.keys()).union(test_dist.keys())
        )
        ...
        return {"max_crisis_level_difference": max_crisis_diff}
    ...
    def balance_dataset(
    ...
    ) -> Tuple[List[Conversation], List[LabelBundle]]:
        """
        ...
        balanced_convs, balanced_labels = (
            zip(*balanced_data) if balanced_data else ([], [])
        )
    ...
    def _oversample(
        self, paired_data: List[Tuple[Conversation, LabelBundle]]
    ) -> List[Tuple[Conversation, LabelBundle]]:
        ...
        for stratum_label, stratum_data in strata.items():
            if len(stratum_data) < max_size:
                ...
                balanced_data.extend(
                    stratum_with_repeats[
                        : max_size
                        - len(
                            [
                                x
                                for x in balanced_data
                                if self._get_crisis_level(x[1]) == stratum_label
                            ]
                        )
                    ]
                )
        ...
        for stratum_label, stratum_data in strata.items():
            if len(stratum_data) < max_size:
                ...
                while synthetic_count < needed and stratum_data:
                    ...
                    augmented_labels = augmenter.augment_label_bundle(
                        orig_labels, augmented_conv
                    )
...
class AdvancedSampler:
    ...
    def cluster_aware_split(
        ...
        """
        if len(conversations) != len(label_bundles) or len(conversations) != len(
            cluster_labels
        ):
            raise ValueError(
                "Conversations, labels, and cluster labels must have the same length"
            )
        ...
        paired_data = list(zip(conversations, label_bundles))
        split_stats = self._calculate_split_statistics(
            paired_data, (train, validation, test)
        )

        return DatasetSplit(
            train=train, validation=validation, test=test, split_stats=split_stats
        )
...
def test_sampling_utilities():
    ...
    print(
        f"  Crisis distribution in train: {stratified_split.split_stats['label_distributions']['train']['crisis_distribution']}"
    )
    print(
        f"  Crisis distribution in validation: {stratified_split.split_stats['label_distributions']['validation']['crisis_distribution']}"
    )
    print(
        f"  Crisis distribution in test: {stratified_split.split_stats['label_distributions']['test']['crisis_distribution']}"
    )

    # Test balancing
    print("\nTesting dataset balancing...")
    balanced_convs, balanced_labels = sampler.balance_dataset(
        conversations, label_bundles, "oversample"
    )
    print(
        f"  Original size: {len(conversations)}, Balanced size: {len(balanced_convs)}"
    )

    # Test cluster-aware split
    print("\nTesting cluster-aware split...")
    cluster_labels = [
        f"cluster_{i % 5}" for i in range(len(conversations))
    ]  # 5 clusters
    advanced_sampler = AdvancedSampler()

    try:
        cluster_split = advanced_sampler.cluster_aware_split(
            conversations, label_bundles, cluster_labels
        )
