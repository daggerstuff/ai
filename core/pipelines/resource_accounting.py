...
class ResourceMonitor:
    ...
    def start_monitoring(self, interval: float = 30.0) -> bool:
        ...
        if self.is_monitoring:
            self.logger.warning(
                f"Resource monitoring already running for run {self.run_id}"
            )
            return False
    ...
    def _update_peak_usage(self, usage: ResourceUsage):
        ...
        if self.peak_usage is None:
            ...
            if (
                usage.gpu_memory_used_gb is not None
                and self.peak_usage.gpu_memory_used_gb is not None
                and usage.gpu_memory_used_gb > self.peak_usage.gpu_memory_used_gb
            ):
                self.peak_usage.gpu_memory_used_gb = usage.gpu_memory_used_gb
            if (
                usage.gpu_utilization_percent is not None
                and self.peak_usage.gpu_utilization_percent is not None
                and usage.gpu_utilization_percent
                > self.peak_usage.gpu_utilization_percent
            ):
    ...
    def _check_limits(self, usage: ResourceUsage) -> List[str]:
        ...
        # Check GPU memory
        if (
            self.budget_limits.max_gpu_memory_gb
            and usage.gpu_memory_used_gb
            and usage.gpu_memory_used_gb > self.budget_limits.max_gpu_memory_gb
        ):
            exceeded.append(
                f"GPU memory ({usage.gpu_memory_used_gb:.2f}GB > {self.budget_limits.max_gpu_memory_gb}GB)"
            )

        # Check system memory
        if (
            self.budget_limits.max_system_memory_gb
            and usage.memory_used_gb > self.budget_limits.max_system_memory_gb
        ):
            exceeded.append(
                f"System memory ({usage.memory_used_gb:.2f}GB > {self.budget_limits.max_system_memory_gb}GB)"
            )

        # Check runtime (if budget has max time limit)
        current_runtime = (time.time() - self.start_time) / 3600
        if (
            self.budget_limits.max_runtime_hours
            and current_runtime > self.budget_limits.max_runtime_hours
        ):
            exceeded.append(
                f"Runtime ({current_runtime:.2f}h > {self.budget_limits.max_runtime_hours}h)"
            )
    ...
    def get_average_usage(self) -> ResourceUsage:
        ...
        # Calculate averages
        cpu_percent = sum(u.cpu_percent for u in self.resource_history) / len(
            self.resource_history
        )
        memory_used_gb = sum(u.memory_used_gb for u in self.resource_history) / len(
            self.resource_history
        )
        ...
        if any(u.gpu_memory_used_gb is not None for u in self.resource_history):
            gpu_values = [
                u.gpu_memory_used_gb
                for u in self.resource_history
                if u.gpu_memory_used_gb is not None
            ]
            if gpu_values:
        ...
        if any(u.gpu_utilization_percent is not None for u in self.resource_history):
            gpu_values = [
                u.gpu_utilization_percent
                for u in self.resource_history
                if u.gpu_utilization_percent is not None
            ]
            if gpu_values:
...
class CostCalculator:
    ...
    def calculate_cost(
        ...
        storage_cost = (
            (self.default_pricing["storage"]["gp3_per_gb_month"] / 730)
            * storage_gb
            * runtime_hours
    ...
    def estimate_remaining_budget(
        self, current_cost: float, budget_limit: float
...
class ResourceManager:
    ...
    def get_run_report(
        self, run_id: str, instance_type: str = "g4dn.xlarge"
    ) -> Optional[ResourceReport]:
        ...
        if monitor.budget_limits:
            if runtime_hours > monitor.budget_limits.max_runtime_hours:
                exceeded_limits.append(
                    f"Runtime exceeded: {runtime_hours:.2f}h > {monitor.budget_limits.max_runtime_hours}h"
                )

            if (
                monitor.budget_limits.max_cost_usd
                and cost_estimation.estimated_cost_usd
                > monitor.budget_limits.max_cost_usd
            ):
                exceeded_limits.append(
                    f"Cost exceeded: ${cost_estimation.estimated_cost_usd:.2f} > ${monitor.budget_limits.max_cost_usd}"
                )
    ...
    def check_budget_alerts(self, run_id: str) -> Optional[Tuple[float, str]]:
        ...
        current_cost = self.cost_calculator.calculate_cost(
            runtime_hours=current_runtime, resources_used=current_usage
        ).estimated_cost_usd
        ...
        if monitor.budget_limits.max_cost_usd > 0:
            ...
            if usage_percentage >= monitor.budget_limits.notification_threshold * 100:
                return (
                    current_cost,
                    f"Budget usage reached {usage_percentage:.1f}% threshold",
                )
...
class BudgetManager:
    ...
    def allocate_budget(self, run_id: str, amount_usd: float) -> bool:
        ...
        if amount_usd > remaining_budget:
            self.logger.error(
                f"Insufficient budget: requested ${amount_usd}, available ${remaining_budget}"
            )
            return False
    ...
    def track_cost(self, run_id: str, cost_usd: float):
        ...
        if self.spent_budget_usd > self.total_budget_usd:
            self.logger.warning(
                f"Total spent exceeds budget: ${self.spent_budget_usd} > ${self.total_budget_usd}"
            )
...
def setup_resource_monitoring(
    run_id: str, budget_limits: Optional[BudgetLimits] = None
) -> ResourceManager:
...
def get_training_cost_estimate(
    resources: ResourceUsage, runtime_hours: float, instance_type: str = "g4dn.xlarge"
) -> CostEstimation:
...
def test_resource_accounting_system():
    ...
    run_id = "test_run_123"
    rm.start_run_monitoring(
        run_id, budget_limits, interval=5.0
    ...
    print(
        f"Current resource usage: CPU {current_usage.cpu_percent}%, Memory {current_usage.memory_used_gb:.2f}GB"
    )
    ...
    if report:
        ...
        print(
            f"  Estimated cost: ${report.cost_estimation.estimated_cost_usd:.2f}"
    ...
    print(
        f"Budget allocation results: Run A: {success1}, Run B: {success2}, Run C: {success3}"
    )
    ...
    print("Tracked costs - Run A: $250, Run B: $100")
    print(f"Remaining budget: ${budget_manager.get_remaining_budget():.2f}")
