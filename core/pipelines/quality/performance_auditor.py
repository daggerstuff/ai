...
class PerformanceAuditor:
    ...
    def audit_directory(self, directory: str | None = None) -> dict[str, Any]:
        ...
        self.logger.info(
            f"Performance audit completed. Found {len(self.issues)} issues."
        )
    ...
    def _audit_file(self, file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            for func in functions:
                func_stats = self._analyze_function_performance(
                    func, lines, str(file_path)
                )
        ...
        def check_io_in_loop(node, in_loop=False):
            ...
            if isinstance(node, (ast.For, ast.While)):
                ...
                if (
                    ...
                    and node.func.attr in ["read", "write", "open"]
                ):
                    count += 1
            else:
                for child in ast.iter_child_nodes(node):
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.Call):
                if (
                    ...
                    and child.func.id in expensive_functions
                ):
    ...
    def _check_file_performance_patterns(
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if self.stats.overall_performance_score < 50:
            recommendations.append(
                "CRITICAL: Performance score is below 50. Immediate optimization required."
            )
        elif self.stats.overall_performance_score < 70:
            recommendations.append(
                "WARNING: Performance score is below 70. Consider optimization."
            )

        if self.stats.algorithmic_complexity_issues > 0:
            recommendations.append(
                f"HIGH PRIORITY: {self.stats.algorithmic_complexity_issues} algorithmic complexity issues found."
            )

        if self.stats.io_bottlenecks > 0:
            recommendations.append(
                f"Optimize {self.stats.io_bottlenecks} I/O bottlenecks for better performance."
            )

        if self.stats.database_query_issues > 0:
            recommendations.append(
                f"Optimize {self.stats.database_query_issues} database query patterns."
            )

        if self.stats.caching_opportunities > 0:
            recommendations.append(
                f"Implement caching for {self.stats.caching_opportunities} expensive operations."
            )
    ...
    def _generate_optimization_priorities(self) -> list[dict[str, Any]]:
        ...
        algorithmic_issues = [
            issue
            for issue in self.issues
            if issue.optimization_category == "ALGORITHMIC_COMPLEXITY"
        ]
        io_issues = [
            issue
            for issue in self.issues
            if issue.optimization_category == "IO_BOTTLENECK"
        ]
