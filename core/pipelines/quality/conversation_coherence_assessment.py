...
class ConversationCoherenceAssessor:
    ...
    def assess_conversation_coherence(
        ...
        if not conversation.messages or len(conversation.messages) < 2:
            return self._create_minimal_assessment(
                conversation, "Insufficient messages for coherence assessment"
            )
    ...
    def _assess_logical_flow(self, conversation: Conversation) -> dict[str, Any]:
        ...
        for i in range(1, len(messages)):
            ...
            if len(curr_msg.content.strip()) < 10 and i > 1:
                issues.append(
                    f"Very short response at turn {i}: '{curr_msg.content.strip()}'"
                )
                score -= 0.15
    ...
    def _assess_contextual_consistency(
        self, conversation: Conversation
    ) -> dict[str, Any]:
    ...
    def _check_pronoun_consistency(self, messages: list[Message]) -> list[str]:
        ...
        if "he" in pronoun_contexts and "she" in pronoun_contexts:
            if len(pronoun_contexts["he"]) > 1 and len(pronoun_contexts["she"]) > 1:
                issues.append(
                    "Potential pronoun inconsistency: both 'he' and 'she' used frequently"
                )
    ...
    def _assess_turn_taking(self, conversation: Conversation) -> dict[str, Any]:
        ...
        if len(role_lengths) > 1:
            ...
            if (
                max_avg > min_avg * 5 and min_avg < 20
            ):  # One role much shorter on average
        ...
        if response_lengths:
            ...
            length_variance = sum(
                (length - avg_length) ** 2 for length in response_lengths
    ...
    def _assess_topic_continuity(self, conversation: Conversation) -> dict[str, Any]:
        ...
        for i in range(1, len(messages)):
            ...
            if prev_content_words and curr_content_words:
                ...
                if (
                    overlap_ratio < 0.1
                    and len(prev_content_words) > 2
                    and len(curr_content_words) > 2
                ):
                    topic_shifts += 1
    ...
    def _assess_response_relevance(self, conversation: Conversation) -> dict[str, Any]:
        ...
        for i in range(1, len(messages)):
            ...
            any(
                word in curr_msg.content.lower()
                for word in ["yes", "no", "maybe", "i think", "i believe", "because"]
            )

            # Check for acknowledgment patterns
            is_acknowledgment = any(
                word in curr_msg.content.lower()
                for word in ["okay", "ok", "i see", "understood", "thanks", "thank you"]
            )

            # Very short responses to long questions might be irrelevant
            if (
                len(prev_msg.content) > 100
                and len(curr_msg.content) < 20
                and not is_acknowledgment
            ):
            ...
            if (
                curr_msg.content.lower().strip() in generic_responses
                and len(prev_msg.content) > 20
            ):
                irrelevant_responses += 1
        ...
        details.update(
            {
                ...
                "relevance_ratio": 1
                - (irrelevant_responses / max(1, len(messages) - 1)),
            }
        )
