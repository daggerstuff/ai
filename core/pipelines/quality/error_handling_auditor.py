...
class ErrorHandlingAuditor:
    ...
    def audit_directory(self, directory_path: str) -> ErrorHandlingAuditReport:
        ...
        logger.info(
            f"Error handling audit complete: {total_files} files, "
                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    for op in unprotected_ops:
                        self._add_error_handling_issue(
                            ...
                            "high",
                            f"Risky operation '{op['operation']}' not protected "
                            "by try-catch",
    ...
    def _get_full_attr_name(self, node: ast.AST) -> str:
        """Safely resolve the full name of an attribute call recursively."""
    ...
    def _generate_recommendations(self) -> None:
        ...
        if issue_types.get("unprotected_operation", 0) > 0:
            self.recommendations.append(
                "Implement try-catch blocks around risky operations "
                "like file I/O, network calls, and database operations"
            )
    ...
    def generate_report(
        ...
        with open(output_path, "w") as f:
            ...
            f.write(
                f"- **Functions with Error Handling**: "
                f"{report.functions_with_error_handling}\n"
            )
            ...
            for severity in ["critical", "high", "medium", "low"]:
                if severity in issues_by_severity:
                    f.write(
                        f"### {severity.title()} "
                        f"({len(issues_by_severity[severity])} issues)\n\n"
                    )
