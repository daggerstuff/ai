        ...
        return {
            CrisisType.SUICIDE_IDEATION: [
                ...
                CrisisIndicator(
                    "direct_statement",
                    1.0,
                    r"(?:i want to die|i'm going to kill myself|i don't want to be alive)",
                    immediate_flag=True,
                ),
                # Method and planning
                CrisisIndicator(
                    "method_mention",
                    0.9,
                    r"(?:pills|gun|rope|bridge|overdose|hanging|jumping|carbon monoxide|knife|razor)",
                    context_required=True,
                ),
                CrisisIndicator(
                    "method_access",
                    0.95,
                    r"(?:have a gun|got the pills|found a rope|know where|easy access to)",
                    context_required=True,
                ),
                CrisisIndicator(
                    "plan_details",
                    0.9,
                    r"(?:have a plan|know how|figured out how|written (?:a )?note|set a date)",
                ),
                # Timeline and urgency
                CrisisIndicator(
                    "timeline_specific",
                    0.95,
                    r"(?:tonight|today|tomorrow|this week|soon|right now|can't wait)",
                    context_required=True,
                ),
                CrisisIndicator(
                    "imminent_action",
                    1.0,
                    r"(?:doing it now|can't stop myself|it's time|this is it)",
                    immediate_flag=True,
                ),
                ...
                CrisisIndicator(
                    "passive_ideation",
                    0.6,
                    r"(?:sometimes think about|wish i was|wouldn't mind if|better off dead|not being here)",
                ),
                CrisisIndicator(
                    "death_wishes",
                    0.7,
                    r"(?:wish i would die|hope i don't wake up|want to disappear|cease to exist)",
                ),
                # Indirect expressions (CRITICAL - was missing)
                CrisisIndicator(
                    "indirect_burden",
                    0.8,
                    r"(?:everyone would be better off without me|burden to everyone|world without me|they'd be happier)",
                ),
                CrisisIndicator(
                    "indirect_planning",
                    0.8,
                    r"(?:getting affairs in order|writing letters|giving things away|saying goodbye)",
                ),
                CrisisIndicator(
                    "no_future",
                    0.7,
                    r"(?:can't see a future|no tomorrow|no point in planning|nothing to live for)",
                ),
                # Emotional indicators
                CrisisIndicator(
                    "hopelessness",
                    0.6,
                    r"(?:no point|nothing left|can't go on|no hope|completely hopeless)",
                ),
                CrisisIndicator(
                    "ideation",
                    0.7,
            ...
            CrisisType.SELF_HARM: [
                ...
                CrisisIndicator(
                    "cutting_escalation",
                    0.9,
                    r"(?:deeper cuts|cutting more|worse cuts|can't stop cutting)",
                ),
                CrisisIndicator(
                    "cutting_tools",
                    0.8,
                    r"(?:razor blade|kitchen knife|broken glass|scissors|sharp object)",
                ),
                # Burning (CRITICAL - was missing patterns)
                CrisisIndicator(
                    "burning_behavior",
                    0.8,
                    r"(?:burn myself|burning my|cigarette burns|lighter burns)",
                ),
                CrisisIndicator(
                    "burning_methods",
                    0.8,
                    r"(?:cigarette|lighter|hot objects|matches|candle) (?:to burn|for burning|on my)",
                ),
                CrisisIndicator(
                    "burning_escalation",
                    0.9,
                    r"(?:bigger burns|more burns|hotter objects|longer burns)",
                ),
                # Physical self-harm
                CrisisIndicator(
                    "hitting",
                ...
                CrisisIndicator(
                    "other_harm",
                    0.7,
                    r"(?:scratching|biting|pinching|pulling hair) (?:myself|until it bleeds)",
                ),
                ...
                CrisisIndicator(
                    "compulsive_harm",
                    0.8,
                    r"(?:can't stop|have to hurt|must cut|compelled to harm)",
                ),
                ...
                CrisisIndicator(
                    "tool_acquisition",
                    0.8,
                    r"(?:bought razors|hiding sharp objects|collecting knives|found something sharp)",
                ),
                # Escalation patterns
                CrisisIndicator(
                    "escalation",
                ...
                CrisisIndicator(
                    "frequency_increase",
                    0.8,
                    r"(?:every day|multiple times|more often|can't go without)",
                ),
            ],
            ...
            CrisisType.PSYCHOTIC_EPISODE: [
                ...
                CrisisIndicator(
                    ...
                    0.8,
                    r"(?:seeing things|shadows|people who aren't there|things moving|faces in walls)",
                ),
                CrisisIndicator(
                    "auditory_hallucinations",
                    0.8,
                    r"(?:hearing voices|voices in my head|someone talking|whispers|sounds)",
                ),
                # Command hallucinations (CRITICAL - was missing)
                CrisisIndicator(
                    "command_voices",
                    0.95,
                    r"(?:voices tell me to|commanded to|must obey|voices say (?:to|i should)|telling me to)",
                    immediate_flag=True,
                ),
                CrisisIndicator(
                    "command_harm",
                    1.0,
                    r"(?:voices tell me to hurt|command me to kill|must hurt|voices say hurt)",
                    immediate_flag=True,
                ),
                # Delusions
                CrisisIndicator(
                ...
                CrisisIndicator(
                    "persecution",
                    0.8,
                    r"(?:out to get me|plotting against|trying to harm|being hunted)",
                ),
                # Disorganization
                CrisisIndicator(
                    "disorganized",
                ...
                CrisisIndicator(
                    "thought_disorder",
                    0.7,
                    r"(?:thoughts scattered|can't focus|mind jumping|thoughts racing)",
                ),
            ],
            ...
            CrisisType.PANIC_ATTACK: [
                ...
                CrisisIndicator(
                    "panic_breathing",
                    0.8,
                    r"(?:can'?t breathe|can'?t catch my breath|suffocating|gasping for air)",
                ),
                CrisisIndicator(
                    "panic_heart",
                    0.8,
                    r"(?:heart racing|heart pounding|chest pounding|heart beating fast)",
                ),
                CrisisIndicator(
                    "panic_chest",
                    0.8,
                    r"(?:chest tight|chest crushing|chest pain|pressure on chest)",
                ),
                CrisisIndicator(
                    "panic_physical",
                    0.7,
                    r"(?:dizzy|shaking|trembling|sweating|nauseous|tingling)",
                ),
                # Cognitive symptoms (CRITICAL - was missing)
                CrisisIndicator(
                    "panic_death_fear",
                    0.8,
                    r"(?:going to die|think i'?m dying|having a heart attack|this is it)",
                ),
                CrisisIndicator(
                    "panic_control_fear",
                    0.8,
                    r"(?:losing control|going crazy|losing my mind|can'?t control)",
                ),
                CrisisIndicator(
                    "panic_escape",
                    0.7,
                    r"(?:need to get out|can'?t escape|trapped|have to leave)",
                ),
                # Intensity and behavioral symptoms
                CrisisIndicator(
                    "panic_intensity",
                    0.7,
                    r"(?:panic attack|overwhelming fear|terror|can'?t calm down|worst feeling)",
                ),
                CrisisIndicator(
                    "panic_behavioral",
                    0.6,
                    r"(?:can'?t sit still|pacing|running away|hiding|need help)",
                ),
                # Panic-specific language
                CrisisIndicator(
                    "panic_language",
                    0.7,
                    r"(?:panic|panicking|freaking out|losing it|can'?t handle)",
                ),
            ],
            CrisisType.EATING_DISORDER_CRISIS: [
                # Medical emergency symptoms (CRITICAL - was completely missing)
                CrisisIndicator(
                    "ed_medical_emergency",
                    0.9,
                    r"(?:fainting|dizzy|weak|heart problems|electrolyte|medical emergency)",
                ),
                CrisisIndicator(
                    "ed_physical_symptoms",
                    0.8,
                    r"(?:losing hair|teeth problems|can'?t stand|chest pain|irregular heartbeat)",
                ),
                # Behavioral escalation (CRITICAL - was missing)
                CrisisIndicator(
                    "purging_escalation",
                    0.8,
                    r"(?:purging multiple times|throwing up all day|can'?t stop purging|vomiting blood)",
                ),
                CrisisIndicator(
                    "restriction_extreme",
                    0.8,
                    r"(?:haven't eaten for days|refusing all food|starving myself|nothing for weeks)",
                ),
                CrisisIndicator(
                    "binge_escalation",
                    0.7,
                    r"(?:can'?t stop eating|ate everything|binge all day|out of control eating)",
                ),
                # Psychological distress (CRITICAL - was missing)
                CrisisIndicator(
                    "ed_psychological",
                    0.7,
                    r"(?:hate my body|disgusting|can'?t look in mirror|want to disappear)",
                ),
                CrisisIndicator(
                    "ed_control_loss",
                    0.8,
                    r"(?:can'?t control|completely out of control|food controls me|obsessed)",
                ),
                ...
                CrisisIndicator(
                    ...
                    0.6,
                    r"(?:not eating|skipping meals|avoiding food)",
                ),
                CrisisIndicator(
                    "binge_behavior",
                    0.6,
                    r"(?:binge eating|eating too much|can'?t stop eating)",
                ),
                CrisisIndicator(
                    "body_dysmorphia",
                    0.5,
                    r"(?:so fat|too big|body image|weight obsession)",
                ),
            ],
        }
