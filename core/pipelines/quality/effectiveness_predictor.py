...
class TherapeuticEffectivenessPredictor:
    ...
    def _extract_therapeutic_elements(
        ...
        alliance_score = sum(
            1 for indicator in alliance_indicators if indicator in content_lower
        )
        ...
        skill_score = sum(
            1 for indicator in skill_indicators if indicator in content_lower
        )
    ...
    def _identify_risk_factors(
        self, content: str, turns: list[dict], metadata: dict
    ) -> list[str]:
    ...
    def _predict_outcome_metrics(
        ...
        for outcome_type, _model in self.outcome_models.items():
            ...
            base_score = self._calculate_base_outcome_score(
                outcome_type, therapeutic_elements
            )
            ...
            predicted_score = max(
                0.0, min(1.0, base_score + risk_adjustment + protective_adjustment)
            )

            # Calculate improvement percentage
            baseline_score = 0.3  # Assume moderate baseline impairment
            improvement_percentage = max(
                0.0, (predicted_score - baseline_score) / baseline_score * 100
            )
            ...
            evidence_strength = self._assess_evidence_strength(
                therapeutic_elements, outcome_type
            )
    ...
    def _calculate_base_outcome_score(
        self, outcome_type: OutcomeType, elements: dict[str, Any]
    ) -> float:
    ...
    def _determine_prediction_confidence(
        ...
        if (
            len(therapeutic_elements["interventions"]) >= 2
            and therapeutic_elements["therapeutic_alliance"] > 0.7
            and len(risk_factors) + len(protective_factors) >= 3
        ):
            return PredictionConfidence.HIGH

        # Moderate confidence: some therapeutic elements, some factors
        if (
            len(therapeutic_elements["interventions"]) >= 1
            and therapeutic_elements["therapeutic_alliance"] > 0.4
            and len(risk_factors) + len(protective_factors) >= 1
        ):
    ...
    def _assess_evidence_strength(
        self, elements: dict[str, Any], outcome_type: OutcomeType
    ) -> str:
        ...
        for intervention in interventions:
            if intervention in self.evidence_base["intervention_effectiveness"]:
                ...
                conditions = self.evidence_base["intervention_effectiveness"][
                    intervention
                ]
                if any(
                    data.get("evidence") == "strong" for data in conditions.values()
                ):
    ...
    def _calculate_overall_effectiveness(
        self, outcome_metrics: list[OutcomeMetric]
    ) -> EffectivenessLevel:
    ...
    def _calculate_success_probability(
        ...
        base_probability = sum(
            metric.predicted_score for metric in outcome_metrics
        ) / len(outcome_metrics)
    ...
    def _generate_recommendations(
        ...
        if not therapeutic_elements["interventions"]:
            ...
            recommendations.append(
                "Consider integrating additional therapeutic approaches"
            )
        ...
        for metric in low_outcome_metrics:
            if metric.metric_type == OutcomeType.SYMPTOM_REDUCTION:
                recommendations.append(
                    "Increase focus on symptom-specific interventions"
                )
            elif metric.metric_type == OutcomeType.FUNCTIONAL_IMPROVEMENT:
                recommendations.append(
                    "Incorporate behavioral activation and skill building"
                )

        return list(set(recommendations))  # Remove duplicates

    def _compile_evidence_base(
        self, therapeutic_elements: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for intervention in therapeutic_elements["interventions"]:
            if intervention in self.evidence_base["intervention_effectiveness"]:
                intervention_data = self.evidence_base["intervention_effectiveness"][
                    intervention
                ]
                ...
                for condition, data in intervention_data.items():
                    evidence["evidence_ratings"][intervention][condition] = data.get(
                        "evidence", "unknown"
                    )
                    evidence["effect_sizes"][intervention][condition] = data.get(
                        "effect_size", 0.0
                    )
                    evidence["success_rates"][intervention][condition] = data.get(
                        "success_rate", 0.0
                    )
    ...
    def get_prediction_summary(self) -> dict[str, Any]:
        ...
        avg_success_probability = (
            sum(p.success_probability for p in self.prediction_history)
            / total_predictions
        )

        # Most common risk factors
        all_risk_factors = [
            factor for p in self.prediction_history for factor in p.risk_factors
        ]
        risk_factor_counts = {}
        ...
        all_protective_factors = [
            factor for p in self.prediction_history for factor in p.protective_factors
        ]
        ...
        return {
            ...
            "common_protective_factors": dict(
                sorted(
                    protective_factor_counts.items(), key=lambda x: x[1], reverse=True
                )[:5]
            ),
    ...
    sample_conversations = [
        {
            ...
            "turns": [
                {
                    "speaker": "user",
                    "text": "I've been depressed for months and nothing helps.",
                },
                {
                    "speaker": "therapist",
                    "text": "Let's work together using CBT techniques. I believe we can make progress.",
                },
            ],
        ...
        },
        {
            ...
            "turns": [
                {
                    "speaker": "user",
                    "text": "I have good family support and want to get better.",
                },
                {
                    "speaker": "therapist",
                    "text": "These are great strengths. Let's use mindfulness and coping strategies.",
                },
            ],
