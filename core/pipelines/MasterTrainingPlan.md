...
2. Cross-reference existing guides:
   - `ai/pipelines/MENTAL_HEALTH_RESOURCES_GUIDE.md`
   - `ai/pipelines/architecture/dataset_training_architecture.md`
   - `ai/pipelines/INTEGRATED_PIPELINE_GUIDE.md`
...
|-------|--------------|---------|-----------------|-----------------------------|
| **Stage 1 – Foundation & Rapport** (`stage1_foundation`) | 40% | Establish baseline therapeutic tone, reflective listening, and low-risk support dialogs. | Processed Phase 1 & 2 datasets (`ai/orchestrator/targets/pixelated-training/processed`), Wendy priority JSONL (`ai/orchestrator/targets/ghost/datasets/priority_*`), pro datasets (`therapist_sft`, `SoulChat`, `counsel_chat`, `Psych8k`, `mental_health_counseling`, `neuro_qa_sft`). | Require empathy score ≥ 0.55, no structural corruption, dedup preference: consolidated `priority_*` overrides raw HF pulls. |
| **Stage 2 – Therapeutic Expertise & Reasoning** (`stage2_therapeutic_expertise`) | 25% | Teach structured reasoning, diagnosis scaffolding, knowledge grounding, MEMO summarization style. | CoT corpora in `ai/pipelines/cot_datasets/`, reasoning JSON under `ai/training_data_consolidated/psychology_knowledge`, DPO preference samples, MEMO (when licensed), professional PDF/book extracts. | Must have reasoning metadata (`chain_of_thought`, `summary`, `technique`). Empathy floor 0.5 but allow sterile clinical tone. Dedup keeps highest-quality reasoning path. |
| **Stage 3 – Edge Stress Test & Scenario Bank** (`stage3_edge_stress_test`) | 20% | Force model to survive nightmare fuel: crisis, trauma, Reddit rawness, edge generator prompts. | `ai/pipelines/edge_case/output/edge_cases_training_format.jsonl`, scenario prompt corpus (Google Drive exports mirrored to `ai/pipelines/prompt_corpus/`), Reddit/Kaggle TF-IDF dumps, Synthetic edge expansions, Safety DPO violations for contrastive pairs. | **Do NOT** sanitize horror details. Only drop malformed JSON or PII (phone, SSN). Flag metadata `{"crisis_intensity": "very_high"}` for gating. Allow safety validator to mark `requires_human_review` without removal. |
...
Use `ConversationRecord` from `ai/pipelines/schemas/conversation_schema.py`:
```json
{
...
- Use `ai/pipelines/ingestion_deduplication.py` (or `processing/deduplication_*`) to generate these hashes and resolve collisions.
...
Modify `ai/pipelines/orchestration/integrated_training_pipeline.py`:
...
4. **Splits**: for each stage output `train/val/test = 80/10/10`, plus aggregated splits consumed by `ai/orchestrator/targets/pixelated-training/training_dataset.json`.
5. **Monitoring hooks**: reuse `ai/pipelines/quality_control.py` & `evaluation_gates.py` to emit:
   ```json
   {
     "stage": "stage3_edge_stress_test",
     "samples": 42000,
...
2. **Scenario Prompt Corpus**
   - Mirror Google Drive prompt sheets to repo under `ai/pipelines/prompt_corpus/{category}.md`.
...
uv run python ai/pipelines/scripts/pre_ingest_checks.py  # validates mounts, MEMO access
uv run python ai/pipelines/processing/unified_preprocessing_pipeline.py --stage all
uv run python ai/pipelines/orchestration/integrated_training_pipeline.py --staged --report reports/stage_report.json
```
...
- [ ] Scenario prompts mirrored to `ai/pipelines/prompt_corpus/`.
...
- `ai/pipelines/ingestion_deduplication.py`
- `ai/pipelines/quality/*`
