...
This document provides the complete blueprint for consolidating all Pixelated Empathy training datasets into a cohesive, multi-stage training pipeline. The plan leverages the operational `ai/pipelines/` system, preserves critical edge case training data (without safety filtering), and defines a staged curriculum from foundation to voice/personality injection.
...
The `ai/pipelines/` is now operational per completed epic with:
...
- **Training orchestrator**: `ai/pipelines/training_orchestrator.py` for H100 execution
- **6-stage pipeline**: Orchestration via `ai/pipelines/orchestration/pipeline_orchestrator.py`

### Harmonization Process

**Stage 1: Normalization**
- Use `ai/pipelines/schemas/conversation_schema.py` for unified format
...
- Leverage `ai/pipelines/orchestration/tier_processor.py` for cluster handling
- Process datasets by tier (1-6) with appropriate quality thresholds
- Apply tier-weighted sampling via `ai/pipelines/composition/tier_balancer.py`

**Stage 3: Deduplication**
- Use Bloom-filter deduplication from `ai/pipelines/ingestion_deduplication.py`
...
3. **Source Tracking**: Edge cases come from:
   - `ai/orchestrator/targets/ghost/edge_case_pipeline_standalone/` (edge case generator)
...
Modify `ai/pipelines/quality/` validators to:
```python
...
**Stage 1: Foundation**
- Empathy score: ≥0.7 (via `ai/pipelines/quality/empathy_mental_health_validator.py`)
...
- **Empathy Scoring**: `ai/pipelines/quality/empathy_mental_health_validator.py`
  - EMNLP 2020 empathy dimensions (ER, IP, EX, VAL)
  - Overall empathy score: 0-1 scale
  
- **Safety Validation**: `ai/pipelines/quality/safety_alignment_validator.py`
  - Crisis content detection
  - Toxic language detection
  - PII exposure prevention
  - **BYPASSED for edge cases**

- **Coherence Validation**: `ai/pipelines/quality/coherence_validator.py`
...
**Datasets:**
- Edge case generator outputs: `ai/orchestrator/targets/ghost/edge_case_pipeline_standalone/`
...
**Location:**
- Generator: `ai/orchestrator/targets/ghost/edge_case_pipeline_standalone/`
...
- Pipeline: `ai/pipelines/voice_pipeline_integration.py`
...
1. **Save this master plan** to `ai/pipelines/MASTER_DATASET_PLAN.md` ✅
...
2. **Dataset Pipeline is Operational**
   - Use existing `ai/pipelines/` infrastructure
...
1. ✅ **Master Plan Document**: This file (`ai/pipelines/MASTER_DATASET_PLAN.md`)

2. **Dataset Inventory JSON**: `ai/pipelines/scripts/dataset_inventory.json`
   - Complete mapping of all datasets
   - Download commands for each source
   - Destination paths and checksums

3. **Execution Scripts**:
   - `ai/pipelines/scripts/vps_dataset_acquisition.sh` - VPS download automation
   - `ai/pipelines/scripts/transfer_to_vps.sh` - Local-to-VPS transfer
   - `ai/pipelines/scripts/upload_to_ovh_s3.sh` - VPS-to-S3 upload

4. **Inventory Files**:
   - `ai/pipelines/scripts/VPS_INVENTORY.json` - VPS dataset inventory
   - `ai/pipelines/scripts/S3_INVENTORY.json` - OVH S3 dataset inventory
