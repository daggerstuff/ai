...
uv run python ai/pipelines/verify_pipeline.py
```
...
- Override with `DATASET_PIPELINE_OUTPUT_DIR` (see `ai/pipelines/storage.env.template`)
...
```bash
uv run python -m ai.pipelines.orchestrator.export_dataset \
    --version 1.0.0 \
    --target-samples 1000 \
    --seed 42
```
...
```python
from ai.pipelines.orchestrator.export_manifest import DatasetManifest
...
- `ai/orchestrator/targets/moe_training_config.json` - Model and training config
- `ai/orchestrator/targets/train_optimized.py` - Training script

### Execute Training

```bash
cd ai/orchestrator/targets
...
After training:
- Model checkpoints in `ai/orchestrator/targets/checkpoints/v1.0.0/`
- Training logs
...
checkpoint_path = Path("ai/orchestrator/targets/checkpoints/v1.0.0/best_model.pt")
...
- **Checkpoints**: `ai/orchestrator/targets/checkpoints/v{VERSION}/`
...
2. Review error logs
3. Check `ai/pipelines/IMPLEMENTATION_SUMMARY.md` for architecture details
