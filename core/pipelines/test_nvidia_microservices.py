...
class TestNvidiaMicroservicesIntegration(unittest.TestCase):
    ...
    @patch("ai.pipelines.youtube_rag_system.NemoRetrieverClient")
    @patch("ai.pipelines.youtube_rag_system.SentenceTransformer", create=True)
    ...
    @patch("ai.pipelines.evaluation_system.NemoEvaluatorClient")
    @patch("ai.pipelines.evaluation_system.AccuracyEvaluator", create=True)
    @patch("ai.pipelines.evaluation_system.SafetyEvaluator", create=True)
    @patch("ai.pipelines.evaluation_system.FairnessEvaluator", create=True)
    @patch(
        "ai.pipelines.evaluation_system.TherapeuticResponseEvaluator",
        create=True,
    )
    def test_evaluator_integration(self, m1, m2, m3, m4, mock_client):
        ...
        with patch("ai.pipelines.evaluation_system.USE_NVIDIA_EVALUATOR", True):
