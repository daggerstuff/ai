...
class ResourceMonitorCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        if torch.cuda.is_available():
            ...
            gpu_memory_pct = current_gpu_memory / (
            ...
            logs["resources/gpu_utilization"] = (
...
class SafetyCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        if train_loss > 1000:  # This is an arbitrary high value as example
            ...
            logger.warning(
                f"Safety alert: High training loss detected at step {state.global_step}: {train_loss}"
            )
            ...
            if checkpoint_dir.is_dir():
                ...
                if metadata_path.exists():
                    try:
                        ...
                        logger.warning(
                            f"Could not read metadata for checkpoint: {checkpoint_dir}"
                        )
...
class TrainingRunner:
    ...
    def load_dataset(self) -> Dataset:
        ...
        if not self.manifest.dataset.path or not os.path.exists(
            self.manifest.dataset.path
        ):
            raise FileNotFoundError(
                f"Dataset file not found: {self.manifest.dataset.path}"
            )
                    ...
                    wandb.log(
                        {
                            ...
                            "dataset/avg_text_length": np.mean(
                    ...
                    wandb.log(
                        {
                            ...
                            "training/total_parameters": sum(
                                p.numel() for p in self.model.parameters()
                            ),
                            "training/trainable_parameters": sum(
                                p.numel()
                    ...
                    wandb.log(
                        {
                            ...
                            "training/runtime_hours": train_result.metrics.get(
                                "train_runtime", 0
                            )
                            / 3600,
                        }
...
# Training command
CMD ["python", "-m", "ai.pipelines.training_runner", "--manifest", "/workspace/config/manifest.json"]
"""
    ...
    def run_in_container(
        self,
        image_name: str = "pixelated-empathy-trainer:latest",
        ...
        cmd.extend(
            [
                "-v",
                f"{os.getcwd()}:/workspace/data",
                "-v",
                f"{self.manifest.output_dir}:/workspace/model_output",
                "-v",
                f"{self.manifest.log_dir}:/workspace/logs",
                "-e",
