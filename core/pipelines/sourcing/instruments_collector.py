...
class ResearchInstrumentCollector:
    ...
    def __init__(self, output_base_path: str = "ai/training_ready/data/datasets"):
        ...
        self.clinical_instruments_path = (
            self.output_base_path / "stage2_reasoning" / "clinical_instruments"
        )
        self._ensure_directories()

        logger.info(
            f"Initialized ResearchInstrumentCollector. Output path: {self.clinical_instruments_path}"
        )
    ...
    def collect_instruments(self) -> list[dict]:
        ...
        return [
            ...
            },
            {
                ...
                "compliance_check": "public_domain",
                "items": [
                    "In the past 30 days, how often did you feel tired out for no good reason?"
                ],
            },
            {
                "name": "MDQ",
    ...
    def validate_compliance(self, instruments: list[dict]) -> list[dict]:
        ...
        for inst in instruments:
            ...
            if inst.get("compliance_check") in ["public_domain", "licensed"]:
                ...
                logger.warning(
                    f"Skipping instrument {inst.get('name')} due to compliance check failure."
                )
        return validated
    ...
    def export_data(self, data: list[dict]):
        ...
        output_file = (
            self.clinical_instruments_path / "validated_instruments_batch_001.json"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully exported {len(data)} instruments to {output_file}"
            )
