...
class GCSConnector(IngestionConnector):
    ...
    def fetch(self) -> Iterable[IngestRecord]:
        ...
        try:
            ...
            for blob in blobs:
                ...
                try:
                    ...
                    if self.rate_limiter:
                        ...
                        if not acquired:
                            raise IngestionError(
                                f"Rate limiter timeout reading {blob_name}"
                            )
                    ...
                    if not self.validate(rec):
                        ...
                        errors = [
                            f"Source validation failed for {self.name} connector: {blob_name}"
                        ]
            ...
            raise IngestionError(
                f"Error listing blobs in bucket {self.config.bucket_name}: {e}"
            )
