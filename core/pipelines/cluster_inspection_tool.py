...
from .processing.deduplication import ConversationDeduplicator
from .processing.tfidf_clusterer import ClusteringResult, TFIDFClusterer
...
class ClusterInspector:
    ...
    def inspect_cluster(
        self, cluster_id: int, clustering_result: ClusteringResult
    ) -> Dict[str, Any]:
    ...
    def find_cluster_by_topic(
        self, topic: str, clustering_result: ClusteringResult
    ) -> List[Dict[str, Any]]:
    ...
    def apply_manual_override(
        ...
        override = ManualOverride(
            ...
            metadata={"override_type": override_type, "item_id": item_id},
        )
...
class DeduplicationInspector:
    ...
    def inspect_duplicate_group(
        self, group_index: int, duplicate_groups: List[List[str]]
    ) -> Dict[str, Any]:
    ...
    def list_duplicate_groups(
        self, duplicate_groups: List[List[str]], min_size: int = 2
    ) -> List[Dict[str, Any]]:
    ...
    def apply_manual_override(
        ...
        override_id = (
            f"dedup_override_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{item_id}"
        )
        ...
        # Log the override
        print(
            f"Applied deduplication override: {override_id} for {item_id} ({override_type})"
        )
...
def create_cluster_inspection_cli():
    ...
    def inspect_clusters():
        ...
        print(
            "  override <type> <id> <old_val> <new_val> <reason> - Apply manual override"
        )
...
def example_dedup_inspection():
    ...
    print(
        "- Perform deduplication: unique, result = deduplicator.deduplicate_conversations(convs)"
    )
    print("- Create inspector: inspector = DeduplicationInspector(deduplicator)")
    print(
        "- Inspect: details = inspector.inspect_duplicate_group(group_idx, result.duplicate_groups)"
    )
