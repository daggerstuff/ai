...
try:
    from ai.core.pipelines.processing.pii_scrubber import PIIScrubber
except ImportError:
    PIIScrubber = None

# Try to import quality scoring (optional dependency)
try:
    from ai.core.pipelines.quality.quality_scoring_v1 import QualityScoringV1
...
class UnifiedPreprocessingPipeline:
    ...
    def _discover_local_generated_synthetic(self) -> None:
        ...
        generated_dir = Path("ai/training_ready/data/datasets/cache/local")
    ...
    def _discover_local_academic_findings(self) -> None:
        ...
        academic_dir = Path(
            "ai/training_ready/data/datasets/stage2_reasoning/academic_literature"
        )
    ...
    def _discover_local_therapeutic_books(self) -> None:
        ...
        books_dir = Path(
            "ai/training_ready/data/datasets/cache/local/therapeutic_books"
        )
    ...
    def _discover_local_generated_youtube_transcripts(self) -> None:
        ...
        youtube_dir = Path(
            "ai/training_ready/data/datasets/cache/local/youtube_transcripts"
        )
    ...
    def _discover_local_journal_findings(self) -> None:
        ...
        journal_dir = Path(
            "ai/training_ready/data/datasets/stage2_reasoning/journal_research"
        )
        if not journal_dir.exists():
            return
