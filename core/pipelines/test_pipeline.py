...
def test_pipeline():
    ...
    try:
        ...
        print(
            f"    Target quality threshold: {pipeline.config.target_quality_threshold}"
        )
        print(f"    Deduplication enabled: {pipeline.config.deduplication_enabled}")
        print(
            f"    Safety filtering enabled: {pipeline.config.safety_filtering_enabled}"
        )
        print(
            f"    Psychology integration enabled: {pipeline.config.psychology_integration_enabled}"
        )
        ...
        for i, source in enumerate(pipeline.data_sources):
            print(
                f"  {i + 1}. {source.name} ({source.source_type}) - {source.size_bytes} bytes"
            )
