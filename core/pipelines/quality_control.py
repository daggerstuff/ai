...
class QualityController:
    ...
    def __init__(
        ...
        label_quality_threshold: float = 0.7,
    ):  # Minimum label quality score
    ...
    def calculate_cohens_kappa(
        self, rater1_labels: List[str], rater2_labels: List[str]
    ) -> float:
        """
        ...
        observed_agreement = sum(
            1 for a, b in zip(rater1_labels, rater2_labels) if a == b
        ) / len(rater1_labels)
    ...
    def calculate_fleiss_kappa(
        self, annotations: List[List[Tuple[str, float]]]
    ) -> float:
        """
        ...
        total_annotations = sum(
            len(item_annotations) for item_annotations in annotations
        )
        ...
        for item_annotations in annotations:
            ...
            if len(item_annotations) > 1:
                ...
                max_possible_agreements = (
                    len(item_annotations) * (len(item_annotations) - 1) / 2
                )
    ...
    def assess_inter_annotator_agreement(
        ...
        if len(label_bundles) < 2:
            return QualityMetric(
                ...
                details={
                    "message": "Insufficient annotators for agreement calculation"
                },
            )
                ...
                for bundle in bundles:
                    ...
                    for label in bundle.therapeutic_response_labels:
                        item_annotations.append(
                            (label.response_type.value, label.metadata.confidence)
                        )
                all_annotations.append(item_annotations)

        if (
            not all_annotations
            or max(len(item_annotations) for item_annotations in all_annotations) < 2
        ):
            return QualityMetric(
                metric_type=QualityMetricType.INTER_ANNOTATOR_AGREEMENT,
                value=0.0,
    ...
    def monitor_label_drift(
        self, history: LabelHistory, window_size: int = 10
    ) -> QualityMetric:
        ...
        for i in range(1, len(recent_versions)):
            ...
            if prev_bundle and curr_bundle:
                ...
                prev_confidence = self._calculate_bundle_confidence(
                    prev_bundle.label_data
                )
                curr_confidence = self._calculate_bundle_confidence(
                    curr_bundle.label_data
                )
    ...
    def assess_confidence_stability(self, history: LabelHistory) -> QualityMetric:
        ...
        if len(confidence_scores) < 2:
            return QualityMetric(
                ...
                timestamp=datetime.utcnow().isoformat(),
                details={
                    "message": "Insufficient confidence scores for stability analysis"
                },
            )
        ...
        if mean_conf == 0:
            ...
            stability_score = (
                std_dev / abs(mean_conf) if mean_conf != 0 else float("inf")
            )
    ...
    def assess_category_balance(
        ...
        if len(proportions) <= 1:
            ...
            if mean_prop == 0:
                ...
                balance_score = (
                    std_dev / abs(mean_prop) if mean_prop != 0 else float("inf")
                )
        ...
        details = {
            ...
            "proportions": {
                cat: count / total_labels for cat, count in category_counts.items()
            },
        }
    ...
    def perform_quality_assessment(
        ...
        version_manager: Optional[LabelVersionManager] = None,
    ) -> List[QualityMetric]:
    ...
    def _create_alert(
        self, metric_type: QualityMetricType, severity: str, message: str
    ):
    ...
    def get_quality_report(self) -> Dict[str, Any]:
        ...
        passing_metrics = sum(
            1 for alert in self.alerts if alert.severity in ["low", "medium"]
        )
        critical_issues = sum(
            1 for alert in self.alerts if alert.severity in ["high", "critical"]
        )
    ...
    def calculate_label_quality_score(self, label_bundle: LabelBundle) -> float:
        ...
        if label_bundle.mental_health_condition_label:
            scores.append(
                label_bundle.mental_health_condition_label.metadata.confidence
            )
    ...
    def assess_label_quality_metrics(
        self,
        label_bundles: List[LabelBundle],
        annotator_performance: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        ...
        quality_scores = [
            self.calculate_label_quality_score(bundle) for bundle in label_bundles
        ]
        all_confidences = []
        ...
        high_confidence_ratio = sum(
            1 for conf in all_confidences if conf >= self.minimum_confidence_threshold
        ) / len(all_confidences)
        ...
        stability_threshold_met = (
            confidence_std_dev <= self.confidence_stability_threshold
        )

        minimum_threshold_met = (
            confidence_threshold_met
            and quality_threshold_met
            and stability_threshold_met
        )
    ...
    def generate_quality_report_with_metrics(
        self,
        label_bundles: List[LabelBundle],
        version_manager: Optional[LabelVersionManager] = None,
    ) -> Dict[str, Any]:
    ...
    def _assess_annotator_performance(
        self, label_bundles: List[LabelBundle]
    ) -> Dict[str, Any]:
        ...
        for bundle in label_bundles:
            ...
            if (
                ...
                and bundle.therapy_modality_label.metadata.annotator_id
            ):
        ...
        for annotator_id, stats in annotator_stats.items():
            if stats["total_labels"] > 0:
                stats["average_confidence"] = (
                    stats["total_confidence"] / stats["total_labels"]
                )
            else:
...
class LabelDriftDetector:
    ...
    def detect_drift_by_provenance(
    ...
    def detect_drift_by_confidence(
        self, histories: List[LabelHistory]
    ) -> Dict[str, Any]:
            ...
            for version in history.versions:
                ...
                if bundle:
                    ...
                    confidence_over_time.append(
                        {"timestamp": version.timestamp, "confidence": avg_conf}
                    )
        ...
        avg_drift = (
            overall_drift / len(all_confidence_trends) if all_confidence_trends else 0.0
        )
...
def test_quality_control():
    ...
    from .label_versioning import create_version_manager
    ...
    for metric in metrics:
        print(
            f"  {metric.metric_type.value}: {metric.value:.3f} ({'PASS' if metric.is_passing else 'FAIL'})"
        )
        if metric.details:
    ...
    if bundles:
        ...
        histories = [
            vm.get_history(bundle.label_id)
            for bundle in bundles
            if vm.get_history(bundle.label_id)
        ]
