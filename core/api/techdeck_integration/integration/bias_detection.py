...
class BiasDetectionClient:
    ...
    async def detect_bias(
        ...
        try:
            ...
            result = await self._make_request_with_retry(
                "POST", f"{self.service_url}/api/v1/bias/detect", payload
            )

            # Parse response
            bias_result = self._parse_bias_detection_result(
                result, operation_id, dataset_id
            )
            ...
            logger.error(
                f"Bias detection failed for operation {operation_id}: {str(e)}"
            )
            raise BiasDetectionError(f"Bias detection failed: {str(e)}") from e

    async def validate_bias_threshold(
        self, bias_result: BiasDetectionResult, threshold: float = 0.7
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Bias validation passed for operation {bias_result.operation_id}"
            )
            return True
    ...
    async def get_bias_recommendations(
        self, bias_result: BiasDetectionResult
    ) -> List[str]:
        """
    ...
    def _parse_bias_detection_result(
        ...
        try:
            ...
            raise BiasDetectionError(
                f"Invalid bias detection response format: {str(e)}"
            ) from e
...
class BiasDetectionManager:
    ...
    async def analyze_dataset(
        ...
        if not self.enabled:
            logger.info(
                f"Bias detection disabled, skipping analysis for {operation_id}"
            )
...
async def validate_bias_compliance(
    bias_result: BiasDetectionResult, threshold: float = 0.7
) -> bool:
    """
