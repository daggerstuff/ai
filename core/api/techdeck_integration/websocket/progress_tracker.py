...
class ProgressTracker:
    ...
    def initialize_handlers(self):
        ...
        def handle_disconnect():
            ...
            try:
                ...
                self.logger.info(
                    f"WebSocket connection closed for session {request.sid}"
                )
        ...
        def handle_subscribe_progress(data):
            ...
            try:
                ...
                self.logger.info(
                    f"User {user_id} subscribed to progress for operation {operation_id}"
                )
        ...
        def handle_unsubscribe_progress(data):
            ...
            try:
                ...
                self.logger.info(
                    f"User {user_id} unsubscribed from progress for operation {operation_id}"
                )
    ...
    def start_progress_tracking(
        self, operation_id: str, user_id: str, operation_type: str, total_steps: int = 1
    ):
        ...
        try:
            ...
            self.redis_client.set_json(
                progress_key, progress_data, ex=3600
            )  # 1 hour TTL
        ...
        try:
            ...
            if not current_progress:
                raise ValidationError(
                    f"Progress tracking not found for operation {operation_id}"
                )
            ...
            if current_step != current_progress.get("current_step"):
                current_progress["current_step_number"] = (
                    current_progress.get("current_step_number", 0) + 1
                )
            ...
            self.logger.debug(
                f"Updated progress for operation {operation_id}: {progress_percent}% - {message}"
            )
    ...
    def cancel_operation(
        self, operation_id: str, user_id: str, message: str = "Operation cancelled"
    ):
        """Cancel an ongoing operation."""
        try:
            self.update_progress(
    ...
    def _broadcast_progress(self, operation_id: str, event: ProgressEvent):
        ...
        try:
            ...
            self.logger.debug(
                f"Broadcasted progress update for operation {operation_id}"
            )
