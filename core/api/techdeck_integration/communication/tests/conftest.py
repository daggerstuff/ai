...
async def performance_monitor(mock_redis_client, test_config):
    ...
    performance_monitor = PerformanceMonitor(
        mock_redis_client, test_config["performance"]
    )
...
async def graceful_degradation_manager(event_bus, test_config):
    ...
    degradation_manager = GracefulDegradationManager(
        event_bus, test_config["graceful_degradation"]
    )
    ...
    class MockBiasService:
        ...
        async def analyze_data(self, data: Dict[str, Any], stage: str) -> BiasMetrics:
            ...
            return BiasMetrics(
                ...
                recommendations=(
                    ["Monitor closely"] if bias_score > 0.2 else ["Continue"]
                ),
                timestamp=datetime.utcnow(),
            )
...
def mock_performance_service():
    ...
    class MockPerformanceService:
        ...
        async def get_performance_summary(self, execution_id: str) -> Dict[str, Any]:
            execution_metrics = [
                m for m in self.metrics if m.execution_id == execution_id
            ]
            ...
            return {
                ...
                "average_response_time_ms": sum(m.value for m in execution_metrics)
                ...
                "threshold_violations": sum(
                    1 for m in execution_metrics if m.threshold_exceeded
                ),
                "sub_50ms_compliance_rate": sum(
                    1 for m in execution_metrics if m.value <= 50.0
                )
                / len(execution_metrics),
            }
...
def test_data_generator():
    ...
    class TestDataGenerator:
        ...
        def generate_stage_result(
            stage_name: str, status: str = "completed"
        ) -> Dict[str, Any]:
            return {
        ...
        def generate_performance_metrics(
            execution_id: str, num_metrics: int = 5
...
def performance_timer():
    ...
    class PerformanceTimer:
        ...
        def assert_under_threshold(self, threshold_ms: float = 50.0):
            ...
            ), f"Operation took {elapsed:.2f}ms, exceeding {threshold_ms}ms threshold"
