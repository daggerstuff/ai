...
class StateManager:
    ...
    def __init__(
        ...
        self.checkpoint_interval_seconds = self.config.get(
    ...
    async def initialize_pipeline_state(
    ...
    async def update_pipeline_state(
        self, execution_id: str, status: str, data: Optional[Dict[str, Any]] = None
    ) -> PipelineState:
        """
    ...
    async def get_stage_state(
        ...
        try:
            ...
            if state_dict["start_time"]:
                state_dict["start_time"] = datetime.fromisoformat(
        ...
        try:
            ...
            for key in keys:
                ...
                if state_data:
                    ...
                    if state_dict["end_time"]:
                        state_dict["end_time"] = datetime.fromisoformat(
                            state_dict["end_time"]
                        )
                    state_dict["last_updated"] = datetime.fromisoformat(
                        state_dict["last_updated"]
    ...
    async def create_checkpoint(
        self, execution_id: str, stage_name: str, checkpoint_data: Dict[str, Any]
    ) -> str:
        """
        ...
        try:
            ...
            await self.redis_client.setex(
                key, self.state_ttl_seconds, json.dumps(checkpoint_info, default=str)
            )
                ...
                if state_data:
                    ...
                    if state_dict.get("user_id") == user_id:
                        ...
                        summary = {
                            ...
                            "overall_quality_score": state_dict.get(
                                "overall_quality_score"
                            ),
                            "total_duration_seconds": state_dict.get(
    ...
    async def _store_pipeline_state(self, state: PipelineState) -> None:
        ...
        try:
            ...
            await self.redis_client.setex(
                key, self.state_ttl_seconds, json.dumps(state_dict, default=str)
            )
    ...
    async def _create_audit_entry(
        self, execution_id: str, event_type: str, event_data: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            await self.redis_client.setex(
                key, self.state_ttl_seconds, json.dumps(audit_trail, default=str)
            )
