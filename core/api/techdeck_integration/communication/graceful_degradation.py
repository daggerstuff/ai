...
class GracefulDegradationManager:
    ...
    async def _execute_fallback(
        ...
        try:
            ...
            raise ServiceUnavailableError(
                f"Fallback failed for service {service_name}: {str(e)}"
            )
        ...
        try:
            ...
            if health.circuit_state == CircuitState.OPEN:
                if (
                    ...
                    and datetime.utcnow() < health.next_retry_time
                ):
                    return False
                else:
    ...
    async def _record_service_failure(
        ...
        try:
            ...
            if health:
                ...
                if (
                    health.failure_count >= self.config.circuit_breaker_threshold
                    and health.circuit_state == CircuitState.CLOSED
                ):
                    # Open circuit breaker
                    health.circuit_state = CircuitState.OPEN
                    health.next_retry_time = datetime.utcnow() + timedelta(
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            # Check configuration
            config_healthy = (
                self.config.circuit_breaker_threshold > 0
                and self.config.circuit_breaker_timeout_seconds > 0
                and self.config.half_open_max_calls > 0
            )
