...
def show(ctx, profile: Optional[str], show_all: bool, decrypt: bool):
    ...
    try:
        if show_all:
            ...
            click.echo(
                f"  Validate certificates: {config.security.validate_certificates}"
            )
...
@click.option(
    "--set", "set_values", multiple=True, help="Set configuration value (key=value)"
)
@click.option(
    ...
    try:
        ...
        if interactive:
            ...
            for value_str in set_values:
                if "=" not in value_str:
                    click.echo(
                        f"❌ Invalid format: {value_str}. Use key=value", err=True
                    )
                    continue
    ...
    try:
        if not force:
            ...
            if not click.confirm(
                f"⚠️  Are you sure you want to reset{profile_msg} to defaults?"
            ):
...
def copy(ctx, from_profile: str, to_profile: str, overwrite: bool):
    ...
    try:
        ...
        try:
            ...
            if not overwrite:
                click.echo(
                    f"❌ Target profile '{to_profile}' already exists. Use --overwrite to replace.",
                    err=True,
                )
                return
...
@click.option(
    "--include-secrets", is_flag=True, help="Include sensitive configuration values"
)
@click.pass_context
def export(
    ctx, profile: Optional[str], output: str, format: str, include_secrets: bool
):
...
def import_config(ctx, file: str, profile: Optional[str], overwrite: bool):
    ...
    try:
        ...
        try:
            ...
            if not overwrite:
                click.echo(
                    f"❌ Profile '{target_profile}' already exists. Use --overwrite to replace.",
                    err=True,
                )
                return
        except FileNotFoundError:
...
def _interactive_config(config: CLIConfig) -> None:
    ...
    api_timeout = click.prompt(
        "API Timeout (seconds)", default=config.api.timeout, type=int
    )
    ...
    log_size = click.prompt(
        "Max Log Size (MB)", default=config.logging.max_size_mb, type=int
    )
    log_backups = click.prompt(
        "Backup Count", default=config.logging.backup_count, type=int
    )
...
def _apply_import_data(config: CLIConfig, data: Dict[str, Any]) -> None:
    ...
    if "security" in data:
        ...
        if "validate_certificates" in security_data:
            config.security.validate_certificates = security_data[
