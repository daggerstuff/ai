...
class SafetyValidationSystem:
    ...
    async def run_comprehensive_safety_validation(self) -> Dict[str, Any]:
        ...
        self.overall_safety_score = self._calculate_overall_safety_score(
            category_accuracies
        )
    ...
    def _generate_safety_recommendations(self) -> List[str]:
        ...
        if critical_failures:
            recommendations.append(
                f"Address {len(critical_failures)} critical safety test failures"
            )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(f"{'='*60}")
        print(
            f"Crisis Detection Accuracy: {report['safety_validation_summary']['crisis_detection_accuracy']}%"
        )
        print(
            f"Overall Safety Score: {report['safety_validation_summary']['overall_safety_score']}%"
        )
        print(
            f"Certification Status: {report['safety_validation_summary']['certification_status']}"
        )
        print(
            f"Production Ready: {'YES' if report['safety_validation_summary']['production_ready'] else 'NO'}"
        )
