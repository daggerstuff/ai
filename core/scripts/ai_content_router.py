...
class AIContentRouter:
    ...
    def analyze_content_structure(self, text: str, metadata: Dict) -> Dict:
        ...
        has_dialogue_markers = any(
            marker in text for marker in [":", "Q:", "A:", "HOST:", "GUEST:"]
        )
        has_question_response = "?" in text and len(text.split("?")) > 1

        # Analyze teaching/explanation patterns
        has_teaching_flow = any(
            word in text_lower
            for word in ["first", "second", "next", "then", "finally"]
        )
        has_explanation_language = any(
            phrase in text_lower
            for phrase in ["because", "the reason", "what happens", "this is why"]
        )

        # Analyze narrative/story elements
        has_narrative_elements = any(
            phrase in text_lower
            for phrase in ["i remember", "there was", "i had a client", "for example"]
        )
        has_personal_experience = any(
            phrase in text_lower
            for phrase in ["in my experience", "i've seen", "what i learned"]
        )

        # Analyze direct advice/guidance
        has_direct_advice = any(
            phrase in text_lower
            for phrase in ["you should", "try this", "i recommend", "what works"]
        )
        has_action_language = any(
            phrase in text_lower
            for phrase in ["do this", "start by", "practice", "the key is"]
        )
        ...
        if has_question_response:
            interview_score += 0.3
        if any(
            word in text_lower for word in ["asked", "question", "interviewer", "host"]
        ):
            interview_score += 0.3
        ...
        if any(
            phrase in text_lower
            for phrase in ["let me explain", "what i want you to understand"]
        ):
            teaching_score += 0.3
        ...
        if any(
            phrase in text_lower
            for phrase in ["definition", "means", "refers to", "is when"]
        ):
        ...
        if any(
            phrase in text_lower
            for phrase in ["strategy", "approach", "technique", "method"]
        ):
            advice_score += 0.3
        ...
        if format_scores:
            ...
            analysis["optimal_qa_strategy"] = self.determine_qa_strategy(
                primary_format, text, metadata
            )
            analysis["reasoning"] = self.generate_reasoning(
                primary_format, confidence, text
            )
    ...
    def generate_reasoning(self, format_type: str, confidence: float, text: str) -> str:
        ...
        return reasoning_map.get(
            format_type, f"General therapeutic content (confidence: {confidence:.2f})"
        )
    ...
    def process_segment(self, segment: Dict) -> Dict:
        ...
        question = self.generate_contextual_question(
            segment["text"], analysis["optimal_qa_strategy"], segment["style"]
        )
