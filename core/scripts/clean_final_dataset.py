...
class DatasetCleaner:
    ...
    def validate_conversation(self, conversation: Dict) -> bool:
        ...
        try:
            ...
            if "expert_id" not in conversation or conversation["expert_id"] not in [
                0,
                1,
                2,
                3,
    ...
    def update_config(self, config_file: Path, train_stats: Dict, val_stats: Dict):
        ...
        config["data_config"]["total_conversations"] = (
            train_stats["final_count"] + val_stats["final_count"]
        )

        # Add cleaning stats
        config["cleaning_stats"] = {
            "original_total": train_stats["original_count"]
            + val_stats["original_count"],
            "duplicates_removed": train_stats["duplicates_removed"]
            + val_stats["duplicates_removed"],
            "invalid_removed": train_stats["invalid_removed"]
            + val_stats["invalid_removed"],
...
def main():
    ...
    print(
        f"Original total: {train_stats['original_count'] + val_stats['original_count']:,}"
    )
    print(
        f"Duplicates removed: {train_stats['duplicates_removed'] + val_stats['duplicates_removed']:,}"
    )
    print(
        f"Invalid removed: {train_stats['invalid_removed'] + val_stats['invalid_removed']:,}"
    )
    print(f"Final total: {train_stats['final_count'] + val_stats['final_count']:,}")
    print(
        f"Retention rate: {round((train_stats['final_count'] + val_stats['final_count']) / (train_stats['original_count'] + val_stats['original_count']) * 100, 2)}%"
    )
