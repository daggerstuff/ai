    ...
    def backup_processed_conversations(self) -> bool:
        ...
        try:
            ...
            archive_manifest_path = (
                self.backup_dir / f"{self.backup_name}_archive_manifest.json"
            )
            ...
            logger.info(
                f"Size: {archive_path.stat().st_size / (1024*1024*1024):.2f} GB"
            )
    ...
    def verify_backup(self, archive_path: str) -> bool:
        ...
        try:
            ...
            if current_checksum != expected_checksum:
                logger.error(
                    f"Archive checksum mismatch! Expected: {expected_checksum}, Got: {current_checksum}"
                )
                return False
...
def main():
    ...
    logger.info(
        "This backup protects against data loss during infrastructure migration"
    )
