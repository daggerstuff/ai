    ...
    def analyze_current_coverage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if coverage_file.exists():
                ...
                self.current_coverage = self.coverage_data.get("totals", {}).get(
                    "percent_covered", 0.0
                )
    ...
    def identify_critical_gaps(self) -> List[Dict[str, Any]]:
        ...
        for module in critical_modules:
            ...
            if module_path.exists():
                ...
                coverage_info = self.coverage_data.get("files", {}).get(
                    str(module_path), {}
                )
                coverage_percent = coverage_info.get("summary", {}).get(
                    "percent_covered", 0.0
                )
                ...
                if "crisis" in module or "safety" in module:
                    ...
                    "pipeline_orchestrator" in module or "production_exporter" in module
                ):
                    target = 90.0
                else:
                    target = 85.0
                    ...
                    gaps.append(
                        {
                            ...
                            "priority": (
                                "CRITICAL"
                                if target >= 95
                                else "HIGH" if target >= 90 else "MEDIUM"
                            ),
                        }
                    )
...
'''
    ...
    def generate_missing_tests(self, gaps: List[Dict[str, Any]]) -> List[str]:
        ...
        for gap in gaps[:5]:  # Focus on top 5 critical gaps
            ...
            test_file_path = (
                self.base_path / "tests" / f"test_{module_name}_coverage.py"
            )
                ...
                for line in coverage_lines:
                    if "TOTAL" in line and "%" in line:
                        ...
                        if match:
                            ...
                            logger.info(
                                f"📈 Coverage improved: {initial_percent:.1f}% → {new_coverage:.1f}% (+{improvement:.1f}%)"
                            )
