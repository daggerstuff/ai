...
from ai.inference.api.techdeck_integration.utils.validation import sanitize_input, validate_pipeline_input
...
class StageCoordinator(EventHandler):
    ...
    def __init__(self, stage_name: str, event_bus: EventBus, bias_integration: BiasDetectionIntegration | None = None):
        super().__init__(
    ...
    async def execute_stage(self, context: PipelineContext, input_data: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        try:
            self.logger.info(f"Executing stage {self.stage_name} for execution {context.execution_id}")
            ...
            if context.enable_bias_detection and self.stage_config.get("bias_detection_enabled", False):
            ...
            self.logger.error(f"Error in stage {self.stage_name} for execution {context.execution_id}: {e}")
            await self._handle_stage_error(context, e)
            raise

    async def _execute_stage_logic(self, context: PipelineContext, input_data: dict[str, Any]) -> dict[str, Any]:
        """Execute stage-specific logic - to be implemented by subclasses."""
        raise NotImplementedError(f"Stage {self.stage_name} must implement _execute_stage_logic")
    ...
    async def _validate_stage_input(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            validation_result = validate_pipeline_input(sanitized_data, self.stage_name)

            if not validation_result["is_valid"]:
                raise ValidationError(f"Stage {self.stage_name} input validation failed: {validation_result['errors']}")
    ...
    async def _perform_bias_detection(self, stage_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            bias_result = await self.bias_integration.analyze_stage_data(stage_data, self.stage_name)

            # Check bias threshold
            if bias_result.get("bias_score", 0) > 0.7:  # Configurable threshold
                self.logger.warning(f"High bias detected in stage {self.stage_name}: {bias_result['bias_score']}")
    ...
    async def _publish_stage_completion(self, context: PipelineContext, result: dict[str, Any]) -> None:
        """Publish stage completion event."""
        event = await self.event_bus.create_stage_event(
            execution_id=context.execution_id,
            stage=self.stage_name,
    ...
    def _is_error_recoverable(self, error: Exception) -> bool:
        ...
        recoverable_errors = ["ValidationError", "TimeoutError", "ResourceNotFoundError"]
    ...
    def _log_stage_performance(self, context: PipelineContext, duration_ms: float) -> None:
        ...
        self.logger.info(
            f"Stage {self.stage_name} completed in {duration_ms:.2f}ms for execution {context.execution_id}"
        )

        # Update performance metrics
        if duration_ms > 50:
            self.logger.warning(f"Stage {self.stage_name} exceeded 50ms threshold")
...
class PipelineCoordinator:
    ...
    def _create_stage_coordinator(self, stage_name: str) -> StageCoordinator:
        ...
        if stage_name == "validation":
            return ValidationStageCoordinator(stage_name, self.event_bus, self.bias_integration)
        if stage_name == "processing":
            return ProcessingStageCoordinator(stage_name, self.event_bus, self.bias_integration)
        return GenericStageCoordinator(stage_name, self.event_bus, self.bias_integration)
    ...
    async def _execute_six_stages(self, context: PipelineContext) -> dict[str, Any]:
        ...
        for stage_name in stage_order:
            try:
                ...
                await self.state_manager.update_stage_state(context.execution_id, stage_name, "completed", stage_result)

                # Check quality threshold for validation stage
                if (
                    stage_name == "validation"
                    and stage_result.get("result", {}).get("validation_score", 0) < context.quality_threshold
                ):
                    self.logger.warning(
                        f"Quality threshold not met in validation stage for execution {context.execution_id}"
                    )
                ...
                self.logger.error(f"Stage {stage_name} failed for execution {context.execution_id}: {e}")

                # Update pipeline state with error
                await self.state_manager.update_stage_state(
                    context.execution_id, stage_name, "failed", {"error": str(e)}
                )

                # Attempt recovery if possible
                recovery_result = await self.error_recovery.attempt_stage_recovery(context, stage_name, e)
    ...
    async def _validate_execution_request(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if request["execution_mode"] not in valid_modes:
                raise ValidationError(f"Invalid execution_mode. Must be one of: {valid_modes}")
    ...
    def _get_stage_input(
        self, context: PipelineContext, stage_name: str, previous_results: dict[str, Any]
    ) -> dict[str, Any]:
        """Get input data for a specific stage."""
        if stage_name == "ingestion":
            # First stage gets dataset information
            return {"dataset_ids": context.dataset_ids, "execution_mode": context.execution_mode}
    ...
    def _get_previous_stage(self, current_stage: str) -> str | None:
        """Get the previous stage in the pipeline."""
        stage_order = ["ingestion", "validation", "processing", "standardization", "quality", "export"]
    ...
    async def _publish_quality_warning(self, context: PipelineContext, validation_result: dict[str, Any]) -> None:
        """Publish quality warning event."""
        event = EventMessage(
    ...
    async def _finalize_pipeline_execution(
        self, context: PipelineContext, stage_results: dict[str, Any]
    ) -> dict[str, Any]:
        """Finalize pipeline execution with comprehensive results."""
        try:
            # Calculate overall metrics
            total_duration = (datetime.now(UTC) - context.start_time).total_seconds()
            ...
            await self.state_manager.update_pipeline_state(context.execution_id, "completed", final_results)
    ...
    def _generate_recommendations(self, stage_results: dict[str, Any]) -> list[str]:
        ...
        if not any("bias" in rec.lower() for rec in recommendations):
            recommendations.append("Consider reviewing dataset diversity to minimize potential biases")
    ...
    def _log_pipeline_performance(self, context: PipelineContext, duration_ms: float) -> None:
        ...
        self.logger.info(
            f"Pipeline execution {context.execution_id} completed in {duration_ms:.2f}ms for user {context.user_id}"
        )

        # Check performance against targets
        if duration_ms > 5000:  # 5 second target for full pipeline
            self.logger.warning(f"Pipeline execution exceeded 5-second target: {duration_ms:.2f}ms")
    ...
    async def cancel_execution(self, execution_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            if current_status in ["completed", "failed", "cancelled"]:
                raise ValidationError(f"Execution cannot be cancelled in {current_status} status")
            ...
            await self.state_manager.update_pipeline_state(
                execution_id, "cancelled", {"cancellation_reason": "User requested", "cancelled_by": user_id}
            )
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "stage_coordinators": {
                    stage: coordinator.name for stage, coordinator in self.stage_coordinators.items()
                },
            }
...
class ValidationStageCoordinator(StageCoordinator):
    ...
    async def _execute_stage_logic(self, _context: PipelineContext, _input_data: dict[str, Any]) -> dict[str, Any]:
...
class GenericStageCoordinator(StageCoordinator):
    ...
    async def _execute_stage_logic(self, context: PipelineContext, _input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.stage_name == "standardization":
            return {
                "target_format": "standardized_json",
...
class ProgressEventHandler(EventHandler):
    ...
    def __init__(self, progress_tracker: ProgressTracker):
        super().__init__(
            ...
            event_types=[EventType.PROGRESS_UPDATE.value, EventType.STAGE_TRANSITION.value],
        )
        self.progress_tracker = progress_tracker

    async def _process_event(self, event: EventMessage) -> dict[str, Any] | None:
        """Process progress update events."""
        await self.progress_tracker.update_progress(event.execution_id, event.payload)
        return None
...
class ErrorRecoveryEventHandler(EventHandler):
    ...
    def __init__(self, error_recovery: ErrorRecoveryManager):
        super().__init__(name="error_recovery_handler", event_types=[EventType.ERROR_OCCURRED.value])
    ...
    async def _process_event(self, event: EventMessage) -> dict[str, Any] | None:
        ...
        self.logger.info(
            f"Processing error event for execution {event.execution_id}: {event.payload.get('error_type', 'unknown')}"
        )
        return None
...
class PerformanceEventHandler(EventHandler):
    ...
    def __init__(self, performance_monitor: PerformanceMonitor):
        super().__init__(
            ...
            event_types=[EventType.PERFORMANCE_METRIC.value, EventType.LATENCY_WARNING.value],
        )
    ...
    async def _process_event(self, event: EventMessage) -> dict[str, Any] | None:
        ...
        self.logger.info(f"Processing performance event for execution {event.execution_id}")
        return None
