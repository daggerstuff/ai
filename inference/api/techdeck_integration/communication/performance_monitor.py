...
from ai.inference.api.techdeck_integration.error_handling.custom_errors import PerformanceMonitoringError, ValidationError
...
class PerformanceMonitor:
    ...
    def _generate_threshold_recommendations(
        ...
        if current_value > threshold.critical_threshold:
            recommendations.append(
                f"Critical performance issue: {metric_name} exceeded {threshold.critical_threshold}{threshold.unit}"
            )
            recommendations.append("Immediate investigation required")
        elif current_value > threshold.warning_threshold:
            recommendations.append(
                f"Performance warning: {metric_name} exceeded {threshold.warning_threshold}{threshold.unit}"
            )
    ...
    def _generate_performance_alerts(self, dashboard: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if dashboard["overall_stats"]["total_metrics_recorded"] > 0:
                error_rate = (
                    dashboard["overall_stats"]["threshold_violations"]
                    / dashboard["overall_stats"]["total_metrics_recorded"]
                )
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            # Check configuration
            config_healthy = (
                self.metrics_retention_hours > 0
                and self.threshold_check_interval > 0
                and self.performance_window_size > 0
            )
