...
class CCDIntegration:
    ...
    def update_ccd_formulation_from_simulation(
        ...
        updated_conceptualization = CCDConceptualization(
            client_id=ccd_conceptualization.client_id, timestamp=datetime.now()
        )
        ...
        if ccd_conceptualization.formulation:
            ...
            note = f"Formulation updated based on simulation: trust={trust_level:.2f}, resistance={resistance_level:.2f}, quality={therapeutic_quality:.2f}"
    ...
    def _generate_legal_issues(personality_type: ClientPersonality) -> list:
        ...
        if base_issues and random.random() < 0.3:  # 30% chance of legal issues
    ...
    def _adjust_strengths(
        ...
        if trust_level > 0.6 and resistance_level < 0.4:
            if "Capacity for trust and vulnerability" not in adjusted:
                adjusted.append("Capacity for trust and vulnerability")

        if therapist_analysis.get("therapeutic_quality", 0) > 0.7:
            if "Responsiveness to skilled interventions" not in adjusted:
    ...
    def _adjust_vulnerabilities(
        base_vulnerabilities: list, trust_level: float, resistance_level: float, therapist_analysis: dict
    ) -> list:
    ...
    def _adjust_confidence(
        ...
        if trust_level > 0.7 and resistance_level < 0.4:
            adjusted += 0.1
        elif trust_level < 0.3 and resistance_level > 0.7:
            adjusted -= 0.1
