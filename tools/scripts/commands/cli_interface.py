...
@click.option(
    "--status",
    type=click.Choice(["running", "completed", "failed", "pending"]),
...
def process(_ctx, input: str, output: str | None, format: str, validate_only: bool):
    ...
    try:
        ...
        if result["success"]:
            ...
            click.echo(
                f"❌ Data processing failed: {result.get('error', 'Unknown error')}",
                err=True,
            )
...
@click.option(
    "--type",
    type=click.Choice(["models", "datasets", "pipelines", "results"]),
    default="models",
    help="Search type",
)
...
def batch(_ctx, input: str, output: str | None, parallel: int, continue_on_error: bool):
    ...
    try:
        ...
        click.echo("✅ Batch operations completed!")
...
@click.option(
    "--type",
    type=click.Choice(["all", "pipeline", "model", "data"]),
    default="all",
