...
class APITestClient:
    ...
    def load_test(
        self, endpoint: str, concurrent_users: int = 10, duration_seconds: int = 60
    ) -> Dict[str, Any]:
...
class APITestSuite:
    ...
    def run_health_checks(self) -> List[TestResult]:
        ...
        for endpoint, method, expected_status in tests:
            result = self.test_client.test_endpoint(
                endpoint, method, expected_status=expected_status
            )
            results.append(result)
    ...
    def run_authentication_tests(self) -> List[TestResult]:
        ...
        result = self.test_client.test_endpoint(
            "/conversation", "POST", expected_status=401
        )
        results.append(result)

        # Test with invalid token
        invalid_client = APITestClient(self.base_url, jwt_token="invalid_token")
        result = invalid_client.test_endpoint(
            "/conversation", "POST", expected_status=401
        )
        results.append(result)
    ...
    def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        test_results["tests"]["health_checks"] = {
            ...
            "success_rate": sum(1 for r in health_results if r.success)
            / len(health_results),
        }
        ...
        test_results["tests"]["authentication"] = {
            ...
            "success_rate": sum(1 for r in auth_results if r.success)
            / len(auth_results),
        }
        ...
        test_results["tests"]["rate_limiting"] = {
            ...
            "rate_limit_triggered": any(
                r.status_code == 429 for r in rate_limit_results
            ),
        }
...
def main():
    ...
    parser.add_argument(
        "--test-type",
        choices=["health", "auth", "rate-limit", "load", "comprehensive"],
    ...
    parser.add_argument(
        "--duration", type=int, default=60, help="Duration for load testing (seconds)"
    )
    ...
    if args.api_key or args.jwt_token:
        test_suite.test_client = APITestClient(
            args.base_url, args.api_key, args.jwt_token
        )
    ...
    if args.test_type == "health":
        ...
        if not args.endpoint:
            ...
            return
        results = test_suite.test_client.load_test(
            args.endpoint, args.concurrent_users, args.duration
        )
