...
```python
from ai.pipelines.design import NeMoDataDesignerService, DataDesignerConfig
...
```python
from ai.pipelines.design import NeMoDataDesignerService
# Note: BiasDetectionService resides in the main Pixelated Empathy app (src/lib/ai/bias-detection/python-service/)
# This import works when running from the monorepo root where 'src' is in the Python path.
...
See `ai/pipelines/design/examples.py` for complete working examples:
