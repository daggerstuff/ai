...
class CheckpointManager:
    ...
    def get_available_checkpoints(self) -> List[CheckpointInfo]:
        ...
        for checkpoint_info in self.history:
            ...
            if checkpoint_path.exists():
                ...
                self.logger.warning(
                    f"Checkpoint path does not exist: {checkpoint_info.path}"
                )
    ...
    def create_recovery_point(self, checkpoint_path: str) -> str:
        ...
        if checkpoint_path_obj.exists():
            shutil.copytree(checkpoint_path_obj, recovery_path)
    ...
    def validate_checkpoint(self, checkpoint_path: str) -> Tuple[bool, str]:
        ...
        for required_file in required_files:
            if not (checkpoint_path_obj / required_file).exists():
        ...
        try:
            ...
            if training_args_path.exists():
                # We can't easily load TrainingArguments from binary
                # without the original args
...
class RollbackManager:
    ...
    def __init__(self, checkpoint_manager: CheckpointManager):
        ...
        self.rollback_log_file = (
            checkpoint_manager.checkpoints_dir / "rollback_log.json"
        )
...
def test_checkpoint_rollback_system():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(
            f"Rollback to best result: {rollback_result.success}, "
        ...
        print(
            f"Rollback to step 200 result: {rollback_result.success}, "
            f"{rollback_result.message}"
        )
        ...
        for chkpt_path in checkpoint_paths[:3]:  # Only test first 3
            ...
            print(
                f"Checkpoint {os.path.basename(chkpt_path)} validation: "
                f"{is_valid}, {msg}"
            )
