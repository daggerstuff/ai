...
class LabelDistributionAnalyzer:
    ...
    def generate_distribution_report(
        self, label_bundles: List[LabelBundle]
        ...
        for bundle in label_bundles:
            ...
            for label in bundle.therapeutic_response_labels:
                ...
                confidence_scores["therapeutic_response"].append(
            ...
            if bundle.crisis_label:
                ...
                confidence_scores["crisis"].append(
            ...
            if bundle.therapy_modality_label:
                ...
                confidence_scores["therapy_modality"].append(
                    bundle.therapy_modality_label.metadata.confidence
                )
                provenance_counts[
                    bundle.therapy_modality_label.metadata.provenance.value
                ] += 1
            ...
            if bundle.mental_health_condition_label:
                ...
                confidence_scores["mental_health_condition"].append(
            ...
            if bundle.demographic_label:
                ...
                confidence_scores["demographic"].append(
    ...
    def find_rare_categories(
        self, label_bundles: List[LabelBundle], threshold_ratio: float = 0.01
        ...
        for (
            response_type,
            count,
        ) in distribution_report.therapeutic_response_distribution.items():
        ...
        for (
            crisis_level,
            count,
        ) in distribution_report.crisis_level_distribution.items():
            if total > 0 and count / total < threshold_ratio:
                rare_crisis.append(
                    {
        ...
        for (
            modality,
            count,
        ) in distribution_report.therapy_modality_distribution.items():
            if total > 0 and count / total < threshold_ratio:
                rare_modalities.append(
                    {"category": modality, "count": count, "proportion": count / total}
                )
        if rare_modalities:
        ...
        for (
            condition,
            count,
        ) in distribution_report.mental_health_condition_distribution.items():
            if total > 0 and count / total < threshold_ratio:
                rare_conditions.append(
                    {"category": condition, "count": count, "proportion": count / total}
                )
        if rare_conditions:
    ...
    def analyze_label_correlations(
        ...
        for bundle in label_bundles:
            ...
            for label in bundle.therapeutic_response_labels:
                ...
                correlations["therapeutic_response_crisis"][crisis_level][
                    response_type
                ] += 1
...
class EdgeCaseDetector:
    ...
    def detect_edge_cases(
        self, conversations: List[Conversation], label_bundles: List[LabelBundle]
    ) -> EdgeCaseReport:
    ...
    def _find_low_confidence_items(
        self, paired_data: List[Tuple[Conversation, LabelBundle]]
    ) -> List[Dict[str, Any]]:
        ...
        for conv, bundle in paired_data:
            ...
            if (
                bundle.crisis_label
                and bundle.crisis_label.metadata.confidence
            ...
            if (
                ...
                and bundle.therapy_modality_label.metadata.confidence
                < self.low_confidence_threshold
            ):
                issues.append(
                    {
            ...
            if (
                ...
                and bundle.mental_health_condition_label.metadata.confidence
                < self.low_confidence_threshold
            ):
                issues.append(
                    {
            ...
            if (
                ...
                and bundle.demographic_label.metadata.confidence
                < self.low_confidence_threshold
            ):
                issues.append(
                    {
            ...
            if issues:
                low_confidence.append(
                    {"conversation_id": conv.conversation_id, "issues": issues}
                )

        return low_confidence

    def _find_conflicting_labels(
        self, paired_data: List[Tuple[Conversation, LabelBundle]]
    ) -> List[Dict[str, Any]]:
        ...
        for conv, bundle in paired_data:
            ...
            response_types = [
                label.response_type for label in bundle.therapeutic_response_labels
            ]

            # Specific conflict detection - example: challenge and support responses might conflict
            if (
                TherapeuticResponseType.CHALLENGE in response_types
                and TherapeuticResponseType.SUPPORT in response_types
            ):
                # Check if they have low confidence, which might indicate uncertainty
                challenge_labels = [
                    l
                    for l in bundle.therapeutic_response_labels
                ...
                if any(l.metadata.confidence < 0.7 for l in challenge_labels) or any(
                    l.metadata.confidence < 0.7 for l in support_labels
                ):
                    conflicts.append(
                        {
                            ...
                            "challenges": [
                                l.response_type.value + f"({l.metadata.confidence:.2f})"
    ...
    def _find_rare_categories_in_bundles(
        self, paired_data: List[Tuple[Conversation, LabelBundle]]
    ) -> List[Dict[str, Any]]:
        ...
        for conv, bundle in paired_data:
            ...
            for label in bundle.therapeutic_response_labels:
                ...
                if (
                    label.response_type.value == "interpretation"
                    and len(
                        [
                            l
                            for l in bundle.therapeutic_response_labels
                            if l.response_type.value == "interpretation"
                        ]
                    )
                    == 1
                ):
    ...
    def _find_high_risk_items(
        ...
        for conv, bundle in paired_data:
            ...
            # High crisis levels
            if bundle.crisis_label:
                if bundle.crisis_label.crisis_level in [
                    CrisisLevelType.HIGH_RISK,
                    CrisisLevelType.IMMEDIATE_RISK,
                ]:
                    risk_factors.append(
                ...
                if (
                    bundle.crisis_label.estimated_risk_probability is not None
                    and bundle.crisis_label.estimated_risk_probability > 0.8
                ):
                    risk_factors.append(
                        {
                ...
                risk_factors.append(
                    {
                        ...
                        "lowest_confidence": min(
                            l.metadata.confidence for l in challenge_labels
                        ),
                    }
                )
    ...
    def _find_inconsistent_patterns(
        self, paired_data: List[Tuple[Conversation, LabelBundle]]
    ) -> List[Dict[str, Any]]:
        ...
        for conv, bundle in paired_data:
            ...
            if (
                bundle.crisis_label
                and bundle.crisis_label.crisis_level
                in [CrisisLevelType.HIGH_RISK, CrisisLevelType.IMMEDIATE_RISK]
                and bundle.therapeutic_response_labels
                and all(
                    l.metadata.confidence < 0.5
                    for l in bundle.therapeutic_response_labels
                )
            ):
                inconsistencies_found.append(
                    {
                        ...
                        "crisis_level": bundle.crisis_label.crisis_level.value,
                        "response_labels_count": len(
                            bundle.therapeutic_response_labels
                        ),
                        "lowest_response_confidence": min(
                            l.metadata.confidence
                            for l in bundle.therapeutic_response_labels
                        ),
                    }
                )
...
class DistributionVisualizer:
    ...
    def print_distribution_summary(self, report: LabelDistributionReport):
        ...
        for response_type, count in sorted(
            report.therapeutic_response_distribution.items(),
            key=lambda x: x[1],
            reverse=True,
        ):
            percentage = (
                (count / report.total_conversations * 100)
        ...
        for crisis_level, count in sorted(
            report.crisis_level_distribution.items(), key=lambda x: x[1], reverse=True
        ):
            percentage = (
                (count / report.total_conversations * 100)
                if report.total_conversations > 0
        ...
        for modality, count in sorted(
            report.therapy_modality_distribution.items(),
            key=lambda x: x[1],
            reverse=True,
        ):
            percentage = (
        ...
        for provenance, count in sorted(
            report.provenance_distribution.items(), key=lambda x: x[1], reverse=True
        ):
            percentage = (
                (count / report.total_conversations * 100)
                if report.total_conversations > 0
        ...
        for label_type, stats in report.confidence_statistics.items():
            if stats["count"] > 0:
                print(
                    f"  {label_type}: Mean={stats['mean']:.3f}, StdDev={stats['std_dev']:.3f}, Min={stats['min']:.3f}, Max={stats['max']:.3f}"
                )
    ...
    def print_edge_case_summary(self, report: EdgeCaseReport):
        ...
        print(
            f"Total conversations with low confidence labels: {len(report.low_confidence_items)}"
        )
        for item in report.low_confidence_items[:5]:  # Show first 5
            print(
                f"  Conversation {item['conversation_id'][:8]}...: {len(item['issues'])} low-confidence issues"
            )

        print(f"\n--- Conflicting Labels ---")
        print(
            f"Total conversations with conflicting labels: {len(report.conflicting_labels)}"
        )
        for item in report.conflicting_labels[:5]:  # Show first 5
            print(
                f"  Conversation {item['conversation_id'][:8]}...: {item['conflict_type']}"
            )
        ...
        for item in report.high_risk_items[:5]:  # Show first 5
            print(
                f"  Conversation {item['conversation_id'][:8]}...: {len(item['risk_factors'])} risk factors"
            )

        print(f"\n--- Inconsistent Patterns ---")
        print(
            f"Total conversations with inconsistencies: {len(report.inconsistent_patterns)}"
        )
        for item in report.inconsistent_patterns[:5]:  # Show first 5
            print(
                f"  Conversation {item['conversation_id'][:8]}...: {len(item['inconsistencies'])} inconsistencies"
            )
...
def test_label_analysis():
    ...
    for i in range(100):
        ...
        response_type = (
            ...
            if i % 3 == 0
            else TherapeuticResponseType.REFLECTION
            if i % 3 == 1
            else TherapeuticResponseType.PROBING
        )
    ...
    print(
        f"Therapeutic response distribution: {dist_report.therapeutic_response_distribution}"
    )
