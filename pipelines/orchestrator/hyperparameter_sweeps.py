...
class HyperparameterSweeper:
                ...
                if param.type == "categorical":
                    ...
                    if param.distribution == "uniform":
                        config[param.name] = random.uniform(
                            param.min_value or 0, param.max_value or 1
                        )
                    elif param.distribution == "log_uniform":
                        config[param.name] = np.exp(
                            random.uniform(
                                np.log(param.min_value or 1e-6),
                    ...
                    if param.distribution == "uniform":
                        config[param.name] = random.randint(
                            int(param.min_value or 0), int(param.max_value or 1)
                        )
                    else:
    ...
    def sweep_hyperparameters(
        ...
        evaluation_function: Optional[Callable[[Any], Dict[str, float]]] = None,
    ) -> SweepResult:
        ...
        for i, config in enumerate(configs):
            ...
            try:
                ...
                is_better = (
                    sweep_config.goal == "minimize" and metric_value < best_metric
                ) or (sweep_config.goal == "maximize" and metric_value > best_metric)
                ...
                self.logger.info(
                    f"Trial {trial_id} completed with {sweep_config.metric}={metric_value:.4f}"
                )
                ...
                trial_result = TrialResult(
                    ...
                    metadata={
                        "error": str(e),
                        "sweep_config_name": sweep_config.sweep_name,
                    },
                )
                trial_results.append(trial_result)
        ...
        self.logger.info(
            f"Sweep completed. Best {sweep_config.metric}: {best_metric:.4f}"
        )
        return sweep_result

    def _update_manifest_with_config(
        self, base_manifest: TrainingManifest, config: Dict[str, Any]
    ) -> TrainingManifest:
        ...
        updated_manifest = TrainingManifest(
            ...
            hyperparameters=Hyperparameters(
                **{**base_manifest.hyperparameters.__dict__, **config}
            ),
...
class WandBIntegration:
    ...
    def initialize_sweep(self, sweep_config: SweepConfiguration) -> str:
        ...
        try:
            ...
            wandb_config = {
                ...
                "metric": {"name": sweep_config.metric, "goal": sweep_config.goal},
                "parameters": {},
            }
    ...
    def run_sweep_trial(
        ...
        try:
            wandb.agent(
                sweep_config.sweep_name, wandb_train, count=sweep_config.total_trials
            )
        except Exception as e:
...
class HyperparameterOptimizationSystem:
    ...
    def _run_wandb_sweep(
        ...
        if not sweep_id:
            self.logger.warning(
                "Failed to initialize WandB sweep, falling back to local sweep"
            )
            return self._run_local_sweep(base_manifest, sweep_config, training_function)
        ...
        if trial_results:
            ...
            sweep_result = SweepResult(
                ...
                completed_trials=len(
                    [r for r in trial_results if r.status == "completed"]
                ),
                failed_trials=len([r for r in trial_results if r.status == "failed"]),
                start_time=datetime.utcnow().isoformat(),
                end_time=datetime.utcnow().isoformat(),
    ...
    def _run_local_sweep(
        ...
        return self.sweeper.sweep_hyperparameters(
            base_manifest, sweep_config, training_function, default_evaluation
        )
    ...
    def generate_sweep_report(self, sweep_result: SweepResult) -> str:
        ...
        report.append(
            f"Best {sweep_result.sweep_config.metric}: {sweep_result.best_metric_value:.6f}"
        )
        report.append("")
        ...
        for i, trial in enumerate(sorted_trials, 1):
            ...
            report.append(
                f"  {i}. {sweep_result.sweep_config.metric}: {metric_val:.6f}"
            )
    ...
    def create_sweep_from_manifest(
        self, base_manifest: TrainingManifest
    ) -> SweepConfiguration:
        ...
        return SweepConfiguration(
            ...
            parameters=[
                ...
                HyperparameterConfig(
                    name="num_train_epochs", values=[1, 2, 3], type="categorical"
                ),
                HyperparameterConfig(
                    name="warmup_steps", values=[100, 500, 1000], type="categorical"
                ),
            ],
            total_trials=10,
        )
...
def test_hyperparameter_system():
    ...
    sweep_config = SweepConfiguration(
        ...
        parameters=[
            HyperparameterConfig(
                name="learning_rate", values=[1e-5, 2e-5, 5e-5], type="categorical"
            ),
            HyperparameterConfig(
                name="per_device_train_batch_size", values=[1, 2], type="categorical"
            ),
        ],
    ...
    sweep_result = hp_system.sweeper.sweep_hyperparameters(
        manifest, sweep_config, dummy_training_fn, dummy_evaluation_fn
    )
    ...
    artifact_path = hp_system.artifact_manager.save_artifact(
        {"test": "artifact", "trial": "test_001"}, "config.json", "test_001"
    )
