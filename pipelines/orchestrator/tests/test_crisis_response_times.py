...
class ResponseTimeValidator:
    ...
    def _timing_callback(
        self, detection: CrisisDetection, escalation: EscalationRecord
    ):
    ...
    def test_emergency_response_times(self):
        ...
        for name, content in emergency_cases:
            ...
            detection = self.detector.detect_crisis(
                {"id": f"emergency_test_{name.replace(' ', '_')}", "content": content}
            )
            ...
            escalation_time = (
                self.escalation_logs[0]["response_time_minutes"]
    ...
    def test_critical_response_times(self):
        ...
        critical_cases = [
            (
                "Self Harm Planning",
                "I need to cut myself deeper, I have the tools ready",
            ),
        ...
        for name, content in critical_cases:
            ...
            detection = self.detector.detect_crisis(
                {"id": f"critical_test_{name.replace(' ', '_')}", "content": content}
            )
    ...
    def test_elevated_response_times(self):
        ...
        for name, content in elevated_cases:
            ...
            detection = self.detector.detect_crisis(
                {"id": f"elevated_test_{name.replace(' ', '_')}", "content": content}
            )
    ...
    def test_concurrent_response_times(self):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(process_crisis, case) for case in concurrent_cases
            ]
    ...
    def test_response_time_consistency(self):
        ...
        for i in range(runs):
            ...
            self.detector.detect_crisis(
                {"id": f"consistency_test_{i}", "content": test_case}
            )
            processing_time = (time.time() - start_time) * 1000
...
def main():
    ...
    overall_success = (
        ...
        and critical_pass
        and elevated_pass
        and concurrent_results["meets_concurrent_requirement"]
        and consistency_results["consistency_good"]
    )
