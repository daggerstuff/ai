...
def test_automated_labeler_reproducibility():
        ...
        all_results.append(
            {
                ...
                "crisis_level": labels.crisis_label.crisis_level.value
                if labels.crisis_label
                else None,
                "crisis_confidence": labels.crisis_label.metadata.confidence
                if labels.crisis_label
                else None,
                "therapeutic_types": [
                    l.response_type.value for l in labels.therapeutic_response_labels
                ],
                "therapeutic_confidences": [
                    l.metadata.confidence for l in labels.therapeutic_response_labels
                ],
            }
        )
    ...
    for result in all_results[1:]:
        ...
        assert result["crisis_confidence"] == pytest.approx(
            first_result["crisis_confidence"], abs=1e-3
        )
        assert result["therapeutic_types"] == first_result["therapeutic_types"]
        assert result["therapeutic_confidences"] == pytest.approx(
            first_result["therapeutic_confidences"], abs=1e-3
        )
...
def test_different_seeds_produce_different_results():
    ...
    config = AugmentationConfig(paraphrase_probability=0.9, contextual_probability=0.9)
...
def test_pattern_detection_reproducibility():
    ...
    conversation = Conversation()
    conversation.add_message(
        "therapist", "So you're saying that work has been incredibly stressful?"
    )
    conversation.add_message("client", "Yes, the deadlines are overwhelming.")
    conversation.add_message(
        "therapist", "It sounds like you're carrying a heavy burden."
    )
...
def test_augmentation_preserves_critical_content():
    ...
    conversation.add_message(
        "therapist", "Thank you for sharing that. That takes courage."
    )
    conversation.add_message(
        "therapist", "Are you having thoughts of harming yourself right now?"
    )

    config = AugmentationConfig(paraphrase_probability=0.8, noise_probability=0.6)
...
def test_low_confidence_reproducibility():
    ...
    for i in range(3):
        ...
        if labels.therapeutic_response_labels:
            avg_confidence = sum(
                l.metadata.confidence for l in labels.therapeutic_response_labels
            ) / len(labels.therapeutic_response_labels)
