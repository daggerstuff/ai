...
from .resource_accounting import BudgetLimits, create_resource_manager
...
class SmokeTestDatasetGenerator:
    ...
    @staticmethod
    def generate_small_therapy_dataset(
        size: int = 10,
...
class SmokeTrainingTest:
    ...
    def run_resource_monitoring_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            from .resource_accounting import BudgetLimits, ResourceManager
            ...
            rm.start_run_monitoring(
                run_id, budget_limits, interval=1.0
...
class IntegrationTestSuite:
    ...
    def test_full_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dataset_data = SmokeTestDatasetGenerator.generate_small_therapy_dataset(
                size=5
            )
    ...
    def test_hyperparameter_sweep(self) -> Dict[str, Any]:
        ...
        try:
            ...
            from .hyperparameter_sweeps import HyperparameterConfig, SweepConfiguration
            ...
            sweep_config = SweepConfiguration(
                ...
                parameters=[
                    HyperparameterConfig(
                        name="learning_rate", values=[1e-5, 2e-5], type="categorical"
                    )
                ],
                total_trials=2,
            )
            ...
            def dummy_training_fn(config):
                ...
                if "learning_rate" in config:
                    updated_manifest.hyperparameters.learning_rate = config[
                        "learning_rate"
                    ]
            ...
            result = hp_system.sweeper.sweep_hyperparameters(
                base_manifest, sweep_config, dummy_training_fn, dummy_evaluation_fn
            )
...
def generate_ci_report(
    results: Dict[str, Any], output_path: str = "./ci_test_report.json"
):
    ...
    with open(summary_path, "w") as f:
        ...
        f.write(
            f"Success Rate: {(results['summary']['passed_tests'] / results['summary']['total_tests']) * 100:.1f}%\n\n"
        )
        ...
        for test_name, test_result in results["tests"].items():
            ...
            f.write(
                f"  {test_name}: {status} ({test_result['duration_seconds']:.2f}s)\n"
            )
...
def run_ci_tests():
    ...
    unit_tests_passed = (
        unit_test_result.testsRun
        - len(unit_test_result.errors)
    ...
    overall_success = (
        total_tests == 0
        or total_passed == total_tests  # All smoke tests passed
