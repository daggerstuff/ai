...
class LabelQualityMetrics:
    ...
    def calculate_label_accuracy(
        ...
        threshold = self._get_threshold(QualityMetric.LABEL_ACCURACY)
        is_passing = (
            threshold.minimum_value <= accuracy_score <= threshold.maximum_value
        )
        ...
        return QualityScore(
            ...
            details={
                ...
                "confidence_range": (
                    min(all_confidences) if all_confidences else 0,
                    max(all_confidences) if all_confidences else 1,
                ),
                "std_deviation": statistics.stdev(all_confidences)
                if len(all_confidences) > 1
                else 0,
            },
        )

    def calculate_label_completeness(
        ...
        for bundle in label_bundles:
            ...
            if bundle.therapeutic_response_labels or bundle.crisis_label:
                labeled_messages += (
                    1  # Simplified - in practice, map to actual messages
                )

        completeness_ratio = (
            labeled_messages / len(label_bundles) if label_bundles else 0
        )

        threshold = self._get_threshold(QualityMetric.LABEL_COMPLETENESS)
        is_passing = (
            threshold.minimum_value <= completeness_ratio <= threshold.maximum_value
        )
    ...
    def calculate_confidence_score(
        ...
        is_passing = (
            threshold.minimum_value <= avg_confidence <= threshold.maximum_value
        )
    ...
    def calculate_crisis_detection_accuracy(
        self, label_bundles: List[LabelBundle]
    ) -> QualityScore:
        ...
        if not crisis_labels:
            ...
            accuracy_score = (
                statistics.mean(crisis_confidences) if crisis_confidences else 1.0
            )
    ...
    def calculate_therapeutic_response_accuracy(
        self, label_bundles: List[LabelBundle]
    ) -> QualityScore:
    ...
    def get_comprehensive_quality_report(
        ...
        completeness_score = self.calculate_label_completeness(
            conversations, label_bundles
        )
...
def test_label_quality_metrics():
        ...
        bundle.therapeutic_response_labels.append(
            TherapeuticResponseLabel(
                ...
                metadata=LabelMetadata(
                    confidence=0.85 + (i * 0.01) if i < 10 else 0.75 - (i * 0.01)
                ),
            )
        )
    ...
    print(
        f"Label Accuracy: {accuracy_score.score:.3f}, Passing: {accuracy_score.is_passing}"
    )

    completeness_score = quality_metrics.calculate_label_completeness(
        conversations, label_bundles
    )
    print(
        f"Label Completeness: {completeness_score.score:.3f}, Passing: {completeness_score.is_passing}"
    )

    confidence_score = quality_metrics.calculate_confidence_score(label_bundles)
    print(
        f"Confidence Score: {confidence_score.score:.3f}, Passing: {confidence_score.is_passing}"
    )

    crisis_score = quality_metrics.calculate_crisis_detection_accuracy(label_bundles)
    print(
        f"Crisis Detection Accuracy: {crisis_score.score:.3f}, Passing: {crisis_score.is_passing}"
    )

    # Generate comprehensive report
    comprehensive_report = quality_metrics.get_comprehensive_quality_report(
        conversations, label_bundles
    )
    print(f"\nComprehensive Quality Report:")
    print(
        f"  Overall Compliance Rate: {comprehensive_report['overall_compliance_rate']:.3f}"
    )
    print(
        f"  Passing Metrics: {comprehensive_report['passing_metrics']}/{comprehensive_report['total_metrics']}"
    )
