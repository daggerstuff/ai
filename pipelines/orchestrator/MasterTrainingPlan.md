...
| **Stage 2 – Therapeutic Expertise & Reasoning** (`stage2_therapeutic_expertise`) | 25% | Teach structured reasoning, diagnosis scaffolding, knowledge grounding, MEMO summarization style. | CoT corpora in `ai/pipelines/orchestrator/cot_datasets/`, reasoning JSON under `ai/training_data_consolidated/psychology_knowledge`, DPO preference samples, MEMO (when licensed), professional PDF/book extracts. | Must have reasoning metadata (`chain_of_thought`, `summary`, `technique`). Empathy floor 0.5 but allow sterile clinical tone. Dedup keeps highest-quality reasoning path. |
| **Stage 3 – Edge Stress Test & Scenario Bank** (`stage3_edge_stress_test`) | 20% | Force model to survive nightmare fuel: crisis, trauma, Reddit rawness, edge generator prompts. | `ai/pipelines/edge_case/output/edge_cases_training_format.jsonl`, scenario prompt corpus (Google Drive exports mirrored to `ai/pipelines/orchestrator/prompt_corpus/`), Reddit/Kaggle TF-IDF dumps, Synthetic edge expansions, Safety DPO violations for contrastive pairs. | **Do NOT** sanitize horror details. Only drop malformed JSON or PII (phone, SSN). Flag metadata `{"crisis_intensity": "very_high"}` for gating. Allow safety validator to mark `requires_human_review` without removal. |
...
Use `ConversationRecord` from `ai/pipelines/orchestrator/schemas/conversation_schema.py`:
```json
{
...
- Use `ai/pipelines/orchestrator/ingestion_deduplication.py` (or `processing/deduplication_*`) to generate these hashes and resolve collisions.
...
5. **Monitoring hooks**: reuse `ai/pipelines/orchestrator/quality_control.py` & `evaluation_gates.py` to emit:
   ```json
   {
     "stage": "stage3_edge_stress_test",
     "samples": 42000,
...
uv run python ai/pipelines/orchestrator/scripts/pre_ingest_checks.py  # validates mounts, MEMO access
uv run python ai/pipelines/orchestrator/processing/unified_preprocessing_pipeline.py --stage all
uv run python ai/pipelines/orchestrator/orchestration/integrated_training_pipeline.py --staged --report reports/stage_report.json
```
...
- `ai/pipelines/orchestrator/ingestion_deduplication.py`
- `ai/pipelines/orchestrator/quality/*`
