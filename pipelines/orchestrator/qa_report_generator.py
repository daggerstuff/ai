...
class QualityMetrics:
    ...
    def calculate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "crisis_flags_percentage": (self.crisis_flags / self.total_samples * 100)
            ...
            "pii_detected_percentage": (self.pii_detected / self.total_samples * 100)
            if self.total_samples > 0
            else 0.0,
        ...
        if self.semantic_coherence_scores:
            summary["semantic_coherence"] = {
                ...
                "std": statistics.stdev(self.semantic_coherence_scores)
                if len(self.semantic_coherence_scores) > 1
                else 0.0,
            }
        else:
        ...
        if self.therapeutic_appropriateness_scores:
            summary["therapeutic_appropriateness"] = {
                ...
                "std": statistics.stdev(self.therapeutic_appropriateness_scores)
                if len(self.therapeutic_appropriateness_scores) > 1
                else 0.0,
            }
        else:
        ...
        if self.bias_scores:
            summary["bias"] = {
                ...
                "std": statistics.stdev(self.bias_scores)
                if len(self.bias_scores) > 1
                else 0.0,
            }
        else:
            summary["bias"] = None
...
class QAReport:
    ...
    def print_summary(self) -> None:
        ...
        if summary["semantic_coherence"]:
            print(
                f"  Semantic Coherence: {summary['semantic_coherence']['mean']:.3f} "
        ...
        if summary["therapeutic_appropriateness"]:
            print(
                f"  Therapeutic Appropriateness: {summary['therapeutic_appropriateness']['mean']:.3f}"
            )
        ...
        print("\n🛡️  Safety Metrics:")
        print(
            f"  Crisis Flags: {summary['crisis_flags_count']} ({summary['crisis_flags_percentage']:.2f}%)"
        )
        ...
        print("\n🔒 Privacy Metrics:")
        print(
            f"  PII Detected: {summary['pii_detected_count']} ({summary['pii_detected_percentage']:.2f}%)"
        )
...
def generate_qa_report(
    ...
    print(
        "   (Note: Full quality validation requires integration with quality validators)"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--detailed", action="store_true", help="Enable detailed analysis"
    )
    ...
    try:
        report = generate_qa_report(
            args.dataset_path, args.version, enable_detailed_analysis=args.detailed
        )
