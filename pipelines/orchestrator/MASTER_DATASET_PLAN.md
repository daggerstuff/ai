...
- **6-stage pipeline**: Orchestration via `ai/pipelines/orchestrator/orchestration/pipeline_orchestrator.py`

### Harmonization Process

**Stage 1: Normalization**
- Use `ai/pipelines/orchestrator/schemas/conversation_schema.py` for unified format
...
- Leverage `ai/pipelines/orchestrator/orchestration/tier_processor.py` for cluster handling
- Process datasets by tier (1-6) with appropriate quality thresholds
- Apply tier-weighted sampling via `ai/pipelines/orchestrator/composition/tier_balancer.py`

**Stage 3: Deduplication**
- Use Bloom-filter deduplication from `ai/pipelines/orchestrator/ingestion_deduplication.py`
...
Modify `ai/pipelines/orchestrator/quality/` validators to:
```python
...
**Stage 1: Foundation**
- Empathy score: ≥0.7 (via `ai/pipelines/orchestrator/quality/empathy_mental_health_validator.py`)
...
- **Empathy Scoring**: `ai/pipelines/orchestrator/quality/empathy_mental_health_validator.py`
  - EMNLP 2020 empathy dimensions (ER, IP, EX, VAL)
  - Overall empathy score: 0-1 scale
  
- **Safety Validation**: `ai/pipelines/orchestrator/quality/safety_alignment_validator.py`
  - Crisis content detection
  - Toxic language detection
  - PII exposure prevention
  - **BYPASSED for edge cases**

- **Coherence Validation**: `ai/pipelines/orchestrator/quality/coherence_validator.py`
...
1. **Save this master plan** to `ai/pipelines/orchestrator/MASTER_DATASET_PLAN.md` ✅
...
2. **Dataset Pipeline is Operational**
   - Use existing `ai/pipelines/orchestrator/` infrastructure
...
1. ✅ **Master Plan Document**: This file (`ai/pipelines/orchestrator/MASTER_DATASET_PLAN.md`)

2. **Dataset Inventory JSON**: `ai/pipelines/orchestrator/scripts/dataset_inventory.json`
   - Complete mapping of all datasets
   - Download commands for each source
   - Destination paths and checksums

3. **Execution Scripts**:
   - `ai/pipelines/orchestrator/scripts/vps_dataset_acquisition.sh` - VPS download automation
   - `ai/pipelines/orchestrator/scripts/transfer_to_vps.sh` - Local-to-VPS transfer
   - `ai/pipelines/orchestrator/scripts/upload_to_ovh_s3.sh` - VPS-to-S3 upload

4. **Inventory Files**:
   - `ai/pipelines/orchestrator/scripts/VPS_INVENTORY.json` - VPS dataset inventory
