...
class S3Connector(IngestionConnector):
    ...
    def __init__(
        ...
        aws_secret_access_key = config.aws_secret_access_key or os.getenv(
    ...
    def fetch(self) -> Iterable[IngestRecord]:
        ...
        try:
            ...
            pages = paginator.paginate(
                Bucket=self.config.bucket_name, Prefix=self.config.prefix
            )
                ...
                for obj in page["Contents"]:
                    ...
                    try:
                        ...
                        if self.rate_limiter:
                            ...
                            if not acquired:
                                raise IngestionError(
                                    f"Rate limiter timeout reading {key}"
                                )
                        ...
                        rec = IngestRecord(
                            ...
                            metadata={
                                ...
                                "key": key,
                                "size": len(content),
                                "content_type": response.get("ResponseMetadata", {})
                                .get("HTTPHeaders", {})
                        ...
                        if not self.validate(rec):
                            ...
                            errors = [
                                f"Source validation failed for {self.name} connector: {key}"
                            ]
            ...
            raise IngestionError(
                f"Error listing objects in bucket {self.config.bucket_name}: {e}"
            )
