...
class VoiceBlendIntegrator:
    ...
    def _create_expert_profiles(self) -> Dict[str, ExpertVoice]:
        ...
        return {"tim": tim_ferriss, "gabor": gabor_mate, "brene": brene_brown}
    ...
    def generate_tri_expert_responses(
        self, client_input: str, context: str = ""
    ) -> Dict[str, str]:
        ...
        # Blended response
        responses["blended"] = self._generate_blended_response(
            client_input, context, responses
        )
    ...
    def _generate_brene_response(self, client_input: str, context: str) -> str:
        ...
        if "shame" in client_input.lower() or "not enough" in client_input.lower():
            ...
            "vulnerability" in client_input.lower() or "scared" in client_input.lower()
        ):
    ...
    def _generate_blended_response(
        self, client_input: str, context: str, individual_responses: Dict[str, str]
    ) -> str:
    ...
    def create_voice_enhanced_datasets(
        self,
        base_conversations: List[Dict],
        output_path: str = "ai/training_data_consolidated/voice_enhanced/",
    ):
        ...
        for conversation in base_conversations:
            if "client" in conversation and "therapist" in conversation:
                ...
                expert_responses = self.generate_tri_expert_responses(
                    conversation["client"], conversation.get("context", "")
                )
        ...
        logger.info(
            f"Created {len(enhanced_datasets)} voice-enhanced datasets at {output_file}"
        )
...
def main():
    ...
    print(
        f"Created blended voice with {len(blended_voice['core_principles'])} principles"
    )
