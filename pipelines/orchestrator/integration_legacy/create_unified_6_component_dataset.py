...
def create_unified_6_component_dataset():
    ...
    for i, conversation in enumerate(base_conversations):
        ...
        if "conversation" in enhanced_conv:
            ...
            enhanced_conv["primary_response"] = expert_responses.get(
                "blended", enhanced_conv["conversation"].get("therapist", "")
            )

        # Apply edge case handling if relevant
        conversation_text = str(enhanced_conv.get("conversation", "")).lower()
        if any(
            trigger in conversation_text
            for trigger in ["crisis", "suicidal", "trauma", "overwhelmed", "emergency"]
        ):
            ...
            enhanced_conv["edge_case_handling"] = (
                edge_case_response[0] if edge_case_response else {}
            )
        ...
        psychology_enhancement = (
            psychology_kb.enhance_conversation_with_psychology_concepts(enhanced_conv)
        )
        enhanced_conv["psychology_concepts"] = psychology_enhancement.get(
            "psychology_concepts", {}
        )
    ...
    print("🎯 Ready for integration with 2.5GB final dataset!")
