...
class TrainingManifest:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "seed": self.seed,
            "safety_metrics": {
                "max_crisis_content_ratio": (
                    self.safety_metrics.max_crisis_content_ratio
                ),
                "demographic_balance_threshold": (
                    self.safety_metrics.demographic_balance_threshold
                ),
                "toxicity_threshold": self.safety_metrics.toxicity_threshold,
                "therapeutic_response_balance": (
                    self.safety_metrics.therapeutic_response_balance
                ),
                "privacy_preservation_enabled": (
                    self.safety_metrics.privacy_preservation_enabled
                ),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TrainingManifest":
        ...
        safety_metrics = SafetyMetrics(
            ...
            demographic_balance_threshold=safety_data.get(
                "demographic_balance_threshold", 0.1
            ),
            toxicity_threshold=safety_data.get("toxicity_threshold", 0.05),
            therapeutic_response_balance=safety_data.get(
                "therapeutic_response_balance", 0.15
            ),
            privacy_preservation_enabled=safety_data.get(
    ...
    def validate(self) -> List[str]:
        ...
        if (
            self.safety_metrics.max_crisis_content_ratio < 0
            or self.safety_metrics.max_crisis_content_ratio > 1
        ):
            errors.append("max_crisis_content_ratio must be between 0 and 1")
        if (
            self.safety_metrics.toxicity_threshold < 0
            or self.safety_metrics.toxicity_threshold > 1
        ):
        ...
        if (
            self.resources.max_budget_usd is not None
            and self.resources.max_budget_usd <= 0
        ):
...
def create_default_manifest(
    dataset_path: str = "./training_dataset.json", dataset_version: str = "1.0"
) -> TrainingManifest:
...
def create_safety_aware_manifest(
    dataset_path: str, dataset_version: str = "1.0"
) -> TrainingManifest:
    ...
    manifest.description = (
        "Safety-focused training manifest for therapeutic conversation model "
...
def test_training_manifest():
    ...
    max_crisis_ratio = safety_manifest.safety_metrics.max_crisis_content_ratio
    print(f"Max crisis content ratio: {max_crisis_ratio}")
    ...
    assert "Dataset name is required" in validation_errors
    assert "Dataset version is required" in validation_errors
