...
class VoiceConversationFormatConverter:
    ...
    def __init__(
        ...
        output_dir: str = "./converted_conversations",
    ):
        ...
        self.speaker_patterns = {
            ...
            "counselor_indicators": [
                "what would you like to work on",
                "let's focus on",
                "that's a great insight",
            ],
            "facilitator_indicators": [
                "let's go around",
    ...
    def convert_voice_to_conversation_format(
        self, voice_file_path: str, target_format: str = "therapeutic"
    ) -> dict[str, Any]:
        ...
        try:
            ...
            role_assignments = self._identify_speaker_roles(
                speaker_segments, target_format
            )
            ...
            output_path = self._save_converted_conversation(
                conversation_data, quality_metrics
            )
            ...
            logger.info(
                f"Successfully converted voice to conversation format: {conversation_data.conversation_id}"
            )
            ...
            "speaker_timestamps": [
                {
                    ...
                    "start": 0.0,
                    "end": 8.5,
                    "text": "I've been feeling really anxious lately and I don't know what to do about it.",
                },
                {
                    "speaker": "Speaker 2",
                    "start": 9.0,
                    "end": 16.2,
                    "text": "I hear you saying that you're experiencing anxiety. Can you tell me more about when these feelings started?",
                },
                {
                    "speaker": "Speaker 1",
                    "start": 17.0,
                    "end": 24.8,
                    "text": "It started about a month ago when I changed jobs. Everything feels overwhelming now.",
                },
                {
                    "speaker": "Speaker 2",
                    "start": 25.5,
                    "end": 33.1,
                    "text": "Job transitions can be really challenging. Let's explore what specific aspects feel most overwhelming to you.",
                },
            ],
            "audio_quality": 0.85,
            "duration": 35.0,
    ...
    def _identify_speaker_roles(
        self, segments: list[dict[str, Any]], target_format: str
    ) -> dict[str, str]:
        """Identify speaker roles based on conversation patterns."""
        format_config = self.conversation_formats.get(
            target_format, self.conversation_formats["therapeutic"]
        )
        ...
        for speaker_id, scores in speaker_scores.items():
            ...
            if best_role not in assigned_roles:
                ...
                remaining_roles = [
                    r for r in available_roles if r not in assigned_roles
                ]
                if remaining_roles:
    ...
    def _convert_to_conversation_format(
        self,
    ...
    def _assess_conversion_quality(
        self, conversation_data: VoiceConversationData
    ) -> dict[str, float]:
        ...
        role_diversity = len(
            {msg["role"] for msg in messages if msg["role"] != "unknown"}
        )
        ...
        if len(speaker_durations) > 1:
            ...
            duration_balance = 1.0 - max(
                d / total_duration for d in speaker_durations.values()
            )
        else:
            duration_balance = 0.0

        overall_quality = (
            role_quality + avg_message_quality + flow_quality + duration_balance
        ) / 4
    ...
    def _save_converted_conversation(
        self,
        conversation_data: VoiceConversationData,
        quality_metrics: dict[str, float],
    ) -> Path:
        """Save converted conversation format."""
        output_file = (
            self.output_dir / f"{conversation_data.conversation_id}_converted.json"
        )
...
if __name__ == "__main__":
    ...
    result = converter.convert_voice_to_conversation_format(
        "mock_audio.wav", "therapeutic"
    )
