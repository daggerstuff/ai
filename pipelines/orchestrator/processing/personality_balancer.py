...
class PersonalityBalancer:
    ...
    def balance_personalities(
        self,
        conversations: list[Conversation],
        target_framework: PersonalityFramework = PersonalityFramework.BIG_FIVE,
    ) -> BalancingResult:
        """
        ...
        """
        self.logger.info(
            f"Balancing {len(conversations)} conversations using {target_framework.value}"
        )

        # Extract personality profiles from conversations
        personality_profiles = self._extract_personality_profiles(
            conversations, target_framework
        )

        # Analyze current distribution
        current_distribution = self._analyze_distribution(
            personality_profiles, target_framework
        )

        # Balance the conversations
        balanced_conversations = self._apply_balancing(
            conversations, personality_profiles, current_distribution, target_framework
        )

        # Calculate final distribution
        final_profiles = self._extract_personality_profiles(
            balanced_conversations, target_framework
        )
        final_distribution = self._analyze_distribution(
            final_profiles, target_framework
        )

        # Calculate quality metrics
        quality_metrics = self._calculate_balancing_quality(
            current_distribution, final_distribution, target_framework
        )
    ...
    def _extract_personality_profiles(
        self, conversations: list[Conversation], framework: PersonalityFramework
    ...
    def _analyze_conversation_personality(
        self, conversation: Conversation, framework: PersonalityFramework
        ...
        try:
            ...
            self.logger.warning(
                f"Could not analyze personality for conversation {conversation.id}: {e}"
            )
            return None
    ...
    def _analyze_generic(
        self, conv_id: str, content: str, framework: PersonalityFramework
    ) -> PersonalityProfile:
    ...
    def _analyze_distribution(
        self, profiles: list[PersonalityProfile], framework: PersonalityFramework
    ) -> dict[str, int]:
    ...
    def _apply_balancing(
        ...
        if remaining_slots > 0:
            ...
            remaining_convs = [
                conv for conv in conversations if conv.id not in used_ids
            ]
            balanced_conversations.extend(remaining_convs[:remaining_slots])

        return balanced_conversations

    def _get_personality_category(
        self, profile: PersonalityProfile, framework: PersonalityFramework
    ) -> str:
    ...
    def _calculate_balancing_quality(
        self,
        current_dist: dict[str, int],
        ...
        coverage = len(
            [cat for cat in target_dist if final_dist.get(cat, 0) > 0]
        ) / len(target_dist)
        ...
        return {
            ...
            "represented_categories": len(
                [cat for cat in target_dist if final_dist.get(cat, 0) > 0]
            ),
        }

    def get_balancing_recommendations(
        self,
        conversations: list[Conversation],
        framework: PersonalityFramework = PersonalityFramework.BIG_FIVE,
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "target_distribution": {
                k: int(total_conversations * v) for k, v in target_dist.items()
            },
