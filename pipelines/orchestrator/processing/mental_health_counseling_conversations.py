...
class MentalHealthCounselingConversations:
        ...
        try:
            ...
            if not self.dataset_path.exists():
                ...
                result["issues"].append(
                    "Created mock counseling conversations for testing"
                )
            ...
            output_path = self._save_processed_conversations(
                processed_conversations, quality_metrics, technique_counts
            )
            ...
            logger.info(
                f"Successfully processed counseling conversations: {len(processed_conversations)} conversations, {total_responses} responses"
            )
    ...
    def _create_mock_counseling_data(self):
        ...
        while total_responses < target_responses:
            ...
            for i in range(num_exchanges):
                ...
                client_msg = {
                    "role": "client",
                    "content": client_concerns[conv_id % len(client_concerns)]
                    if i == 0
                    else f"Follow-up concern {i} from client in conversation {conv_id}",
                    "timestamp": datetime.now().isoformat(),
                }
                conversation["messages"].append(client_msg)

                # Licensed therapist response
                therapist_response = self._generate_licensed_response(
                    client_msg["content"], conv_id, i
                )
                therapist_msg = {
            ...
            conversation["metadata"] = {
                ...
                "client_demographics": {
                    ...
                    "presenting_concern": client_concerns[
                        conv_id % len(client_concerns)
                    ],
                },
    ...
    def _generate_licensed_response(
        self, client_content: str, conv_id: int, exchange_num: int
    ) -> dict[str, Any]:
        ...
        if exchange_num == 0:
            ...
            response_parts.append(
                f"I hear you saying that {client_content.lower()[:50]}..."
            )
        ...
        if "anxious" in client_content.lower() or "worry" in client_content.lower():
            ...
            response_parts.append(
                "Anxiety is a common experience, and there are effective techniques we can explore together."
            )
        elif "depressed" in client_content.lower() or "sad" in client_content.lower():
            techniques_used.append("cognitive_reframing")
            response_parts.append(
                "Let's explore some different ways of looking at this situation."
            )
        else:
            techniques_used.append("solution_focused")
            response_parts.append(
                "What small step do you think might be helpful to try this week?"
            )

        return {"content": " ".join(response_parts), "techniques": techniques_used}
    ...
    ) -> CounselingConversation | None:
        """Process a single counseling conversation."""
        try:
            conversation_id = conv_data.get(
                "conversation_id", f"conv_{hash(str(conv_data)) % 10000}"
            )
    ...
    def _assess_individual_conversation_quality(
        self,
        client_messages: list[str],
        therapist_responses: list[str],
        techniques: list[str],
        ...
        if therapist_responses:
            avg_response_length = sum(
                len(r.split()) for r in therapist_responses
            ) / len(therapist_responses)
            quality_scores["therapeutic_alliance"] = min(
                1.0, avg_response_length / 30
            )  # 30 words = good response
        else:
            quality_scores["therapeutic_alliance"] = 0.0

        # Empathy demonstration (technique diversity)
        quality_scores["empathy_demonstration"] = (
            len(set(techniques)) / len(self.therapeutic_techniques)
            if techniques
            else 0.0
        )
        ...
        for response in therapist_responses:
            if any(
                ...
                for inappropriate in ["personal", "my own", "i think you should"]
            ):
                boundary_score -= 0.2
        ...
        evidence_based_count = sum(
            1 for t in techniques if t in self.therapeutic_techniques
        )
        quality_scores["evidence_based_techniques"] = evidence_based_count / max(
            1, len(techniques)
        )
        ...
        for msg in client_messages:
            if any(
                crisis_word in msg.lower()
                for crisis_word in ["suicide", "kill", "hurt myself", "end it all"]
            ):
                # Check if therapist addressed safety
                safety_addressed = any(
                    "safety" in r.lower() or "crisis" in r.lower()
                    for r in therapist_responses
                )
    ...
    def _assess_conversation_quality(
        self, conversations: list[CounselingConversation]
    ) -> dict[str, float]:
        ...
        # Overall quality
        quality_metrics["overall_quality"] = sum(quality_metrics.values()) / len(
            self.quality_criteria
        )

        # Additional metrics
        quality_metrics["average_responses_per_conversation"] = sum(
            len(c.therapist_responses) for c in conversations
        ) / len(conversations)
    ...
    def _save_processed_conversations(
        self,
        conversations: list[CounselingConversation],
        quality_metrics: dict[str, float],
        technique_counts: dict[str, int],
    ) -> Path:
        """Save processed conversations."""
        output_file = (
            self.output_dir / "mental_health_counseling_conversations_processed.json"
        )
