...
class DatasetCategorizeRequest(BaseModel):
    ...
    dataset_name: str = Field(
        default="unnamed_dataset", description="Name of the dataset"
    )
    dataset_description: str = Field(
        default="", description="Description of the dataset"
    )
    sample_size: int = Field(
        default=100, ge=10, le=1000, description="Sample size for analysis"
    )
...
class DatasetCategorizeResponse(BaseModel):
    ...
    recommended_subcategories: List[str] = Field(
        ..., description="Recommended subcategories"
    )
...
class TrainingStyleSelectRequest(BaseModel):
    ...
    training_goals: Dict[str, Any] = Field(
        default={}, description="Training goals and constraints"
    )
    available_compute: str = Field(
        default="medium", description="Available compute resources"
    )
...
class TrainingStyleSelectResponse(BaseModel):
    ...
    style_info: Dict[str, Any] = Field(
        ..., description="Information about the selected style"
    )
...
class TrainingConfigRequest(BaseModel):
    ...
    optimize: bool = Field(
        default=False, description="Whether to optimize configuration"
    )
...
class PipelineExecuteRequest(BaseModel):
    ...
    training_objective: str = Field(
        default="therapeutic_accuracy", description="Training objective"
    )
    available_compute: str = Field(
        default="medium", description="Available compute resources"
    )
    safety_requirements: str = Field(
        default="moderate", description="Safety requirements"
    )
    use_container: bool = Field(
        default=False, description="Whether to use containerized training"
    )
    enable_monitoring: bool = Field(
        default=True, description="Whether to enable monitoring"
    )
...
class TrainingResultsResponse(BaseModel):
    ...
    therapeutic_appropriateness: float = Field(
        ..., description="Therapeutic appropriateness score"
    )
...
class DatasetTrainingAPI:
    ...
    def _setup_routes(self):
        ...
        @self.app.post(
            "/api/datasets/categorize", response_model=DatasetCategorizeResponse
        )
        async def categorize_dataset(request: DatasetCategorizeRequest):
            ...
            try:
                ...
                return DatasetCategorizeResponse(
                    ...
                    recommended_subcategories=result["results"][
        ...
        async def validate_dataset(metadata: Dict[str, Any]):
            ...
            try:
                # Create MCP task
                task = MCPTask(
                    task_type="dataset_validate", parameters={"metadata": metadata}
                )
        ...
        @self.app.post(
            "/api/training/styles/select", response_model=TrainingStyleSelectResponse
        )
        ...
        @self.app.post(
            "/api/training/styles/configure", response_model=TrainingConfigResponse
        )
        ...
        async def execute_training_pipeline(
            request: PipelineExecuteRequest, background_tasks: BackgroundTasks
        ):
        ...
        @self.app.get(
            "/api/training/{execution_id}/status", response_model=PipelineStatusResponse
        )
        ...
        @self.app.get(
            "/api/training/{execution_id}/results",
        ...
        async def get_taxonomy_categories():
            ...
            try:
                # Create MCP task
                task = MCPTask(task_type="taxonomy_info", parameters={})
        ...
        async def get_taxonomy_category(category: str):
            ...
            try:
                # Create MCP task
                task = MCPTask(
                    task_type="taxonomy_info", parameters={"category": category}
                )
        ...
        async def get_training_styles():
            ...
            try:
                # Create MCP task
                task = MCPTask(task_type="style_info", parameters={})
        ...
        async def get_training_style(style: str):
            ...
            try:
                # Create MCP task
                task = MCPTask(task_type="style_info", parameters={"style": style})
                ...
                if not style_info:
                    raise HTTPException(
                        status_code=404, detail="Training style not found"
                    )
    ...
    def _estimate_completion(self, status: Dict[str, Any]) -> Optional[str]:
        ...
        if progress > 0:
            elapsed_time = (
                datetime.utcnow() - datetime.fromisoformat(status["start_time"])
            ).total_seconds()
            estimated_total_time = elapsed_time / (progress / 100)
            remaining_time = estimated_total_time - elapsed_time
            return (
                datetime.utcnow().isoformat() + f" (in {remaining_time/3600:.1f} hours)"
            )
