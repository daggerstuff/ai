...
class EvaluationGatesSystem:
    ...
    def register_gate_configuration(
        self, stage: PromotionStage, config: GateConfiguration
    ):
    ...
    def evaluate_model_for_promotion(
        ...
        self.logger.info(
            f"Evaluating model {model_id} v{model_version} for promotion from {from_stage.value} to {to_stage.value}"
        )
        ...
        required_passing = config.required_passing_gates or len(
            config.gates
        )  # 0 means all
        ...
        is_approved = (
            has_required_passing and meets_score_threshold and no_critical_failures
        )
        ...
        self.logger.info(
            f"Promotion evaluation result: {'APPROVED' if is_approved else 'REJECTED'} - {reason}"
        )
...
class ModelPromotionManager:
    ...
    def promote_model(
        ...
        self.logger.info(
            f"Attempting to promote model {model_id} v{model_version} from {from_stage.value} to {to_stage.value}"
        )
        ...
        if evaluation.is_approved:
            self.logger.info(
                f"✅ Model {model_id} approved for promotion to {to_stage.value}"
            )
        else:
            self.logger.warning(
                f"❌ Model {model_id} rejected for promotion to {to_stage.value}: {evaluation.reason}"
            )
    ...
    def generate_promotion_report(self, model_id: str) -> str:
        ...
        for eval_result in history:
            ...
            report_parts.append(
                f"From: {eval_result.from_stage.value} -> To: {eval_result.to_stage.value}"
            )
            report_parts.append(
                f"Status: {'APPROVED' if eval_result.is_approved else 'REJECTED'}"
            )
            report_parts.append(
                f"Score: {eval_result.overall_score:.3f} ({eval_result.passed_gates}/{eval_result.total_gates} gates passed)"
            )
