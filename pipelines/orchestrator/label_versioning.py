...
class LabelVersion:
    ...
    confidence_change: Optional[float] = (
        None  # Change in confidence from previous version
    )
...
class LabelVersionManager:
    ...
    def create_initial_version(
        ...
        history = self._get_or_create_history(
            label_bundle.label_id, label_bundle.conversation_id
        )
        history.add_version(version)
        history.add_provenance_record(provenance)

        self.logger.info(
            f"Created initial version {version.version_number} for label bundle {label_bundle.label_id}"
        )
        return version

    def update_label_bundle(
        self,
        label_bundle: LabelBundle,
        ...
        new_version = LabelVersion(
            ...
            or f"Update to label bundle version {previous_version.version_number + 1}",
            confidence_change=confidence_change,
        )

        # Create provenance record
        provenance = self._create_provenance_record(
            label_bundle, new_version.version_id, actor=actor
        )
        ...
        self.logger.info(
            f"Updated label bundle {label_bundle.label_id} to version {new_version.version_number}"
        )
        return new_version
    ...
    def _get_or_create_history(
        self, label_bundle_id: str, conversation_id: str
    ) -> LabelHistory:
        ...
        if label_bundle_id not in self.label_histories:
            self.label_histories[label_bundle_id] = LabelHistory(
                label_bundle_id=label_bundle_id, conversation_id=conversation_id
            )
    ...
    def _deserialize_label_bundle(self, data: Dict[str, Any]) -> LabelBundle:
        ...
        for label_data in data.get("therapeutic_response_labels", []):
            label = TherapeuticResponseLabel(
                ...
                metadata=LabelMetadata(
                    ...
                    additional_context=label_data["metadata"].get(
                        "additional_context", {}
                    ),
                ),
            )
        ...
        if data.get("crisis_label"):
            ...
            crisis_label = CrisisLabel(
                ...
                metadata=LabelMetadata(
                    ...
                    additional_context=crisis_data["metadata"].get(
                        "additional_context", {}
                    ),
                ),
            )
        ...
        if data.get("therapy_modality_label"):
            ...
            modality_label = TherapyModalityLabel(
                ...
                metadata=LabelMetadata(
                    ...
                    additional_context=modality_data["metadata"].get(
                        "additional_context", {}
                    ),
                ),
            )
        ...
        if data.get("mental_health_condition_label"):
            ...
            condition_label = MentalHealthConditionLabel(
                ...
                if condition_data.get("co_morbidities")
                else [],
                metadata=LabelMetadata(
                    ...
                    additional_context=condition_data["metadata"].get(
                        "additional_context", {}
                    ),
                ),
            )
        ...
        if data.get("demographic_label"):
            ...
            demographic_label = DemographicLabel(
                demographics=[
                    DemographicType(d) for d in demo_data.get("demographics", [])
                ],
                ...
                metadata=LabelMetadata(
                    ...
                    confidence_explanation=demo_data["metadata"][
                        "confidence_explanation"
                    ],
                    ...
                    additional_context=demo_data["metadata"].get(
                        "additional_context", {}
                    ),
                ),
            )
    ...
    def _create_provenance_record(
        ...
        if label_bundle.therapy_modality_label:
            all_provenances.append(
                label_bundle.therapy_modality_label.metadata.provenance
            )
        if label_bundle.mental_health_condition_label:
            all_provenances.append(
                label_bundle.mental_health_condition_label.metadata.provenance
            )
        ...
        if any(
            p
            in [LabelProvenanceType.HUMAN_EXPERT, LabelProvenanceType.HUMAN_IN_THE_LOOP]
            for p in all_provenances
        ):
            primary_source = LabelProvenanceType.HUMAN_IN_THE_LOOP
        elif any(
            p == LabelProvenanceType.COMBINED_MODEL_HUMAN for p in all_provenances
        ):
    ...
    def compute_bundle_hash(self, label_bundle: LabelBundle) -> str:
        ...
        serialized = json.dumps(
            self._serialize_label_bundle(label_bundle), sort_keys=True
        )
        # Create hash
...
class ProvenanceAnalyzer:
    ...
    def analyze_provenance_distribution(self) -> Dict[str, int]:
        ...
        for history in self.version_manager.label_histories.values():
            if history.provenance_records:
                ...
                latest_record = max(
                    history.provenance_records, key=lambda r: r.timestamp
                )
                provenance_type = latest_record.source.value
                provenance_counts[provenance_type] = (
                    provenance_counts.get(provenance_type, 0) + 1
                )
    ...
    def find_low_confidence_provenance_chains(self) -> List[Dict[str, Any]]:
        ...
        for history in self.version_manager.label_histories.values():
            ...
            for version in history.versions:
                ...
                bundle = self.version_manager._deserialize_label_bundle(
                    version.label_data
                )
    ...
    def _has_low_confidence_labels(self, bundle: LabelBundle) -> bool:
        ...
        if bundle.crisis_label and bundle.crisis_label.metadata.confidence < 0.8:
            return True
        if (
            bundle.therapy_modality_label
            and bundle.therapy_modality_label.metadata.confidence < 0.7
        ):
            return True
        if (
            bundle.mental_health_condition_label
            and bundle.mental_health_condition_label.metadata.confidence < 0.7
        ):
            return True
        if (
            bundle.demographic_label
            and bundle.demographic_label.metadata.confidence < 0.6
        ):
            return True
        return False
    ...
    def _get_confidence_issues(self, bundle: LabelBundle) -> List[str]:
        ...
        for i, label in enumerate(bundle.therapeutic_response_labels):
            if label.metadata.confidence < 0.7:
                issues.append(
                    f"Therapeutic response {i}: {label.response_type.value} confidence {label.metadata.confidence}"
                )
        if bundle.crisis_label and bundle.crisis_label.metadata.confidence < 0.8:
            issues.append(
                f"Crisis label: {bundle.crisis_label.crisis_level.value} confidence {bundle.crisis_label.metadata.confidence}"
            )
        if (
            bundle.therapy_modality_label
            and bundle.therapy_modality_label.metadata.confidence < 0.7
            ...
            issues.append(
                f"Modality label: {bundle.therapy_modality_label.modality.value} confidence {bundle.therapy_modality_label.metadata.confidence}"
            )
        if (
            bundle.mental_health_condition_label
            and bundle.mental_health_condition_label.metadata.confidence < 0.7
            ...
            issues.append(
                f"Condition label confidence {bundle.mental_health_condition_label.metadata.confidence}"
            )
        if (
            bundle.demographic_label
            and bundle.demographic_label.metadata.confidence < 0.6
            ...
            issues.append(
                f"Demographic label confidence {bundle.demographic_label.metadata.confidence}"
            )
        return issues
...
def test_label_versioning():
    ...
    initial_bundle.therapeutic_response_labels.append(
        TherapeuticResponseLabel(
            ...
            metadata=LabelMetadata(
                confidence=0.85, provenance=LabelProvenanceType.AUTOMATED_MODEL
            ),
        )
    )
    ...
    initial_bundle.crisis_label = CrisisLabel(
        ...
        metadata=LabelMetadata(
            confidence=0.9, provenance=LabelProvenanceType.AUTOMATED_MODEL
        ),
    )
    ...
    initial_version = vm.create_initial_version(
        initial_bundle, "system", "Initial automated labels"
    )
    ...
    updated_bundle.therapeutic_response_labels[
        0
    ].metadata.provenance = LabelProvenanceType.HUMAN_EXPERT
    updated_bundle.therapeutic_response_labels[
        0
    ].metadata.annotator_id = "human_expert_123"
    ...
    if current_bundle:
        print(
            f"Current bundle has {len(current_bundle.therapeutic_response_labels)} therapeutic labels"
        )
        print(
            f"First label confidence: {current_bundle.therapeutic_response_labels[0].metadata.confidence}"
        )
        print(
            f"First label provenance: {current_bundle.therapeutic_response_labels[0].metadata.provenance.value}"
        )
