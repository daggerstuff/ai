...
class ProductionDatasetGenerator:
    ...
    def generate_production_dataset(
        self,
        conversations: list[Conversation],
        dataset_name: str = "therapeutic_training",
    ) -> ProductionDataset:
        """Generate a production-ready dataset."""
        self.logger.info(
            f"Generating production dataset '{dataset_name}' with {len(conversations)} conversations"
        )
        ...
        generation_stats = {
            ...
            "quality_filter_rate": (
                len(quality_conversations) / len(conversations) if conversations else 0
            ),
        }

        self.logger.info(
            f"Generated production dataset with {len(enhanced_conversations)} conversations"
        )
    ...
    def _filter_by_quality(
        ...
        for conv in conversations:
            ...
            if len(conv.messages) >= 2:  # At least one exchange
                if all(
                    len(msg.content.strip()) > 10 for msg in conv.messages
                ):  # Substantial content
