                ...
                for item in eval_dataset:
                    if "text" in item:
                        ...
                        text = tokenizer.decode(
                            item["input_ids"][:512], skip_special_tokens=True
                        )
                        texts.append(text)
...
class SafetyEvaluator:
    ...
    def evaluate_safety(
        ...
        results["crisis_content_ratio"] = (
            crisis_content_count / total_content_count if total_content_count > 0 else 0
        )
            ...
            for item in eval_dataset:
                ...
                if text and len(text) > 10:  # Only evaluate non-trivial text
                    ...
                    try:
                        ...
                        if any(
                            pattern in text_lower for pattern in self.toxicity_patterns
                        ):
                            toxic_count += 1
            results["toxicity_ratio"] = (
                toxic_count / total_evaluated if total_evaluated > 0 else 0
            )
        ...
        results["therapeutic_content_ratio"] = (
            therapeutic_content_count / len(eval_dataset)
...
class FairnessEvaluator:
    ...
    def evaluate_fairness(
        ...
        ratios = [
            results.get(f"{cat}_representation_ratio", 0)
...
class TherapeuticResponseEvaluator:
    ...
    def evaluate_therapeutic_responses(
        self,
        model: PreTrainedModel,
        tokenizer: PreTrainedTokenizer,
        dataset: Dataset,
...
class ComprehensiveEvaluator:
        ...
        return EvaluationResults(
            ...
            metadata={
                "evaluated_samples": min(
                    len(dataset), max_samples if max_samples else len(dataset)
                ),
                "evaluation_timestamp": __import__("datetime")
                .datetime.utcnow()
                .isoformat(),
            },
        )
    ...
    def _calculate_summary_metrics(
        self, all_metrics: Dict[str, float]
    ) -> Dict[str, float]:
        ...
        if self.accuracy_evaluator.bleu_metric:
            try:
                bleu_results = self.accuracy_evaluator.bleu_metric.compute(
                    predictions=predictions, references=[[gt] for gt in ground_truth]
                )
    ...
    def generate_evaluation_report(self, eval_results: EvaluationResults) -> str:
        ...
        for key, value in acc_metrics.items():
            if (
                ...
                or "balance" in key
                or any(
                    demo in key for demo in ["age", "gender", "race", "socioeconomic"]
                )
            ):
                report.append(f"  {key}: {value:.4f}")
        ...
        summary_keys = [
            ...
            if "score" in k and "overall" in k or "quality" in k
        ]
...
def test_evaluation_system():
    ...
    therapeutic_results = (
        evaluator.therapeutic_evaluator.evaluate_therapeutic_responses(
            model=None,
            tokenizer=None,
            dataset=mock_data,
            max_samples=3,
