...
class ClinicalAccuracyValidator:
    ...
    def validate_conversation(
        self, conversation: Conversation, scenario: ClientScenario | None = None
    ) -> ValidationResult:
        ...
        therapeutic_issues = self._validate_therapeutic_appropriateness(
        ...
        result = ValidationResult(
            ...
            validation_metadata={
                ...
                "therapist_messages": len(
                    [m for m in conversation.messages if m.role == "therapist"]
                ),
                "client_messages": len(
                    [m for m in conversation.messages if m.role == "client"]
                ),
                "validation_categories": list(
                    {issue.category.value for issue in issues}
                ),
        ...
        logger.info(
            f"Validated conversation {conversation.id}: Score {overall_score:.2f}, Acceptable: {is_acceptable}"
        )
        return result

    def _validate_ethical_compliance(
    ...
    def _validate_clinical_accuracy(
        self, conversation: Conversation, scenario: ClientScenario | None
    ) -> list[ValidationIssue]:
    ...
    def _validate_therapeutic_appropriateness(
        self, conversation: Conversation, scenario: ClientScenario | None
    ) -> list[ValidationIssue]:
        ...
        for i, message in enumerate(conversation.messages):
            ...
            if technique in self.technique_appropriateness:
                ...
                if scenario:
                    scenario_concerns = [
                        *scenario.presenting_problem.symptoms,
                        scenario.presenting_problem.primary_concern.lower(),
                    ]

                    # Check inappropriate usage
                    for inappropriate in rules.get("inappropriate_for", []):
                        if any(
                            inappropriate in concern.lower()
                            for concern in scenario_concerns
                        ):
                            issues.append(
                                ValidationIssue(
                                    ...
                                    evidence_base=[
                                        f"Evidence-based practice for {technique_str}"
                                    ],
                                )
                            )
    ...
    def _validate_safety_protocols(
        self, conversation: Conversation, scenario: ClientScenario | None
    ) -> list[ValidationIssue]:
    ...
    def _validate_professional_boundaries(
        self, conversation: Conversation
    ) -> list[ValidationIssue]:
    ...
    def _validate_cultural_sensitivity(
        self, conversation: Conversation, scenario: ClientScenario | None
    ) -> list[ValidationIssue]:
    ...
    def _identify_strengths(
        ...
        if (
            empathy_count >= len(therapist_messages) * 0.3
        ):  # 30% of messages show empathy
    ...
    def _generate_recommendations(
        ...
        if ValidationCategory.ETHICAL_COMPLIANCE.value in issue_categories:
            recommendations.append(
                "Review ethical guidelines and confidentiality requirements"
            )

        if ValidationCategory.SAFETY_PROTOCOLS.value in issue_categories:
            recommendations.append(
                "Implement comprehensive safety assessment and planning protocols"
            )

        if ValidationCategory.THERAPEUTIC_APPROPRIATENESS.value in issue_categories:
            recommendations.append(
                "Ensure therapeutic techniques match client presentation and readiness"
            )
        ...
        if critical_issues:
            recommendations.insert(
                0, "Address critical safety and ethical concerns immediately"
            )

        return recommendations

    def _calculate_overall_score(
        self, issues: list[ValidationIssue], strengths: list[str]
    ) -> float:
    ...
    def _determine_acceptability(
        ...
        high_issues = [
            issue for issue in issues if issue.severity == ValidationSeverity.HIGH
        ]
    ...
    def validate_conversation_batch(
        self,
        conversations: list[Conversation],
        scenarios: list[ClientScenario] | None = None,
    ...
    def export_validation_results(
        ...
        try:
            ...
            logger.info(
                f"Exported validation results for {len(results)} conversations to {output_path}"
            )
            return True
    ...
    def get_validation_statistics(
        ...
        stats = {
            ...
            "acceptance_rate": sum(1 for r in results if r.is_clinically_acceptable)
            / len(results),
        ...
        for issue in all_issues:
            category = issue.category.value
            stats["issue_categories"][category] = (
                stats["issue_categories"].get(category, 0) + 1
            )
        ...
        stats["common_issues"] = dict(
            sorted(issue_messages.items(), key=lambda x: x[1], reverse=True)[:5]
        )
