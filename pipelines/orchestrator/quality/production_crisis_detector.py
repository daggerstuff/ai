...
class ProductionCrisisDetector:
        ...
        return {
            "suicide_ideation": [
                ...
                CrisisIndicator(
                    "explicit_intent",
                    1.0,
                    r"(?:want to|going to|plan to) (?:kill myself|end my life|commit suicide)",
                    immediate_flag=True,
                ),
                CrisisIndicator(
                    "imminent_action",
                    1.0,
                    r"(?:doing it now|can't stop myself|it's time|tonight)",
                    immediate_flag=True,
                ),
                # High severity patterns
                # Reduced from 0.9 to avoid false positive emergency
                CrisisIndicator(
                    "method_with_access",
                    0.82,
                    r"(?:have (?:a )?gun|got (?:the )?pills|found (?:a )?rope)",
                ),
                CrisisIndicator(
                    "detailed_plan",
                    0.82,
                ...
                # Removed timeline_urgent as a standalone indicator to prevent false positives "right now"
                ...
                CrisisIndicator(
                    "passive_ideation",
                    0.5,
                    r"(?:sometimes think about|wish i was|wouldn't mind if|not being here|wasn't here|gone forever)",
                ),
                CrisisIndicator(
                    "burden_thoughts",
                    0.6,
            ...
            "self_harm": [
                ...
                CrisisIndicator(
                    "burning_specific",
                    0.8,
                    r"(?:burning myself with|cigarette burns|lighter burns)",
                ),
                CrisisIndicator(
                    "tools_dangerous",
                    0.8,
                    r"(?:razor blade|kitchen knife|broken glass|sharp object)",
                ),
                # Medium severity patterns
                CrisisIndicator(
                    "cutting_behavior",
            ...
            "psychotic_episode": [
                # Immediate emergency patterns
                # Made regex more flexible to catch "voices are telling me to"
                CrisisIndicator(
                    "command_harm",
                    1.0,
                    r"(?:voices .*? (?:tell|command) .*? to hurt|command me to kill|must hurt)",
                    immediate_flag=True,
                ),
                CrisisIndicator(
                    "command_voices",
                    0.9,
                    r"(?:voices .*? (?:tell|command) .*? to|commanded to|must obey|voices say)",
                    immediate_flag=True,
                ),
                # High severity patterns
                CrisisIndicator(
            ...
            "eating_disorder_crisis": [
                ...
                CrisisIndicator(
                    ...
                    0.9,
                    r"(?:fainting|heart problems|medical emergency) (?:from|because|due to) (?:not eating|eating disorder|restriction)",
                ),
                # High severity patterns
                CrisisIndicator(
                    "restriction_extreme",
                    0.8,
                    r"(?:haven't eaten (?:in|for) days|refusing all food|starving myself)",
                ),
                CrisisIndicator(
                    "purging_escalation",
                    0.8,
                    r"(?:throwing up|vomiting|purging) (?:multiple times|all day|after every)",
                ),
                # Medium severity patterns
                CrisisIndicator(
                    "ed_physical_symptoms",
                    0.6,
                    r"(?:dizzy|weak|losing hair|teeth problems) (?:from|because|due to) (?:eating|food|restriction)",
                ),
                CrisisIndicator(
                    "ed_control_loss",
                    0.7,
                    r"(?:can'?t control|completely out of control) (?:eating|food|binge)",
                ),
            ],
            "violence_threat": [
                # Immediate emergency patterns
                CrisisIndicator(
                    "threat_explicit",
                    1.0,
                    r"(?:going to|will|plan to) (?:kill|hurt|attack|shoot|stab)",
                    immediate_flag=True,
                ),
                # High severity patterns
                CrisisIndicator(
                ...
                CrisisIndicator(
                    "stalking_behavior",
                    0.8,
                    r"(?:following (?:her|him|them)|know (?:her|him|their) schedule|watching (?:her|him|them)|stalking)",
                ),
                # Medium severity patterns
                CrisisIndicator(
                    "violence_fantasy",
                    0.6,
                    r"(?:imagine|fantasize about|dream of) (?:hurting|killing|attacking)",
                ),
                CrisisIndicator(
                    "anger_escalation", 0.5, r"(?:so angry|rage|furious|can't control)"
                ),
            ],
            "substance_overdose": [
                CrisisIndicator(
                    "overdose_explicit",
                    1.0,
                    r"(?:overdosed|took too many|drank too much|whole bottle)",
                    immediate_flag=True,
                ),
                CrisisIndicator(
                    "medical_symptoms",
            ...
            "severe_depression": [
                CrisisIndicator(
                    "severe_hopelessness",
                    0.7,
                    r"(?:completely hopeless|no reason to live|nothing matters)",
                ),
                # Boosted weight to meet threshold
                CrisisIndicator(
                    "functioning_loss",
                    0.62,
                ...
                CrisisIndicator(
                    "emotional_overwhelm",
                    0.6,
                    r"(?:emotional pain|can't handle this|can't take it anymore|overwhelming pain)",
                ),
            ],
        }

    # ... (skipping unchanged methods until _apply_production_modifiers) ...

    def _validate_context_production(
        self, indicator: CrisisIndicator, content: str, crisis_type: str
    ) -> bool:
        """Production-level context validation."""

        # Eating disorder patterns need eating context
        if crisis_type == "eating_disorder_crisis" and indicator.indicator_type in [
            "ed_physical_symptoms"
        ]:
            eating_context = re.search(
                r"(?:eat|food|meal|hungry|starv|binge|purg|vomit|restriction|disorder)",
                content,
                re.IGNORECASE,
            )
        ...
        if crisis_type == "panic_attack" and indicator.indicator_type in [
            "panic_breathing",
            "panic_heart",
            "panic_chest",
        ]:
            panic_context = re.search(
                r"(?:panic|attack|anxiety|sudden|overwhelming|terror|afraid)",
                content,
                re.IGNORECASE,
            )
        ...
        if crisis_type == "self_harm":
            if "myself" not in content and "self" not in content:
                return False
            # Explicit negation check for "hurt myself"
            if re.search(
                r"(?:don'?t|do not|never) (?:want to|mean to|intend to) (?:hurt|harm|cut) myself",
                content,
                re.IGNORECASE,
            ):
                return False
    ...
    def _apply_production_modifiers(
        ...
        if crisis_type == "suicide_ideation":
            ...
            # Check for urgency keywords (moved from indicators)
            is_urgent = re.search(
                r"(?:tonight|today|tomorrow|soon|right now)", content, re.IGNORECASE
            )
            ...
            if any(ind.indicator_type in passive_indicators for ind in indicators):
                ...
                if not active_indicators:
                    ...
                    # If urgent but only passive thoughts, boost slightly but keep below critical unless explicit
                    if is_urgent:
                        base_score = min(base_score + 0.1, 0.6)

            # If active ideation AND urgent, boost to near emergency
            elif is_urgent:
        ...
        # Eating disorder modifiers
            ...
            medical_context = re.search(
                r"(?:medical|emergency|hospital|doctor|health)", content, re.IGNORECASE
            )
            if medical_context:
    ...
    def detect_crisis(self, conversation: dict[str, Any]) -> CrisisDetection:
        ...
        crisis_level, confidence_score = self._calculate_crisis_level_production(
            detected_crises, content
        )
    ...
    def _extract_content(self, conversation: dict[str, Any]) -> str:
        ...
        def _extract_from_item(item: Any, key: str = "content") -> None:
            if isinstance(item, str):
                content_parts.append(item)
            elif isinstance(item, dict):
                if key in item:
                    content_parts.append(item[key])
                elif "text" in item:
                    content_parts.append(item["text"])

        # Handle "content" field
        if "content" in conversation:
            val = conversation["content"]
        ...
        for field in ["turns", "messages"]:
            if field in conversation:
                for item in conversation[field]:
                    _extract_from_item(item)
    ...
    def _calculate_crisis_level_production(
        self, detected_crises: dict[str, list[CrisisIndicator]], content: str
        ...
        for crisis_type, indicators in detected_crises.items():
            ...
            adjusted_score = self._apply_production_modifiers(
                crisis_type, max_weight, indicators, content
            )
