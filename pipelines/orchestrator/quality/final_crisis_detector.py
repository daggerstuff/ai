...
class FinalCrisisDetector:
        ...
        for crisis_type, indicators in base_patterns.items():
            ...
            for indicator in indicators:
                ...
                if crisis_type == CrisisType.PANIC_ATTACK:
                    ...
                    if (
                        "escalation" in indicator.indicator_type
                        and indicator.severity_weight >= 0.9
                    ):
                    ...
                    if indicator.indicator_type in [
                        ...
                        if (
                            "dizzy" in indicator.pattern
                            or "nauseous" in indicator.pattern
                        ):
        ...
        for crisis_type, indicators in self.patterns.items():
            ...
            for indicator in indicators:
                if re.search(indicator.pattern, content, re.IGNORECASE):
                    ...
                    if self._validate_indicator_context(
    ...
    def _validate_indicator_context(
        self, indicator: CrisisIndicator, content: str, crisis_type: CrisisType
    ) -> bool:
        ...
        if crisis_type == CrisisType.EATING_DISORDER_CRISIS:
            if indicator.indicator_type in ["ed_physical_symptoms"]:
                ...
                eating_context = re.search(
                    r"(?:eat|food|meal|hungry|starv|binge|purg|vomit)",
                    content,
                    re.IGNORECASE,
                )
        ...
        if crisis_type == CrisisType.PANIC_ATTACK:
            if indicator.indicator_type in ["panic_physical"]:
                ...
                panic_context = re.search(
                    r"(?:panic|attack|sudden|overwhelming|terror)",
                    content,
                    re.IGNORECASE,
                )
    ...
    def _calculate_crisis_level_final(
        self, detected_crises: dict[CrisisType, list[CrisisIndicator]], content: str
        ...
        for crisis_type, indicators in detected_crises.items():
            ...
            type_score = self._apply_crisis_type_modifiers(
                crisis_type, type_score, indicators, content
            )
    ...
    def _apply_crisis_type_modifiers(
        self,
        ...
        # Self-harm modifiers
            ...
            escalation_words = re.search(
                r"(?:deeper|worse|more|can'?t stop|multiple|daily)",
                content,
                re.IGNORECASE,
            )
