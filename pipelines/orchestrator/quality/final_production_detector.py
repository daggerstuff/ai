...
class FinalProductionDetector:
        ...
        return {
            "suicide_ideation": [
                ...
                CrisisIndicator(
                    "method_with_access",
                    0.75,
                    r"(?:have (?:a )?gun|got (?:the )?pills|found (?:a )?rope|access to)",
                ),
                CrisisIndicator(
                    "detailed_plan",
                    0.75,
                ...
                CrisisIndicator(
                    "timeline_urgent",
                    0.70,
                    r"(?:tonight|today|tomorrow|soon|right now)",
                ),
                ...
                CrisisIndicator(
                    "passive_ideation",
                    0.45,
                    r"(?:sometimes think about|wish i was|wouldn't mind if|not being here)",
                ),
                CrisisIndicator(
                    "burden_thoughts",
                    0.50,
                ...
                CrisisIndicator(
                    "emotional_pain",
                    0.40,
                    r"(?:so much (?:emotional )?pain|can'?t handle|can'?t take)",
                ),
            ],
            "self_harm": [
            ...
            "psychotic_episode": [
                ...
                CrisisIndicator(
                    "command_harm",
                    1.0,
                    r"(?:voices (?:are )?telling me to hurt|voices (?:are )?commanding me to|voices tell me to hurt|command me to (?:kill|hurt))",
                    immediate_flag=True,
                ),
                CrisisIndicator(
                    "command_voices",
                    0.85,
                    r"(?:voices tell me to|commanded to|must obey the voices|voices say (?:to|i should))",
                    immediate_flag=True,
                ),
                # High severity patterns
                CrisisIndicator(
            ...
            "panic_attack": [
                ...
                CrisisIndicator(
                    "panic_chest", 0.55, r"(?:chest tight|chest crushing|chest pain)"
                ),
                # Cognitive symptoms
                CrisisIndicator(
                    "panic_death_fear",
            ...
            "eating_disorder_crisis": [
                ...
                CrisisIndicator(
                    ...
                    0.55,
                    r"(?:dizzy|weak|losing hair|teeth problems) (?:from|because|due to) (?:eating|food|restriction|disorder)",
                ),
                CrisisIndicator(
                    "ed_control_loss",
                    0.65,
            ...
            "violence_threat": [
                ...
                CrisisIndicator(
                    "stalking_behavior",
                    0.70,
                    r"(?:following (?:her|him|them)|been following|stalking|know (?:her|his|their) schedule)",
                ),
                CrisisIndicator(
                    "violence_planning",
                    0.75,
                    r"(?:planning something|know where (?:she|he|they) live|waiting for)",
                ),
                CrisisIndicator(
                    "violence_fantasy",
                    0.55,
                ...
                CrisisIndicator(
                    "anger_escalation", 0.45, r"(?:so angry|rage|furious|can't control)"
                ),
            ],
            "substance_overdose": [
                CrisisIndicator(
            ...
            "child_abuse": [
                ...
                CrisisIndicator(
                    "child_neglect", 0.65, r"(?:can't take care|left alone|not feeding)"
                ),
            ],
            "severe_depression": [
                CrisisIndicator(
                ...
                CrisisIndicator(
                    "worthlessness", 0.45, r"(?:worthless|useless|burden to everyone)"
                ),
            ],
        }
    ...
    def detect_crisis(self, conversation: dict[str, Any]) -> CrisisDetection:
        ...
        crisis_level, confidence_score = self._calculate_crisis_level_final(
    ...
    def _calculate_crisis_level_final(
        ...
        for crisis_type, indicators in detected_crises.items():
            ...
            adjusted_score = self._apply_final_modifiers(
    ...
    def _apply_final_modifiers(
        ...
        if crisis_type == "suicide_ideation":
            ...
            if not active_indicators:
                ...
                ind.indicator_type in ["method_with_access", "detailed_plan"]
                for ind in indicators
            ):
        ...
        # Severe depression modifiers - FIXED: Ensure functioning loss gets proper weight
            ...
            if any(ind.indicator_type == "functioning_loss" for ind in indicators):
                base_score = max(
                    base_score, 0.55
                )  # Ensure it reaches elevated threshold
