...
class VoiceDataQualityAssessment:
    ...
    def __init__(
        ...
        output_dir: str = "./quality_assessments",
    ):
    ...
    def assess_voice_data_quality(self, voice_file_path: str) -> VoiceDataAssessment:
        ...
        try:
            ...
            filter_decision, filter_reason = self._make_filter_decision(
                quality_metrics, voice_data
            )
            ...
            logger.info(
                f"Voice data quality assessment completed: {assessment.file_id} - {filter_decision}"
            )
            return assessment
        ...
        return {
            ...
            "transcription": {
                ...
                "speaker_segments": [
                    {
                        "speaker": "client",
                        "text": "I've been having trouble with anxiety lately.",
                        "confidence": 0.92,
                    },
                    {
                        "speaker": "therapist",
                        "text": "Can you tell me more about when you notice these feelings?",
                        "confidence": 0.89,
                    },
                ],
    ...
    def _assess_transcription_quality(
        ...
        speaker_confidence = sum(
            seg.get("confidence", 0.5) for seg in speaker_segments
        ) / max(1, len(speaker_segments))
    ...
    def _assess_conversation_quality(
        ...
        therapeutic_score = min(
            1.0, len(therapeutic_indicators) / 5
        )  # Expect 5+ indicators
    ...
    def _make_filter_decision(
        self, quality_metrics: VoiceQualityMetrics, voice_data: dict[str, Any]
    ) -> tuple[str, str]:
        ...
        silence_ratio = voice_data.get("conversation_data", {}).get(
            "silence_ratio", 1.0
        )
        ...
        if duration < reject_criteria["min_duration"]:
            return (
                "reject",
                f"Duration too short: {duration}s < {reject_criteria['min_duration']}s",
            )
        if duration > reject_criteria["max_duration"]:
            return (
                "reject",
                f"Duration too long: {duration}s > {reject_criteria['max_duration']}s",
            )
        if word_count < reject_criteria["min_word_count"]:
            return (
                "reject",
                f"Word count too low: {word_count} < {reject_criteria['min_word_count']}",
            )
        if speaker_count < reject_criteria["min_speaker_count"]:
            return (
                "reject",
                f"Insufficient speakers: {speaker_count} < {reject_criteria['min_speaker_count']}",
            )
        if silence_ratio > reject_criteria["max_silence_ratio"]:
            return (
                "reject",
                f"Excessive silence: {silence_ratio:.2f} > {reject_criteria['max_silence_ratio']}",
            )
        ...
        if quality_metrics.overall_quality_score >= overall_threshold:
            return (
                "accept",
                f"Quality score {quality_metrics.overall_quality_score:.2f} meets threshold {overall_threshold}",
            )
        ...
        if (
            ...
            >= conditional_criteria["min_overall_quality"]
            and quality_metrics.audio_quality_score
            >= conditional_criteria["min_audio_quality"]
            and quality_metrics.transcription_quality_score
            >= conditional_criteria["min_transcription_quality"]
        ):
            return "conditional", "Meets minimum criteria for conditional acceptance"

        return (
            "reject",
            f"Quality score {quality_metrics.overall_quality_score:.2f} below threshold {overall_threshold}",
        )

    def _calculate_batch_statistics(
        self, assessments: list[VoiceDataAssessment]
    ) -> dict[str, Any]:
    ...
    def _save_batch_assessment_results(
        self, assessments: list[VoiceDataAssessment], batch_stats: dict[str, Any]
    ) -> Path:
        """Save batch assessment results."""
        output_file = (
            self.output_dir
            / f"voice_quality_batch_assessment_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
