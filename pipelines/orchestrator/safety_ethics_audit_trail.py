...
class SafetyEthicsAuditTrail:
    ...
    def log_intervention_required(
        ...
        event = AuditEvent(
            ...
            conversation_id=conversation_id,
            user_id=user_id,
            details={"reason": reason, "urgency_level": urgency_level},
        )
        self._add_audit_event(event)
        return event_id

    def log_manual_review_request(
        self,
        conversation_id: str,
        reviewer_id: str,
        reason: str,
    ...
    def log_manual_review_completion(
        self,
        conversation_id: str,
        reviewer_id: str,
        decision: str,
    ...
    def log_dataset_change(
        ...
        logger.info(
            f"Dataset change logged: {change_id} for conversation {conversation_id}"
        )
        return change_id
    ...
    def log_remediation_action(
        ...
        logger.info(
            f"Remediation action logged: {action_id} for conversation {conversation_id}"
        )
        return action_id
    ...
    def _add_audit_event(self, event: AuditEvent) -> None:
        ...
        logger.info(
            f"Audit event logged: {event.event_type.value} for {event.conversation_id}"
        )
    ...
    def get_audit_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "users_involved": len(
                set(event.user_id for event in self.audit_events if event.user_id)
            ),
            "first_event": first_event.isoformat() if first_event else None,
            "last_event": last_event.isoformat() if last_event else None,
            "conversations_affected": len(
                set(event.conversation_id for event in self.audit_events)
            ),
        }
...
if __name__ == "__main__":
    ...
    audit_trail.log_validation_completion(
        conversation_id, {"final_status": "flagged_for_review", "safety_score": 0.1}
    )
