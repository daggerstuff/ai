...
class AnnotationResult:
    ...
    confidence_adjustment: Optional[float] = (
        None  # Adjustment to confidence after human review
    )
...
class AnnotationQueue:
    ...
    def add_task(self, task: AnnotationTask) -> str:
        ...
        logger.info(
            f"Added annotation task {task.task_id} for conversation {task.conversation_id}"
        )
        return task.task_id

    def get_next_task(
        self, annotator_id: str, min_priority: ReviewPriority = ReviewPriority.HIGH
        ...
        available_tasks = [
            ...
            and ReviewPriority(task.review_priority) >= min_priority
        ]
        ...
        if available_tasks:
            ...
            self.active_annotators[annotator_id] = (
                self.active_annotators.get(annotator_id, 0) + 1
            )
    ...
    def submit_result(self, result: AnnotationResult):
        ...
        if result.task_id in self.tasks:
            ...
            if task.assigned_annotator:
                self.active_annotators[task.assigned_annotator] = max(
                    0, self.active_annotators.get(task.assigned_annotator, 0) - 1
                )

            self.results[result.task_id] = result
            logger.info(
                f"Annotator {result.annotator_id} completed task {result.task_id}"
            )
        else:
            logger.warning(
                f"Attempted to submit result for non-existent task {result.task_id}"
            )
    ...
    def get_annotator_stats(self, annotator_id: str) -> Dict[str, int]:
        ...
        completed_count = len(
            [r for r in self.results.values() if r.annotator_id == annotator_id]
        )
...
class HumanInLoopLabeler:
    ...
    def __init__(
        self, automated_labeler: AutomatedLabeler, confidence_threshold: float = 0.7
    ):
    ...
    def identify_low_confidence_items(self, label_bundle: LabelBundle) -> List[str]:
        ...
        # Check crisis label
        if (
            label_bundle.crisis_label
            and label_bundle.crisis_label.metadata.confidence
        ...
        if (
            label_bundle.therapy_modality_label
            and label_bundle.therapy_modality_label.metadata.confidence
            < self.confidence_threshold
        ):
        ...
        if (
            label_bundle.mental_health_condition_label
            and label_bundle.mental_health_condition_label.metadata.confidence
            < self.confidence_threshold
        ):
        ...
        if (
            label_bundle.demographic_label
            and label_bundle.demographic_label.metadata.confidence
            < self.confidence_threshold
        ):
    ...
    def should_escalate_to_human(
        self, label_bundle: LabelBundle, conversation: Conversation
    ) -> bool:
        ...
        if label_bundle.crisis_label and label_bundle.crisis_label.crisis_level in [
    ...
    def process_conversation_for_human_review(
        ...
        if self.should_escalate_to_human(automated_labels, conversation):
            ...
            if any(
                label.metadata.confidence < 0.5
                for label in automated_labels.therapeutic_response_labels
            ):
                priority = ReviewPriority.HIGH
            elif (
                automated_labels.crisis_label
                and automated_labels.crisis_label.metadata.confidence < 0.8
            ):
                priority = ReviewPriority.HIGH
            ...
            if automated_labels.crisis_label:
                notes.append(
                    f"Crisis level: {automated_labels.crisis_label.crisis_level.value}"
                )
            ...
            self.logger.info(
                f"Created annotation task {task_id} for conversation {conversation.conversation_id}"
            )
            return task_id
    ...
    def integrate_human_labels(
        self, result: AnnotationResult, original_bundle: LabelBundle
    ) -> LabelBundle:
        ...
        if (
            updated_bundle.crisis_label
            and updated_bundle.crisis_label.metadata.provenance
            == LabelProvenanceType.AUTOMATED_MODEL
        ):
            updated_bundle.crisis_label.metadata.provenance = (
                LabelProvenanceType.COMBINED_MODEL_HUMAN
            )

        if (
            updated_bundle.therapy_modality_label
            and updated_bundle.therapy_modality_label.metadata.provenance
            == LabelProvenanceType.AUTOMATED_MODEL
        ):
            updated_bundle.therapy_modality_label.metadata.provenance = (
                LabelProvenanceType.COMBINED_MODEL_HUMAN
            )

        if (
            updated_bundle.mental_health_condition_label
            and updated_bundle.mental_health_condition_label.metadata.provenance
            == LabelProvenanceType.AUTOMATED_MODEL
        ):
            updated_bundle.mental_health_condition_label.metadata.provenance = (
                LabelProvenanceType.COMBINED_MODEL_HUMAN
            )

        if (
            updated_bundle.demographic_label
            and updated_bundle.demographic_label.metadata.provenance
            == LabelProvenanceType.AUTOMATED_MODEL
        ):
            updated_bundle.demographic_label.metadata.provenance = (
                LabelProvenanceType.COMBINED_MODEL_HUMAN
            )
    ...
    def get_review_statistics(self) -> Dict[str, Any]:
        ...
        pending_tasks = len(
            [t for t in self.annotation_queue.tasks.values() if t.status == "pending"]
        )
        in_progress_tasks = len(
            [
                t
                for t in self.annotation_queue.tasks.values()
    ...
    def submit_annotation_result(self, result_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if task_id in self.human_in_loop.annotation_queue.tasks:
                ...
                integrated_labels = self.human_in_loop.integrate_human_labels(
                    result, original_task.original_labels
                )
    ...
    def _bundle_to_dict(self, bundle: LabelBundle) -> Dict[str, Any]:
        ...
        return {
            ...
            "mental_health_condition_label": (
                {
                    "conditions": (
                        [
                            c.value
                            for c in bundle.mental_health_condition_label.conditions
                        ]
                        if bundle.mental_health_condition_label
                        else []
                    ),
                    "severity": (
                        bundle.mental_health_condition_label.severity
                        if bundle.mental_health_condition_label
                        else None
                    ),
                    "primary_condition": (
                        bundle.mental_health_condition_label.primary_condition.value
                        if bundle.mental_health_condition_label
                        and bundle.mental_health_condition_label.primary_condition
                        else None
                    ),
                    "co_morbidities": (
                        [
                            c.value
                            for c in bundle.mental_health_condition_label.co_morbidities
                        ]
                        if bundle.mental_health_condition_label
                        else []
                    ),
                    "metadata": {
                        "confidence": (
                            bundle.mental_health_condition_label.metadata.confidence
                            if bundle.mental_health_condition_label
                            else None
                        ),
                        "confidence_explanation": (
                            bundle.mental_health_condition_label.metadata.confidence_explanation
                            if bundle.mental_health_condition_label
                            else None
                        ),
                        "provenance": (
                            bundle.mental_health_condition_label.metadata.provenance.value
                            if bundle.mental_health_condition_label
                            else None
                        ),
                        "annotator_id": (
                            bundle.mental_health_condition_label.metadata.annotator_id
                            if bundle.mental_health_condition_label
                            else None
                        ),
                        "model_name": (
                            bundle.mental_health_condition_label.metadata.model_name
                            if bundle.mental_health_condition_label
                            else None
                        ),
                        "model_version": (
                            bundle.mental_health_condition_label.metadata.model_version
                            if bundle.mental_health_condition_label
                            else None
                        ),
                    },
            ...
            "demographic_label": (
                {
                    "demographics": (
                        [d.value for d in bundle.demographic_label.demographics]
                        if bundle.demographic_label
                        else []
                    ),
                    "estimated_accuracy": (
                        bundle.demographic_label.estimated_accuracy
                        if bundle.demographic_label
                        else None
                    ),
                    "metadata": {
                        "confidence": (
                            bundle.demographic_label.metadata.confidence
                            if bundle.demographic_label
                            else None
                        ),
                        "confidence_explanation": (
                            bundle.demographic_label.metadata.confidence_explanation
                            if bundle.demographic_label
                            else None
                        ),
                        "provenance": (
                            bundle.demographic_label.metadata.provenance.value
                            if bundle.demographic_label
                            else None
                        ),
                        "annotator_id": (
                            bundle.demographic_label.metadata.annotator_id
                            if bundle.demographic_label
                            else None
                        ),
                        "model_name": (
                            bundle.demographic_label.metadata.model_name
                            if bundle.demographic_label
                            else None
                        ),
                        "model_version": (
                            bundle.demographic_label.metadata.model_version
                            if bundle.demographic_label
                            else None
                        ),
                    },
    ...
    def _dict_to_bundle(self, data: Dict[str, Any]) -> LabelBundle:
        ...
        for label_data in data.get("therapeutic_response_labels", []):
            label = TherapeuticResponseLabel(
                ...
                metadata=LabelMetadata(
                    ...
                    confidence_explanation=label_data["metadata"][
                        "confidence_explanation"
                    ],
                    provenance=LabelProvenanceType(
                        label_data["metadata"]["provenance"]
                    ),
        ...
        if data.get("crisis_label"):
            ...
            crisis_label = CrisisLabel(
                ...
                estimated_risk_probability=crisis_data.get(
                ...
                metadata=LabelMetadata(
                    ...
                    confidence_explanation=crisis_data["metadata"][
                        "confidence_explanation"
                    ],
                    provenance=LabelProvenanceType(
                        crisis_data["metadata"]["provenance"]
                    ),
        ...
        if data.get("therapy_modality_label"):
            ...
            modality_label = TherapyModalityLabel(
                ...
                modality_specific_techniques=modality_data.get(
                    "modality_specific_techniques", []
                ),
                modality_adherence_score=modality_data.get("modality_adherence_score"),
                metadata=LabelMetadata(
                    confidence=modality_data["metadata"]["confidence"],
                    confidence_explanation=modality_data["metadata"][
                        "confidence_explanation"
                    ],
                    provenance=LabelProvenanceType(
                        modality_data["metadata"]["provenance"]
                    ),
        ...
        if data.get("mental_health_condition_label"):
            ...
            condition_label = MentalHealthConditionLabel(
                conditions=[
                    MentalHealthConditionType(c)
                    for c in condition_data.get("conditions", [])
                ],
                severity=condition_data.get("severity"),
                primary_condition=(
                    MentalHealthConditionType(condition_data["primary_condition"])
                    if condition_data.get("primary_condition")
                    else None
                ),
                co_morbidities=[
                    MentalHealthConditionType(c)
                    for c in condition_data.get("co_morbidities", [])
                ],
                metadata=LabelMetadata(
                    confidence=condition_data["metadata"]["confidence"],
                    confidence_explanation=condition_data["metadata"][
                        "confidence_explanation"
                    ],
                    provenance=LabelProvenanceType(
                        condition_data["metadata"]["provenance"]
                    ),
        ...
        if data.get("demographic_label"):
            ...
            demographic_label = DemographicLabel(
                demographics=[
                    DemographicType(d) for d in demographic_data.get("demographics", [])
                ],
                estimated_accuracy=demographic_data.get("estimated_accuracy"),
                metadata=LabelMetadata(
                    confidence=demographic_data["metadata"]["confidence"],
                    confidence_explanation=demographic_data["metadata"][
                        "confidence_explanation"
                    ],
                    provenance=LabelProvenanceType(
                        demographic_data["metadata"]["provenance"]
                    ),
...
def test_human_in_loop():
    ...
    conversation.add_message(
        "client",
        "I've been really down lately and having thoughts about not wanting to be here anymore.",
    )
    conversation.add_message(
        "therapist",
        "I'm sorry to hear you're feeling this way. Can you tell me more about these thoughts?",
    )
    ...
    if task_id:
        ...
        if task:
            ...
            updated_bundle = human_loop.automated_labeler.label_conversation(
                conversation
            )
