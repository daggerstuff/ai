...
class SafetyFilter:
    ...
    def _check_text_safety(self, text: str) -> SafetyScore:
        ...
        if not text or not isinstance(text, str):
            return SafetyScore(
                overall_score=1.0, category_scores={}, flagged_categories=[]
            )
    ...
    def _check_harm_content(self, text: str) -> float:
        """Check for harmful content"""
        harm_keywords = [
            kw for kw in self.crisis_keywords if "harm" in kw or "kill" in kw
        ]
        ...
        for key, value in response_dict.items():
            if isinstance(value, str):
                ...
                if safety_score.overall_score < 0.5:
                    ...
                    filtered_dict[key] = self._apply_redactions(
                        value, safety_score.flagged_categories
                    )
                        ...
                        if safety_score.overall_score < 0.5:
                            ...
                            filtered_list.append(
                                self._apply_redactions(
                                    item, safety_score.flagged_categories
                                )
                            )
                    else:
    ...
    def _apply_redactions(
        self, text: str, flagged_categories: List[SafetyCategory]
    ) -> str:
            ...
            if "content" in message:
                ...
                if safety_score.overall_score < 0.3:  # Very unsafe
                    ...
                    moderated_message["content"] = self._apply_redactions(
                        message["content"], safety_score.flagged_categories
                    )
                # Add safety metadata
                moderated_message["safety_score"] = safety_score.overall_score
                moderated_message["flagged_categories"] = [
                    cat.value for cat in safety_score.flagged_categories
                ]
...
def test_safety_filter():
    ...
    for i, text in enumerate(test_cases):
        ...
        print(
            f"Flagged categories: {[cat.value for cat in safety_score.flagged_categories]}"
        )

        # Check for crisis specifically
        crisis_result = check_crisis_content(text)
        print(
            f"Crisis detected: {crisis_result['is_crisis']}, Types: {crisis_result['crisis_types']}"
        )
