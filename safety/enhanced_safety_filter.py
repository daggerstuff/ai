...
class EnhancedSafetyFilter:
    ...
    def _init_safety_models(self):
        ...
        try:
            ...
            self.bias_classifier = pipeline(
                "text-classification",
                model=bias_model if cuda_available else None,
                tokenizer=bias_model if cuda_available else None,
            )
    ...
    def _initialize_privacy_patterns(self) -> Dict[str, str]:
        ...
        return {
            ...
            "medical_record": r"\b[A-Z]{2,3}\d{2,}\b",  # Simplified medical record
        }
    ...
    def check_output_safety(
        ...
        category_scores[SafetyCategory.CRISIS] = (
            crisis_result.confidence if crisis_result.is_crisis else 0.0
        )
        ...
        if inappropriate_score > self._get_threshold_for_category(
        ...
        if flagged_categories:
            explanation_parts.append(
                f"Flagged categories: {[cat.value for cat in flagged_categories]}"
            )
            max_score_cat = max(category_scores.items(), key=lambda x: x[1])
            explanation_parts.append(
                f"Highest risk: {max_score_cat[0].value} ({max_score_cat[1]:.2f})"
            )
        else:
        ...
        self.logger.debug(
            f"Safety check completed in {processing_time:.2f}ms - Safe: {is_safe}"
        )
            ...
            for pattern in patterns:
                if re.search(pattern, content_lower, re.IGNORECASE):
                    ...
                    return CrisisDetectionResult(
                        ...
                        confidence=min(
                            1.0,
                            len(re.findall("|".join(patterns), content_lower)) * 0.4,
                        ),
    ...
    def _determine_crisis_urgency(self, content: str, crisis_type: str) -> str:
        ...
        immediate_matches = sum(
            1
            for pattern in urgency_indicators["immediate"]
        ...
        high_matches = sum(
            1
            for pattern in urgency_indicators["high"]
            if re.search(pattern, content, re.IGNORECASE)
        )
        if high_matches > 0:
            return "high"
        ...
        medium_matches = sum(
            1
            for pattern in urgency_indicators["medium"]
            if re.search(pattern, content, re.IGNORECASE)
        )
        if medium_matches > 0:
            return "medium"
    ...
    def _check_toxicity_content(self, content: str) -> float:
        ...
        if self.toxicity_classifier:
            try:
                result = self.toxicity_classifier(content[:512])  # Limit length
        ...
        toxic_matches = sum(
            1
            for pattern in self.harm_patterns["physical_harm"]
    ...
    def _check_bias_content(self, content: str) -> float:
        ...
        bias_matches = sum(
            1 for pattern in bias_keywords if re.search(pattern, content.lower())
        )
    ...
    def _check_legal_medical_advisory(self, content: str) -> float:
        ...
        advisory_matches = sum(
            1 for pattern in advisory_patterns if re.search(pattern, content.lower())
        )
    ...
    def _redact_content(self, content: str, redactions: List[Dict[str, Any]]) -> str:
        ...
        sorted_redactions = sorted(
            redactions, key=lambda x: x["start_position"], reverse=True
        )
    ...
    def _calculate_assessment_confidence(
        self, category_scores: Dict[SafetyCategory, float]
    ) -> float:
    ...
    def filter_response(
        ...
        """
        Filter a response to ensure safety, returning
        (is_safe, filtered_response, safety_result)
        """
...
class CrisisInterventionSystem:
    ...
    def _handle_suicide_crisis(
        ...
        # (in production, this would notify crisis teams)
    ...
    def _handle_harm_to_others_crisis(
        ...
        response_content = {
            "message": (
                "I'm concerned about safety. It's important to seek help immediately."
            ),
            "resources": crisis_result.resources
            or ["Emergency Services: 911", "Local Crisis Center Hotline"],
            "recommended_action": crisis_result.recommended_action,
            "urgency_level": crisis_result.urgency_level,
        }
    ...
    def _handle_substance_abuse_crisis(
        ...
        response_content = {
            "message": (
                "Substance use concerns can be challenging. Support is available."
            ),
            "resources": [
    ...
    def _handle_eating_disorder_crisis(
        ...
        response_content = {
            "message": (
                "Concerns about eating patterns can be addressed "
    ...
    def _trigger_external_alert(self, crisis_log: Dict[str, Any]):
        ...
        self.logger.info(
            f"External alert triggered for crisis: {crisis_log.get('crisis_type')}"
        )
...
def integrate_safety_filtering_with_api():
    ...
    def safety_wrapper(endpoint_func):
        def wrapped(*args, **kwargs):
            ...
            if hasattr(response, "content") or (
...
def test_enhanced_safety_filter():
    ...
    test_cases = [
        ...
        (
            "I found some explicit adult content online that was really graphic.",
        ...
        (
            (
                "You should definitely sue your employer for that. "
                "I recommend this specific lawyer."
            ),
            "legal_advice",
        ),
        # Mixed content with multiple issues
        (
            "I'm thinking about overdosing on my medication tonight. "
    ...
    for content, test_type in test_cases:
        ...
        if safety_result.flagged_categories:
            print(
                f"Flagged Categories: "
                f"{[cat.value for cat in safety_result.flagged_categories]}"
            )
        ...
        if safety_result.filtered_content:
            print(
                f"Filtered Content: "
                f"{safety_result.filtered_content[:50]}"
                f"{'...' if len(safety_result.filtered_content) > 50 else ''}"
            )
        ...
        if not is_safe:
            print(
                f"Final Filtered Content: {filtered_content[:50]}"
                f"{'...' if len(filtered_content) > 50 else ''}"
            )

        # Test crisis detection specifically
        if "crisis" in test_type or any(
    ...
    for i, (is_safe, f_content, s_result) in enumerate(batch_results):
        print(f"Batch Item {i + 1}: Safe={is_safe}, Score={s_result.overall_score:.3f}")
    ...
    for safety_level in [
        ...
        print(
            f"{safety_level.value}: Safe={result.is_safe}, "
