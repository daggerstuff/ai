...
class ManualReviewSystem:
    ...
    def create_review_assignment(
        ...
        logger.info(
            f"✅ Created review assignment {assignment_id} for reviewer {reviewer_id}"
        )
        return assignment_id

    def _auto_assign_reviewer(
        self, conversation: Dict[str, Any], priority: ReviewPriority
    ) -> str:
        ...
        for reviewer_id, reviewer in self.reviewers.items():
            ...
            current_assignments = sum(
                ...
                and a.status in [ReviewStatus.PENDING, ReviewStatus.IN_PROGRESS]
            )
    ...
    def _estimate_review_time(
        self, conversation: Dict[str, Any], criteria: List[ReviewCriteria]
    ) -> int:
        ...
        complexity_time = min(
            text_length / 1000 * 5, 30
        )  # 5 min per 1000 chars, max 30
        ...
        for criteria in assignment.criteria:
            if criteria.criteria_id not in criteria_scores:
                if criteria.required:
                    raise ValueError(
                        f"Missing required criteria score: {criteria.criteria_id}"
                    )
                else:
    ...
    def _update_review_stats(self, review: ReviewResult):
        ...
        self.review_stats["pending_reviews"] = max(
            0, self.review_stats["pending_reviews"] - 1
        )
        ...
        if reviewer_id in self.review_stats["reviewer_performance"]:
            ...
            perf["average_score_given"] = (
                (current_avg * (total_reviews - 1)) + review.overall_score
            ) / total_reviews

            # Update average review time
            current_time_avg = perf["average_review_time"]
            perf["average_review_time"] = (
                (current_time_avg * (total_reviews - 1)) + review.review_time_minutes
            ) / total_reviews
        ...
        self.review_stats["average_review_time"] = (
            current_avg_time * (total_completed - 1) + review.review_time_minutes
        ) / total_completed
    ...
    def get_review_results(
        self, conversation_id: str = None, reviewer_id: str = None
    ) -> List[ReviewResult]:
    ...
    def generate_review_report(
        ...
        if start_date or end_date:
            ...
            for review in reviews:
                review_date = datetime.fromisoformat(
                    review.completed_at.replace("Z", "+00:00")
                )
        ...
        return {
            ...
            "quality_trends": self.review_stats["quality_trends"][
                -50:
            ],  # Last 50 reviews
    ...
    def get_review_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_reviewers": len(
                [r for r in self.reviewers.values() if r.get("active", True)]
            ),
    ...
    def export_review_data(
        self, output_path: str, include_sensitive: bool = False
    ) -> bool:
...
def main():
    ...
    assignment_id = review_system.create_review_assignment(
        conversation=test_conversation, priority=ReviewPriority.HIGH
    )
