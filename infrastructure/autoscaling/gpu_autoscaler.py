...
class ScalingPolicy:
    ...
    scale_up_threshold: float = (
        70.0  # CPU/GPU utilization percentage to trigger scale-up
    )
    scale_down_threshold: float = (
        30.0  # CPU/GPU utilization percentage to trigger scale-down
    )
    scale_up_factor: float = (
        1.5  # Multiply current instances by this factor when scaling up
    )
    scale_down_factor: float = (
        0.7  # Multiply current instances by this factor when scaling down
    )
...
class GPUCostModel:
    ...
    INSTANCE_COSTS = {
        InstanceType.CPU_SMALL: CostModel(
            hourly_rate=0.05, startup_cost=0.0, idle_cost_multiplier=0.1
        ),
        InstanceType.CPU_MEDIUM: CostModel(
            hourly_rate=0.15, startup_cost=0.0, idle_cost_multiplier=0.1
        ),
        InstanceType.CPU_LARGE: CostModel(
            hourly_rate=0.30, startup_cost=0.0, idle_cost_multiplier=0.1
        ),
        InstanceType.GPU_T4: CostModel(
            hourly_rate=0.526,
            startup_cost=0.5,
            shutdown_cost=0.1,
            idle_cost_multiplier=0.3,
        ),
...
class Autoscaler:
    ...
    def __init__(
        ...
        self.last_scaling_action = datetime.utcnow() - timedelta(
            minutes=self.scaling_policy.cooldown_period_minutes
        )
    ...
    def make_scaling_decision(
        ...
        incoming_requests: Optional[int] = None,
    ) -> ScalingDecision:
        ...
        if self.scaling_strategy in [
            ...
            if (
                predicted_load > utilization_metric * 1.2
            ):  # Predicted load is 20% higher than current
                # Adjust scaling decision upwards
                target_instances = min(
                    int(target_instances * 1.2), self.scaling_policy.max_instances
                )
                decision.target_instances = target_instances
                decision.reason += (
                    f" | Predictive adjustment for expected load increase"
                )
    ...
    def _identify_cost_drivers(
        self, utilization: float, hourly_base_cost: float
    ) -> List[Dict[str, Any]]:
        ...
        drivers.append(
            {
                ...
                "impact": (
                    ...
                    in [
                        InstanceType.GPU_A100,
                        InstanceType.GPU_H100,
                        InstanceType.MULTI_GPU,
                    ]
                    else "medium"
                ),
                "description": f"Using {self.instance_type.value} instances",
                "savings_potential": (
                    hourly_base_cost * 0.3
                    if self.instance_type
                    in [
                        InstanceType.GPU_A100,
                        InstanceType.GPU_H100,
    ...
    def optimize_instance_allocation(self) -> Dict[str, Any]:
        ...
        if avg_utilization < 30:
            ...
            recommendation["recommended_instances"] = max(
                1, int(self.current_instances * 0.7)
            )
            recommendation["reason"] = (
                f"Average utilization {avg_utilization:.1f}% is low"
            )
            ...
            recommendation["reason"] = (
                f"Average utilization {avg_utilization:.1f}% is high"
            )
        else:
            recommendation["recommendation"] = "maintain"
            recommendation["recommended_instances"] = self.current_instances
            recommendation["reason"] = (
                f"Utilization {avg_utilization:.1f}% is within optimal range"
            )
...
class RequestForecaster:
    ...
    def predict_future_load(
        self, usage_history: List[ResourceUsage], horizon_minutes: int = 30
    ) -> float:
        ...
        if len(recent_utils) > 1:
            ...
            forecast = smoothed + (trend / len(recent_utils)) * (
                horizon_minutes / 5
...
class CostForecaster:
    ...
    def forecast_costs(
        self, autoscaler: Autoscaler, hours_ahead: int = 24
    ) -> CostForecast:
    ...
    def identify_optimization_opportunities(
        ...
        if utilization < 30 and autoscaler.instance_type not in [
    ...
    def make_all_scaling_decisions(self) -> Dict[str, ScalingDecision]:
        ...
        for model_name, autoscaler in self.autoscalers.items():
            try:
                ...
                self.logger.error(
                    f"Failed to make scaling decision for {model_name}: {e}"
                )
        return decisions
    ...
    def get_total_cost_forecast(self, hours_ahead: int = 24) -> CostForecast:
        ...
        for autoscaler in self.autoscalers.values():
            try:
                ...
                self.logger.error(
                    f"Failed to get cost forecast for {autoscaler.model_name}: {e}"
                )
...
def register_model_for_autoscaling(
    ...
    """Register a model for autoscaling"""
    return multi_model_autoscaler.register_model(
        model_name, instance_type, scaling_policy
    )
...
def test_autoscaling_system():
    ...
    autoscaler = register_model_for_autoscaling(
        model_name, instance_type=InstanceType.GPU_T4, scaling_policy=policy
    )
