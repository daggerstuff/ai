...
class ResumeOrchestrator:
    ...
    def register_resumable_process(
        self,
        process_id: str,
        task_id: str,
    ...
    async def resume_process_with_dependencies(
        self, process_id: str, interruption_context: InterruptionContext = None
    ) -> bool:
            ...
            if dependency.dependent_process == process_id:
                ...
                if dependency_state:
                    ...
                    if completion_percent < dependency.required_completion_percent:
                        logger.debug(
                            f"Dependency {dependency.dependency_process} only {completion_percent:.1f}% complete"
                        )
                        return False
                else:
    ...
    def _check_dependency_updates(self):
        ...
        for process_id in list(self.registered_processes.keys()):
            if process_id not in self.active_resumes and process_id not in [
