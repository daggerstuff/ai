...
class BusinessDocumentRepository(MongoBaseRepository):
    ...
    async def find_by_owner(
        ...
        return await self.list_by_field(
            "owner", owner_id, sort=[("updatedAt", -1)], skip=skip, limit=limit
        )

    async def find_by_type(
        self, doc_type: str, skip: int = 0, limit: int = 100
    ) -> list[dict[str, Any]]:
        return await self.list_by_field(
            "type", doc_type, sort=[("updatedAt", -1)], skip=skip, limit=limit
        )
    ...
    async def add_revision(
        self, document_id: str, revision: dict[str, Any]
    ) -> dict[str, Any] | None:
...
class MarketResearchRepository(MongoBaseRepository):
    ...
    async def find_by_type(
        self, research_type: str, skip: int = 0, limit: int = 100
    ) -> list[dict[str, Any]]:
        return await self.find_many(
...
class StrategicPlanRepository(MongoBaseRepository):
    ...
    async def find_by_fiscal_year(
        self, fiscal_year: int, skip: int = 0, limit: int = 100
    ) -> list[dict[str, Any]]:
        return await self.list_by_field(
            "fiscalYear", fiscal_year, sort=[("quarter", 1)], skip=skip, limit=limit
        )
...
class SalesOpportunityRepository(MongoBaseRepository):
    ...
    async def find_by_stage(
        self, stage: str, skip: int = 0, limit: int = 100
    ) -> list[dict[str, Any]]:
        return await self.find_many(
    ...
    async def find_by_owner(
        ...
        return await self.list_by_field(
            "owner", owner_id, sort=[("expectedCloseDate", 1)], skip=skip, limit=limit
        )
...
class KnowledgeArticleRepository(MongoBaseRepository):
    ...
    async def find_by_category(
        ...
        return await self.list_by_field(
            "category", category, sort=[("publishedDate", -1)], skip=skip, limit=limit
        )

    async def increment_views(self, article_id: str) -> None:
        """Atomically increment view count."""
        await self.collection.update_one(
            {"articleId": article_id}, {"$inc": {"views": 1}}
        )
