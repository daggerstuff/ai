...
class TrainingDataSelector:
    ...
    def select_data(
        ...
        if profile_name not in PROFILE_CONFIGS:
            raise ValueError(
                f"Unknown profile: {profile_name}. Available profiles: {', '.join(PROFILE_CONFIGS.keys())}"
            )
...
def get_profile_config(profile_name: str) -> ProfileConfig:
    ...
    if profile_name not in PROFILE_CONFIGS:
        raise ValueError(f"Unknown profile: {profile_name}. Available: {', '.join(PROFILE_CONFIGS.keys())}")
...
def validate_profile_config(profile_name: str) -> tuple[bool, str | None]:
    """
