...
class ValidationReport:
    ...
    def add_error(self, message: str, field: str = None, value: Any = None, suggestion: str = None):
        ...
        self.results.append(
            ValidationResult(
                level=ValidationLevel.ERROR, message=message, field=field, value=value, suggestion=suggestion
            )
        )
    ...
    def add_warning(self, message: str, field: str = None, value: Any = None, suggestion: str = None):
        ...
        self.results.append(
            ValidationResult(
                level=ValidationLevel.WARNING, message=message, field=field, value=value, suggestion=suggestion
            )
        )
...
class ConfigValidator:
    ...
    def _validate_redis_url(self, value: str, field: str):
        ...
        try:
            ...
            if not parsed.scheme:
                self.report.add_error(
                    "Redis URL missing scheme", field=field, suggestion="Use format: redis://[:password@]host:port[/db]"
                )
    ...
    def _validate_encryption_key(self, value: str, field: str):
        ...
        if len(value) < 32:
            self.report.add_error(
                "Encryption key too short (minimum 32 characters)", field=field, value=f"Length: {len(value)}"
            )
    ...
    def _validate_log_level(self, value: str, field: str):
        ...
        if value.upper() not in valid_levels:
            self.report.add_error(
                f"Invalid log level: {value}", field=field, suggestion=f"Use one of: {', '.join(valid_levels)}"
            )
    ...
    def _validate_debug_flag(self, value: str, field: str):
        ...
        if value.lower() in ["true", "1", "yes"]:
            ...
            if env in ["production", "prod"]:
                self.report.add_warning(
                    "Debug enabled in production environment", field=field, suggestion="Disable debug in production"
                )
    ...
    def _validate_sentry_dsn(self, value: str, field: str):
        ...
        try:
            ...
            if not parsed.scheme or not parsed.hostname:
                self.report.add_error(
                    "Invalid Sentry DSN format", field=field, suggestion="Check Sentry project settings for correct DSN"
                )
        except Exception as e:
            self.report.add_error(f"Invalid Sentry DSN: {e}", field=field)
    ...
    def _validate_file_permissions(self):
        ...
        sensitive_files = [".env", "secrets.yaml", "private.key", "ssl.key"]
...
def main():
    ...
    parser.add_argument("--fail-on-warnings", action="store_true", help="Exit with error code if warnings are found")
