...
try: import boto3; from botocore.config import Config
...
class S3DatasetLoader:
    ...
    def s3_client(self):
        with self._client_lock:
            if self._s3_client is None:
                ...
                self._s3_client = boto3.client("s3", endpoint_url=self.endpoint_url, aws_access_key_id=self.access_key, aws_secret_access_key=self.secret_key, config=s_config)
    ...
    def stream_jsonl(self, s3_path: str, byte_offset: int = 0) -> Iterator[tuple[dict[str, Any], int]]:
        ...
        if byte_offset > 0: get_kwargs["Range"] = f"bytes={byte_offset}-"
        try:
            ...
            for chunk in body:
                ...
                while True:
                    ...
                    line_len = (idx - ptr) + 1
                    if line.strip():
                        try: yield json.loads(line.decode("utf-8", errors="strict")), current_pos + line_len
                        except Exception as e: logger.error(f"JSON Skipped in {key}: {e}")
                    current_pos += line_len
                    ptr = idx + 1
                if ptr > 10 * 1024 * 1024:
                    buffer = buffer[ptr:]; ptr = 0
            if buffer[ptr:].strip():
                try: yield json.loads(buffer[ptr:].decode("utf-8")), current_pos + len(buffer[ptr:])
                except: pass
        except Exception as e: logger.error(f"S3 Fatal in {key}: {e}"); raise
