...
class EnhancedCrisisDetectorV5:
    ...
    def __init__(self):
        ...
        self.semantic_context_analyzers = {
            "temporal_progression": {
                "immediate": [
                    r"\b(?:right now|immediately|urgent|emergency|asap|tonight|today)\b"
                ],
            ...
            "help_seeking_spectrum": {
                "direct_help": [
                    r"\b(?:need help|please help|someone help|help me|save me)\b"
                ],
                "indirect_help": [
                    r"\b(?:what (?:should|can) I do|don\'t know what to do|lost)\b"
                ],
                "professional_help": [
                    r"\b(?:therapist|counselor|doctor|professional|crisis line)\b"
                ],
                "ambivalent_help": [
                    r"\b(?:maybe|perhaps|might).*(?:help|support|therapy)\b"
                ],
            },
        ...
        self.advanced_linguistic_features = {
            ...
            "cognitive_distortions": {
                ...
                "catastrophizing": [
                    r"\b(?:disaster|catastrophe|end of the world|worst thing)\b"
                ],
                "personalization": [
                    r"\b(?:my fault|because of me|I caused|I\'m responsible)\b"
                ],
    ...
    def extract_semantic_embeddings(self, text: str) -> Dict[str, float]:
        ...
        embeddings["contextual_relationships"] = len(crisis_contexts) / max(
            len(words) / 10, 1
        )
    ...
    def calculate_linguistic_complexity(self, text: str) -> Dict[str, float]:
        ...
        cognitive_markers = 0
        for distortion_type, patterns in self.advanced_linguistic_features[
            "cognitive_distortions"
        ].items():
            for pattern in patterns:
                cognitive_markers += len(re.findall(pattern, text, re.IGNORECASE))

        complexity["cognitive_distortion_density"] = cognitive_markers / max(
            len(words), 1
        )
    ...
    def multi_model_ensemble_analysis(self, text: str) -> Dict[str, float]:
        ...
        for tier, categories in self.semantic_crisis_patterns.items():
            tier_weight = (
                1.0 if "immediate" in tier else (0.9 if "severe" in tier else 0.8)
            )
        ...
        for analyzer_name, patterns in self.semantic_context_analyzers[
            "protective_factors_complex"
        ].items():
            for pattern in patterns:
    ...
    def calculate_ensemble_consensus(
    ...
    def detect_crisis(self, text: str) -> Dict:
        ...
        consensus_score, consensus_confidence = self.calculate_ensemble_consensus(
            ensemble_scores
        )
        ...
        context = SemanticCrisisContext(
            ...
            contextual_relationships=[
                (tier, cat, score)
                for tier, cat, score in [("pattern", "match", consensus_score)]
            ],
            emotional_trajectory=emotional_trajectory,
            linguistic_complexity=linguistic_complexity,
            crisis_probability_distribution=ensemble_scores,
            confidence_intervals={
                "consensus": (consensus_confidence * 0.8, consensus_confidence * 1.2)
            },
        ...
        reasoning_parts.append(
            f"Consensus: {consensus_score:.2f} (confidence: {consensus_confidence:.2f})"
        )
        ...
        reasoning = (
            ...
            if reasoning_parts
            else "No significant crisis indicators detected"
        )
    ...
    def _create_result(
        self, is_crisis: bool, confidence: float, reasoning: str, details: Dict
    ) -> Dict:
...
if __name__ == "__main__":
    ...
    for i, text in enumerate(test_cases, 1):
        ...
        print(
            f"Level: {result['details']['confidence_level']} | Final Score: {result['details']['final_score']:.3f}"
        )
        print(
            f"Consensus: {result['details']['consensus_score']:.3f} (conf: {result['details']['consensus_confidence']:.3f})"
        )

        # Show ensemble model scores
        ensemble = result["details"]["ensemble_scores"]
        print(
            f"Models: Pattern={ensemble['pattern_matcher']:.2f}, Semantic={ensemble['semantic_analyzer']:.2f}, "
