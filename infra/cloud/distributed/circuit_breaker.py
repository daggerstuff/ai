...
class CircuitBreaker:
    ...
    async def _handle_half_open_circuit(self, func: Callable, *args, **kwargs) -> Any:
        ...
        logger.info(
            f"Circuit breaker {self.name} is HALF-OPEN - testing service recovery"
        )
    ...
    def _should_open_circuit(self) -> bool:
        ...
        if len(recent_calls) >= 10:  # Minimum calls for rate calculation
            failure_rate = sum(1 for call in recent_calls if not call.success) / len(
                recent_calls
            )
    ...
    def _should_attempt_reset(self) -> bool:
        ...
        return (
            time_since_failure.total_seconds() >= self.config.recovery_timeout_seconds
        )
    ...
    def _record_call_result(self, call_result: CallResult):
        ...
        with self.lock:
            ...
            cutoff_time = datetime.utcnow() - timedelta(
                seconds=self.config.monitoring_window_seconds * 2
            )
            self.call_history = [
    ...
    for i in range(20):
        try:
            result = await cb_manager.call_with_circuit_breaker(
                "external_api", unreliable_service
            )
