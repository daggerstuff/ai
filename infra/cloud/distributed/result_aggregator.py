...
class ResultAggregator:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_batch_id ON validation_results(batch_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_timestamp ON validation_results(timestamp)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_worker_id ON validation_results(worker_id)"
            )
    ...
    def aggregate_results(
        self, batch_id: str, force_refresh: bool = False
    ...
    def _calculate_processing_time_stats(
        self, results: List[Dict[str, Any]]
    ) -> Dict[str, float]:
        """Calculate processing time statistics"""
        processing_times = [
            r.get("processing_time", 0.0) for r in results if r.get("processing_time")
        ]
        ...
        return {
            ...
            "std_dev": (
                statistics.stdev(processing_times) if len(processing_times) > 1 else 0.0
            ),
    ...
    def _analyze_worker_performance(
        self, results: List[Dict[str, Any]]
    ) -> Dict[str, Dict[str, float]]:
        ...
        for result in results:
            ...
            if result.get("success", False):
                ...
                worker_data[worker_id]["quality_scores"].append(
                    result.get("quality_score", 0.0)
                )
        ...
        for worker_id, data in worker_data.items():
            ...
            worker_stats[worker_id]["success_rate"] = (
                data["successful_tasks"] / data["total_tasks"]
                if data["total_tasks"] > 0
                else 0.0
            )

            if data["quality_scores"]:
                worker_stats[worker_id]["avg_quality_score"] = statistics.mean(
                    data["quality_scores"]
                )

            if data["processing_times"]:
                worker_stats[worker_id]["avg_processing_time"] = statistics.mean(
                    data["processing_times"]
                )
                worker_stats[worker_id]["total_processing_time"] = sum(
                    data["processing_times"]
                )
    ...
    def generate_report(self, batch_id: str, output_file: str = None) -> str:
        ...
        for category, count in aggregated["quality_distribution"].items():
            percentage = (
                (count / aggregated["processed_files"]) * 100
                if aggregated["processed_files"] > 0
                else 0
            )
        ...
        for issue in aggregated["common_issues"][:5]:  # Top 5 issues
            ...
            report_lines.append(
                f"  Occurrences: {issue['count']} ({issue['percentage']:.1f}%)"
            )
            report_lines.append("")
...
def main():
    ...
    aggregate_parser = subparsers.add_parser(
        "aggregate", help="Aggregate results for a batch"
    )
    aggregate_parser.add_argument("batch_id", help="Batch ID to aggregate")
    aggregate_parser.add_argument(
        "--force-refresh", action="store_true", help="Force refresh from database"
    )
