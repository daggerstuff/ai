...
class TopicThemeAnalyzer:
    ...
    def analyze_topics_and_themes(
        ...
        try:
            ...
            # Generate insights
            insights = self._generate_topic_theme_insights(
                topic_analyses, theme_analyses
            )
            ...
            print(
                f"✅ Analyzed {len(topic_analyses)} topics and {len(theme_analyses)} themes from {len(conversations)} conversations"
            )
            return results
    ...
    def _analyze_topics(self, conversations: List[Dict]) -> Dict[str, TopicAnalysis]:
        ...
        try:
            ...
            for conv_data in conversations:
                conversation_text = self._extract_conversation_text(
                ...
                for topic_name, keywords in self.analysis_config[
                    "topic_keywords"
                ].items():
                    found_keywords = []
            ...
            for topic_name, count in topic_counts.items():
                if count >= self.analysis_config["min_topic_frequency"]:
                    ...
                    quality_score = min(
                        1.0, count / 100
                    )  # Normalize to max 100 conversations
                    ...
                    topic_analyses[topic_name] = TopicAnalysis(
                        ...
                        conversation_examples=topic_conversations[topic_name][
                            :5
                        ],  # First 5 examples
    ...
    def _analyze_themes(self, conversations: List[Dict]) -> Dict[str, ThemeAnalysis]:
        ...
        try:
            ...
            for conv_data in conversations:
                ...
                for theme_name, keywords in self.analysis_config[
                    "therapeutic_themes"
                ].items():
                    found_keywords = []
                    for keyword in keywords:
                    ...
                    if found_keywords:
                        ...
                        # Find related topics
                        for topic_name, topic_keywords in self.analysis_config[
                            "topic_keywords"
                        ].items():
            ...
            for theme_name, count in theme_counts.items():
                if count >= self.analysis_config["min_topic_frequency"]:
                    ...
                    complexity_score = min(
                        1.0, len(theme_topics[theme_name]) / 5
                    )  # Normalize to max 5 topics

                    # Calculate therapeutic relevance
                    therapeutic_relevance = min(
                        1.0, count / 50
                    )  # Normalize to max 50 conversations
    ...
    def _generate_topic_theme_insights(
        self, topics: Dict[str, TopicAnalysis], themes: Dict[str, ThemeAnalysis]
    ) -> List[str]:
        ...
        try:
            ...
            if topics:
                ...
                insights.append(
                    f"Most common topic: '{most_common_topic[0]}' appears in {most_common_topic[1].frequency} conversations ({most_common_topic[1].percentage:.1f}%)"
                )

                # Topic diversity
                topic_count = len(topics)
                insights.append(
                    f"Topic diversity: {topic_count} distinct topics identified"
                )

                # High-frequency topics
                high_freq_topics = [
                    name for name, analysis in topics.items() if analysis.frequency > 20
                ]
                if high_freq_topics:
                    insights.append(
                        f"High-frequency topics: {', '.join(high_freq_topics)}"
                    )

            # Theme insights
            if themes:
                most_relevant_theme = max(
                    themes.items(), key=lambda x: x[1].therapeutic_relevance
                )
                insights.append(
                    f"Most therapeutically relevant theme: '{most_relevant_theme[0]}' with {most_relevant_theme[1].conversation_count} conversations"
                )
                ...
                if complex_themes:
                    insights.append(
                        f"Complex themes with multiple topic clusters: {', '.join(complex_themes)}"
                    )

            # Cross-analysis insights
            if topics and themes:
                insights.append(
                    f"Analysis coverage: {len(topics)} topics and {len(themes)} therapeutic themes identified"
                )

                # Find most connected themes
                theme_connections = {
                    name: len(analysis.topic_clusters)
                    for name, analysis in themes.items()
                }
                if theme_connections:
                    most_connected = max(theme_connections.items(), key=lambda x: x[1])
                    insights.append(
                        f"Most connected theme: '{most_connected[0]}' relates to {most_connected[1]} different topics"
                    )

            if not insights:
                insights.append(
                    "Insufficient data for comprehensive topic and theme analysis"
                )
    ...
    def export_topic_theme_report(self, analysis_results: Dict[str, Any]) -> str:
        ...
        try:
            ...
            report_file = (
                self.output_dir / f"topic_theme_analysis_report_{timestamp}.json"
            )
            ...
            export_data = {
                "report_metadata": {
                    ...
                    "conversations_analyzed": analysis_results.get(
    ...
    def _create_summary_statistics(
        ...
        try:
            ...
            # Coverage statistics
            total_conversations = analysis_results.get(
                "total_conversations_analyzed", 0
            )
            topic_coverage = (
                sum(topic_frequencies) / total_conversations
            ...
            return {
                ...
                "most_frequent_topic": (
                    max(topics.items(), key=lambda x: x[1].frequency)[0]
                    if topics
                    else None
                ),
                "most_relevant_theme": (
                    max(themes.items(), key=lambda x: x[1].therapeutic_relevance)[0]
                    if themes
                    else None
                ),
            }
...
def main():
    ...
    print(
        f"   - Conversations analyzed: {analysis_results.get('total_conversations_analyzed', 0)}"
    )
    ...
    if topics:
        ...
        sorted_topics = sorted(
            topics.items(), key=lambda x: x[1].frequency, reverse=True
        )
        for name, analysis in sorted_topics[:5]:
            print(
                f"   • {name.replace('_', ' ').title()}: {analysis.frequency} conversations ({analysis.percentage:.1f}%)"
            )
    ...
    if themes:
        ...
        for name, analysis in themes.items():
            print(
                f"   • {name.replace('_', ' ').title()}: {analysis.conversation_count} conversations"
            )
