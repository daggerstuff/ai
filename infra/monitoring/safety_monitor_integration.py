...
class SafetyMonitor:
        ...
        if contacts_file.exists():
            with open(contacts_file, "r") as f:
                ...
                self.clinical_contacts = [
                    ClinicalContact(**contact) for contact in contacts_data
                ]
        else:
    ...
    async def _calculate_trend(self, user_id: str, current_risk: float) -> str:
        ...
        # Calculate trend
        recent_avg = (
            np.mean(recent_scores[-3:])
            if len(recent_scores) >= 3
            else recent_scores[-1]
        )
        previous_avg = (
            np.mean(recent_scores[-6:-3])
            if len(recent_scores) >= 6
            else np.mean(recent_scores[:-1])
        )
    ...
    async def _trigger_safety_alert(self, metrics: SafetyMetrics):
        ...
        logger.warning(
            f"Safety alert triggered for user {metrics.user_id}: {alert.trigger_reason}"
        )
    ...
    async def _send_alert_notifications(self, alert: SafetyAlert):
        ...
        contacts = [
            c
            for c in self.clinical_contacts
            if c.escalation_level <= alert.escalation_level
        ]
    ...
    async def get_safety_dashboard_data(self) -> Dict[str, Any]:
        ...
        high_risk_sessions = [
            ...
            if metrics.safety_level
            in [SafetyLevel.HIGH_RISK, SafetyLevel.CRITICAL_RISK, SafetyLevel.EMERGENCY]
        ]
        ...
        avg_risk_score = (
            np.mean([m.risk_score for m in self.active_sessions.values()])
...
async def main():
    ...
    if dashboard_data["high_risk_users"]:
        ...
        for user in dashboard_data["high_risk_users"]:
            print(
                f"  {user['user_id']}: {user['safety_level']} (Risk: {user['risk_score']:.3f})"
            )
    ...
    if dashboard_data["recent_alerts"]:
        ...
        for alert in dashboard_data["recent_alerts"]:
            print(
                f"  {alert['alert_id']}: {alert['safety_level']} - User {alert['user_id']}"
            )
