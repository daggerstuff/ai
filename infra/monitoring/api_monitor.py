...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self.time_series_data: Dict[str, deque] = defaultdict(
    ...
    def record_business_metric(
        ...
        self.record_metric(
            f"business.{metric_name}", value, MetricType.GAUGE, dimensions
        )
...
class PerformanceAnalyzer:
    ...
    def get_top_errors(
        self, limit: int = 10, time_window_minutes: int = 60
    ) -> List[Dict[str, Any]]:
        ...
        for rule in self.alert_rules:
            ...
            if should_alert:
                ...
                if suppression_key in self.alert_suppression:
                    if (
                        current_time - self.alert_suppression[suppression_key]
...
class DashboardGenerator:
    ...
    def generate_executive_dashboard(self) -> Dict[str, Any]:
        ...
        avg_response_time = self.analyzer.get_response_time_percentiles(
    ...
    def generate_technical_dashboard(self) -> Dict[str, Any]:
        ...
        # Performance metrics
        response_times = self.analyzer.get_response_time_percentiles(
            time_window_minutes=60
        )
        error_rate = self.analyzer.get_error_rate(time_window_minutes=60)
    ...
    def generate_business_dashboard(self) -> Dict[str, Any]:
        ...
        recent_business_metrics = [
            m
            for m in self.collector.business_metrics
            if m.timestamp >= current_time - timedelta(hours=24)
        ]
...
class APIMonitor:
    ...
    async def _cleanup_old_data(self):
        ...
        self.metrics_collector.metrics = [
            m for m in self.metrics_collector.metrics if m.timestamp >= cutoff_time
        ]

        # Clean up performance metrics
        self.metrics_collector.performance_metrics = [
            m
            for m in self.metrics_collector.performance_metrics
...
async def main():
    ...
    print(
        f"  Total Requests (24h): {executive_dashboard['kpis']['total_requests_24h']}"
    )
    print(f"  Error Rate (1h): {executive_dashboard['kpis']['error_rate_1h']}%")
    print(
        f"  Avg Response Time (1h): {executive_dashboard['kpis']['avg_response_time_1h']}ms"
    )
