...
class DifficultClientGenerator:
        ...
        self.personality_patterns = {
            ClientPersonality.RESISTANT: {
                ...
                "resistance": [
                    ...
                    "Therapist competence challenges",
                ],
                "triggers": [
            ...
            ClientPersonality.NARCISSISTIC_TRAITS: {
                ...
                "triggers": [
                    "Criticism",
                    "Not being seen as special",
                    "Accountability",
                ],
                "response_patterns": {
                    "grandiosity": [
                        "I'm obviously more intelligent than most people",
                        "I shouldn't have to wait",
                    ],
                    "devaluation": [
                        "You clearly don't understand someone of my caliber",
                        "This is beneath me",
                    ],
                    "manipulation": [
    ...
    def generate_difficult_client(
        ...
        num_comorbidities = min(
            complexity_multiplier, len(self.complexity_factors["comorbidities"])
        )
        num_stressors = min(
            complexity_multiplier, len(self.complexity_factors["social_stressors"])
        )

        comorbidities = random.sample(
            self.complexity_factors["comorbidities"], num_comorbidities
        )
        social_factors = random.sample(
            self.complexity_factors["social_stressors"], num_stressors
        )

        # Generate presenting problem based on personality type
        presenting_problem = self._generate_presenting_problem(
            personality_type, comorbidities
        )

        # Create AI behavior instructions
        ai_instructions = self._create_ai_instructions(
            personality_type, difficulty_level, base_pattern
        )
        ...
        return DifficultClientProfile(
            ...
            interpersonal_patterns=self._generate_interpersonal_patterns(
                personality_type
            ),
            ...
            common_therapist_mistakes=self._generate_therapist_mistakes(
                personality_type
            ),
            ...
            de_escalation_responses=self._generate_de_escalation_responses(
                personality_type
            ),
        )
    ...
    def _generate_presenting_problem(
        self, personality_type: ClientPersonality, comorbidities: List[str]
    ) -> str:
        ...
        base_problem = problems.get(
            personality_type, "Complex psychological presentation"
        )
    ...
    def _generate_trauma_history(
        self, personality_type: ClientPersonality
    ) -> List[str]:
    ...
    def _generate_defense_mechanisms(
        self, personality_type: ClientPersonality
    ) -> List[str]:
    ...
    def _generate_emotional_patterns(
        self, personality_type: ClientPersonality
    ) -> List[str]:
        ...
        return patterns.get(
            personality_type, ["Emotional avoidance", "Mood instability"]
        )

    def _generate_interpersonal_patterns(
        self, personality_type: ClientPersonality
    ) -> List[str]:
        ...
        return patterns.get(
            personality_type, ["Relationship difficulties", "Trust issues"]
        )
    ...
    def _generate_therapist_mistakes(
        self, personality_type: ClientPersonality
    ) -> List[str]:
    ...
    def _generate_de_escalation_responses(
        self, personality_type: ClientPersonality
    ) -> List[str]:
