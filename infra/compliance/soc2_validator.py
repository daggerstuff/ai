...
class SystemMonitor:
    ...
    def get_availability_metrics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        available_measurements = sum(
            1 for m in recent_metrics if m["cpu_usage"] < 95 and m["memory_usage"] < 95
        )
...
class SOC2Storage:
    ...
    def _init_database(self):
        ...
        try:
            ...
            # Create indexes
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_control_tests_timestamp ON control_tests(timestamp)"
            )
            cursor.execute(
...
class SOC2Validator:
    ...
    def _perform_control_test(
        self, control: SOC2Control
        ...
        if control.control_id == "CC6.1":  # Access controls
            ...
            evidence.append(
                f"System monitoring active - CPU: {metrics.get('avg_cpu', 0):.1f}%"
            )
            evidence.append(f"Memory usage: {metrics.get('avg_memory', 0):.1f}%")

            if metrics.get("incidents", 0) > 0:
                exceptions.append(
                    f"{metrics['incidents']} availability incidents detected"
                )
    ...
    def _calculate_principle_scores(
        self, control_results: List[ControlTest]
    ) -> Dict[SOC2Principle, float]:
        ...
        for principle in SOC2Principle:
            ...
            compliant_count = sum(
                1
                for result in relevant_controls
    ...
    def _determine_overall_status(
        self, principle_scores: Dict[SOC2Principle, float]
    ) -> ComplianceStatus:
    ...
    def _generate_soc2_recommendations(
        ...
        if failed_controls:
            recommendations.append(
                f"Address {len(failed_controls)} non-compliant controls"
            )
        ...
        availability_issues = [
            result for result in availability_controls if result.exceptions
        ]
        ...
        if any(result.remediation_required for result in control_results):
            recommendations.append(
                "Implement remediation plans for identified control gaps"
            )
        ...
        try:
            ...
            """,
                (
                    ...
                    json.dumps(
                        {k.value: v for k, v in assessment.principle_scores.items()}
                    ),
