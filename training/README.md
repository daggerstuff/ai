...
1.  **⚡ Unsloth (Preferred)**: Uses `unsloth.FastLanguageModel` for 2x faster training and 60% less VRAM.
2.  **🐢 HuggingFace (Fallback)**: Standard `transformers` + `peft` implementation for compatibility.
...
1.  Ensure you have a GPU available (or modify script for CPU/MPS if debugging logic only).
2.  Run the command above.
3.  If `ULTIMATE_FINAL_DATASET.jsonl` is missing, the script will automatically generate dummy data to test the flow.
