        ...
        try:
            ...
            if preds and len(preds) > 0:
                ...
                predictions.append({
                    ...
                    "maturity_score": float(p.maturity_score) if p.maturity_score is not None else None,
                    "probabilities": [round(float(pr), 4) for pr in p.probabilities],
                })
            else:
...
def write_submission(predictions: list[dict], output_path: str):
    ...
    with open(path, "w", encoding="utf-8") as f:
        for pred in predictions:
            line = json.dumps({"id": pred["id"], "label": pred["label"]}, ensure_ascii=False)
            f.write(line + "\n")
...
def main():
    ...
    parser.add_argument("--checkpoints", type=str, nargs="+", required=False, help="Legacy: ignored")
    parser.add_argument("--weights", type=float, nargs="+", default=None, help="Legacy: ignored")
    parser.add_argument("--temperature", type=float, default=0.0, help="Temperature scaling")
    parser.add_argument("--out", type=str, default="submission.jsonl", help="Output file path")
    parser.add_argument("--batch-size", type=int, default=16, help="Legacy: ignored")
    parser.add_argument("--limit", type=int, default=-1, help="Limit number of samples (-1 = all)")
