...
def discover_datasets():
    """Discover all datasets in HETZNER S3"""
    ...
    report_path = Path("training_ready/data/hetzner_s3_discovery.json")
...
def download_file(remote_path, local_path):
    """Download file from HETZNER S3"""
    local_path = Path(local_path)
    local_path.parent.mkdir(parents=True, exist_ok=True)

    cmd = f"{HETZNER_AI_CLI} data download hel1 pixelated-training-data {remote_path} --output {local_path}"
...
def main():
    ...
    print("🚀 HETZNER S3 Dataset Discovery")
    print("=" * 50)
