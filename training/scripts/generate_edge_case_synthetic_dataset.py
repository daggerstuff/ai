...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--prompts",
        default="",
        help="Path to local prompts JSON",
    )
    parser.add_argument(
        "--prompts-key",
...
def _load_s3_manifest(manifest_path: Path) -> tuple[str, str]:
    ...
    # Support enhanced registry format
    if not bucket:
        storage = manifest.get("storage", {}).get("primary", {})
...
def main() -> int:
    ...
    if Path(args.prompts).exists():
         ...
         logger.info(f"Loaded local prompts from {args.prompts}")
    else:
