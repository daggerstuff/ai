...
def _setup_simulator(args: argparse.Namespace, loader: S3DatasetLoader, device: str) -> GestaltSimulator:
    ...
    if model_path and not model_path.exists():
        if not args.defense_model_s3_key:
            logger.info(
                "No defense model S3 key provided. Continuing with NIM/Directives only."
            )
            model_path = None
            return GestaltSimulator(
                defense_model_path=None if args.nim_only else None,
                device=device,
                use_nim_for_llm=True,
            )
    ...
    defense_model_path = (
        None if args.nim_only else (str(model_path) if model_path else None)
    )
    return GestaltSimulator(
        defense_model_path=defense_model_path,
        device=device,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional S3 key for the defense model if not present locally.",
    )
    parser.add_argument(
        "--s3-bucket",
        type=str,
    ...
    if args.resume:
        checkpoint_state = _load_checkpoint(loader, checkpoint_s3_key)
        if checkpoint_state:
            checkpoint_records = _safe_int(
                checkpoint_state.get("output_records_written", 0), 0
            )
            resume_offset = max(0, checkpoint_records)
            logger.info(
                "Loaded checkpoint %s with %d completed records.",
                checkpoint_s3_key,
                checkpoint_records,
            )

        best_checkpoint = _find_best_checkpoint_for_resume(
            loader=loader,
            checkpoint_prefix=args.checkpoint_prefix,
            target_max_records=args.max_records,
            target_input_prefix=args.input_s3_prefix,
            minimum_records=resume_offset,
        )
        if (
            best_checkpoint is not None
            and best_checkpoint[1]
            and _safe_int(best_checkpoint[1].get("output_records_written"), 0)
            > resume_offset
        ):
            best_key = best_checkpoint[0]
            best_state = best_checkpoint[1]
            best_records = _safe_int(best_state.get("output_records_written"), 0)
            if best_key != checkpoint_s3_key:
                logger.warning(
                    "Promoting resume context from %s (%d records) to %s (%d records).",
                    checkpoint_s3_key,
                    resume_offset,
                    best_key,
                    best_records,
                )
                checkpoint_s3_key = best_key
                args.checkpoint_s3_key = checkpoint_s3_key
            checkpoint_state = best_state
            resume_offset = best_records

        if checkpoint_state is not None:
            checkpoint_output = checkpoint_state.get("output_s3_key")
            if (
                isinstance(checkpoint_output, str)
                and checkpoint_output.strip()
                and checkpoint_output != args.output_s3_key
            ):
                checkpoint_output = checkpoint_output.strip()
                if _s3_object_exists(loader, checkpoint_output):
                    logger.warning(
                        "Aligning output target with checkpoint context: %s",
                        checkpoint_output,
                    )
                    args.output_s3_key = checkpoint_output
                else:
                    logger.warning(
                        "Checkpoint context points to output key %s, but it is "
                        "not present in S3. Continuing with requested output key "
                        "%s using source resume offset.",
                        checkpoint_output,
                        args.output_s3_key,
                    )

        if resume_offset <= 0:
            output_count = _count_s3_lines(loader, args.output_s3_key, args.max_records)
            resume_offset = max(resume_offset, output_count)
            if output_count:
                logger.info(
                    "Derived resume offset from existing output (%s) as %d.",
                    args.output_s3_key,
                    output_count,
                )

        resume_offset = min(max(0, resume_offset), args.max_records)
        if resume_offset > 0:
            if checkpoint_state is None:
                checkpoint_state = _new_checkpoint_state(
                    args, input_files, resume_offset
                )
            else:
                checkpoint_state["output_records_written"] = resume_offset
    ...
    try:
        output_bucket, output_key = loader._parse_s3_path(args.output_s3_key)
        _upload_file_with_retries(
            loader.s3_client, temp_out_path, output_bucket, output_key
        )
        if args.resume and checkpoint_state is not None:
            checkpoint_state["status"] = "complete"
            checkpoint_state["output_records_written"] = records_processed
            _save_checkpoint(loader, checkpoint_s3_key, checkpoint_state)
