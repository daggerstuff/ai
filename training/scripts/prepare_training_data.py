...
class DataPreparationOrchestrator:
    ...
    def _run_script(
        self, stage: str, script_path: Path, args: Optional[list] = None
    ) -> bool:
    ...
    def run_stage(self, stage: str, force: bool = False) -> bool:
        ...
        if stage in self.checkpoint["completed_stages"] and not force:
            logger.info(
                f"⏭️  Skipping {stage} (already completed). Use --force to rerun."
            )
            return True
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report_files = {
            ...
            "filtering": self.base_path
            / "ai"
            / "training_ready"
            / "scripts"
            / "output"
            / "filtering_report.json",
            "formatting": self.base_path
            / "ai"
            / "training_ready"
            / "scripts"
            / "output"
            / "formatting_report.json",
            "assembly": self.base_path
            / "ai"
            / "training_ready"
            / "scripts"
            / "output"
