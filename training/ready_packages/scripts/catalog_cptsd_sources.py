...
class CPTSDSourceCataloger:
    ...
    def run_HETZNER_AI_CLI_command(self, command: List[str]) -> str:
        """Execute an HETZNER_AI_CLI CLI command and return output."""
        try:
            import shlex

            logger.debug(f"Executing command: {shlex.join(command)}")
        # SECURITY: Always validate the command base and arguments
    ...
    def list_s3_objects(self, prefix: str) -> List[Dict]:
        ...
        output = self.run_HETZNER_AI_CLI_command(
            [
                HETZNER_AI_CLI,
                "bucket",
                "object",
