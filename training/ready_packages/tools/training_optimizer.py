...
def optimize_for_dataset(
    ...
    print(
        f"   Completion: {estimate.estimated_completion.strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print(
        f"   Fits in {max_hours}h window: {'✅ Yes' if estimate.fits_in_window else '❌ No'}"
    )
    ...
    training_args = optimizer.create_optimized_training_args(
        profile=profile, num_epochs=num_epochs
    )
