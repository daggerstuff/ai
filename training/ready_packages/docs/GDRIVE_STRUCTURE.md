...
```bash
# Use HETZNER sync script
./ai/training_ready/platforms/hetzner/gdrive-download.sh download-all

# Or interactive mode
./ai/training_ready/platforms/hetzner/gdrive-download.sh download-interactive
```
...
def sync_gdrive_to_s3():
    ...
    # See: ai/training_ready/platforms/hetzner/sync-datasets.sh
    pass
```
...
- **Download Scripts**: `ai/training_ready/platforms/hetzner/gdrive-download.sh` - For staging/syncing
- **Sync Scripts**: `ai/training_ready/platforms/hetzner/sync-datasets.sh` - Google Drive → S3 sync
