...
class MoEConfig:
    ...
    def __post_init__(self):
        if self.expert_domains is None:
            self.expert_domains = ["psychology", "mental_health", "bias_detection", "general_therapeutic"]
...
class ExpertRouter(nn.Module):
    ...
    def __init__(self, config: MoEConfig):
        ...
        self.domain_classifiers = nn.ModuleDict(
            {domain: nn.Linear(config.hidden_size, 1) for domain in config.expert_domains}
        )
    ...
    def forward(
        ...
        routing_weights, expert_indices = torch.topk(routing_probs, k=self.expert_capacity, dim=-1)
    ...
    def compute_load_balancing_loss(self, router_logits: torch.Tensor) -> torch.Tensor:
        ...
        load_balancing_loss = F.kl_div(expert_probs.log(), uniform_probs, reduction="batchmean")
...
class MoELayer(nn.Module):
    ...
    def __init__(self, config: MoEConfig):
        ...
        # Domain experts
        self.experts = nn.ModuleList([DomainExpert(config, domain) for domain in config.expert_domains])
...
class TherapeuticMoEModel(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        outputs = self.model(
            input_ids=input_ids, attention_mask=attention_mask, labels=labels, output_hidden_states=True, **kwargs
        )
        ...
        for moe_layer in self.moe_layers:
            hidden_states, routing_info = moe_layer(hidden_states, return_routing_info=return_routing_info)
            if routing_info:
        ...
        if labels is not None:
            ...
            if routing_infos:
                load_balancing_loss = sum(info.get("load_balancing_loss", 0) for info in routing_infos) / len(
                    routing_infos
                )
                loss = loss + self.config.load_balancing_weight * load_balancing_loss

        return {"loss": loss, "logits": logits, "routing_info": routing_infos if return_routing_info else None}
    ...
    def save_pretrained(self, save_directory: str):
        ...
        moe_state = {"config": self.config, "moe_layers": [layer.state_dict() for layer in self.moe_layers]}
    ...
    def from_pretrained(cls, load_directory: str, base_model: PreTrainedModel):
        ...
        with torch.serialization.safe_globals([MoEConfig]):
            moe_state = torch.load(f"{load_directory}/moe_layers.pt", weights_only=True, map_location="cpu")
...
def create_therapeutic_moe_model(
    base_model_name: str, moe_config: MoEConfig | None = None, device: str = "auto"
) -> TherapeuticMoEModel:
    """
