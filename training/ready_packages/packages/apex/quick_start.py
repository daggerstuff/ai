...
def validate_data():
    ...
    if not DATA_PREP_AVAILABLE or prepare_data is None:
        logger.error("❌ Data preparation module not available")
        return False
    try:
...
def start_training():
    ...
    try:
        ...
        subprocess.run([sys.executable, "train_enhanced.py"], check=True, text=True, shell=False)
...
def display_package_info():
    ...
    if config_path.exists():
        ...
        logger.info(
            f"📊 Total Conversations: {config.get('dataset_config', {}).get('total_conversations', 'Unknown'):,}"
        )
        logger.info(
            f"🔧 Components Integrated: {len(config.get('kan28_components', {}).get('integrated_components', []))}"
        )
        logger.info(f"👥 Expert Voices: {', '.join(config.get('kan28_components', {}).get('expert_voices', []))}")
        logger.info(
            f"🧠 Psychology Concepts: {config.get('kan28_components', {}).get('psychology_concepts', 'Unknown'):,}"
        )
