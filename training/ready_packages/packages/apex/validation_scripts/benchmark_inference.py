...
class InferenceBenchmark:
    ...
    def run_sequential_benchmark(
        self, num_requests: int = 100, use_cache: bool = True
    ) -> BenchmarkResult:
    ...
    async def run_concurrent_benchmark(
        self, num_requests: int = 100, concurrency: int = 10, use_cache: bool = True
    ) -> BenchmarkResult:
        ...
        async def make_request(prompt: str):
            ...
            try:
                _, metadata = await self.engine.generate_async(
                    prompt, use_cache=use_cache
                )
                latencies.append(metadata["latency"])
                successful += 1
    ...
    def print_results(self, result: BenchmarkResult, title: str = "Benchmark Results"):
        ...
        print(
            f"  P95:               {result.p95_latency:.3f}s {'✅' if result.p95_latency < 2.0 else '❌'}"
        )
        ...
        print(f"")
        print(
            f"SLA Status:          {'✅ PASS' if result.meets_sla else '❌ FAIL'} (P95 < 2.0s)"
        )
        print(f"{'=' * 60}")
...
async def main():
    ...
    result_seq_no_cache = benchmark.run_sequential_benchmark(
        num_requests=50, use_cache=False
    )
    ...
    result_seq_cache = benchmark.run_sequential_benchmark(
        num_requests=50, use_cache=True
    )
    ...
    result_concurrent = await benchmark.run_concurrent_benchmark(
        num_requests=50, concurrency=5, use_cache=True
    )
    ...
    print(f"")
    print(
        f"Cache Improvement: {((result_seq_no_cache.p95_latency - result_seq_cache.p95_latency) / result_seq_no_cache.p95_latency) * 100:.1f}%"
    )
    print(f"")
