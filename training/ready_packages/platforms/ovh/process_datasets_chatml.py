...
class DatasetProcessor:
    ...
    def __init__(self, base_dir: str = "~/datasets/consolidated"):
        ...
        self.stats = defaultdict(
            lambda: {"total": 0, "valid": 0, "skipped": 0, "tokens_approx": 0}
        )
            ...
            try:
                ...
                if isinstance(data, list):
                    for item in data:
                        ...
                        if self.validate_conversation(
                            messages
                        ) and not self.is_duplicate(messages):
                            conversations.append(
            ...
            try:
                ...
                for item in items:
                    ...
                    answer = self.clean_text(
                        item.get(
                            "answerText", item.get("answer", item.get("Response", ""))
                        )
                    )
        ...
        for json_file in psych_dir.rglob("*.json"):
            try:
                ...
                for item in items:
                    ...
                    if not self.is_duplicate(messages):
                        conversations.append(
                            {"messages": messages, "source": "psych8k"}
                        )
                    ...
                    for line in f:
                        ...
                        if "messages" in item:
                            ...
                            messages = [
                                {
                                    "role": "system",
                                    "content": SYSTEM_PROMPTS["foundation"],
                                },
                                {
                                    "role": "user",
                                    "content": self.clean_text(item["prompt"]),
                                },
                                {
                                    "role": "assistant",
                                    "content": self.clean_text(item["response"]),
                                },
                            ]
                        else:
                            continue
    ...
    def generate_report(self, start_time: datetime):
        ...
        report = {
            ...
            "summary": {
                ...
                "estimated_tokens": sum(
                    s["tokens_approx"] for s in self.stats.values()
                ),
        ...
        logger.info("-" * 60)
        logger.info(
            f"TOTAL VALID CONVERSATIONS: {report['summary']['total_conversations']:,}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process datasets for ChatML training format"
    )
    parser.add_argument(
        "--base-dir",
    ...
    parser.add_argument(
        "--stage",
        choices=["all", "foundation", "reasoning", "processed"],
        default="all",
