#!/bin/bash
# HETZNER AI Deploy Inference Deployment Script
# Deploys Pixelated Empathy inference service to HETZNER AI Deploy
...
check_prereqs() {
    if ! command -v ${HETZNER_AI_CLI} &> /dev/null; then
        log_error "${HETZNER_AI_CLI} CLI not found"
        exit 1
    fi
    
    if ! ${HETZNER_AI_CLI} me &> /dev/null; then
        log_error "Not authenticated. Run: ${HETZNER_AI_CLI} login"
        exit 1
    fi
}
...
build_image() {
    ...
    if [ -z "$registry" ]; then
        registry=$(${HETZNER_AI_CLI} registry list --json 2>/dev/null | jq -r '.[0].url' || echo "")
        if [ -z "$registry" ]; then
    ...
    docker build -f docker/hetzner/Dockerfile.inference -t pixelated-inference:latest .
    
    log_info "Logging into HETZNER registry..."
...
deploy_app() {
    ...
    # Check if app exists
    if ${HETZNER_AI_CLI} app get "$app_name" &> /dev/null; then
        log_info "App exists, updating..."
        ${HETZNER_AI_CLI} app stop "$app_name" || true
        sleep 5
    fi
    
    # Deploy
    ${HETZNER_AI_CLI} app create \
        --name "$app_name" \
        --gpu 1 \
        --gpu-model "$GPU_MODEL" \
        --cpu 4 \
        --memory 16Gi \
        --replicas "$REPLICAS" \
        --volume "$CHECKPOINT_BUCKET@$HETZNER_REGION:/models:ro" \
    ...
    # Get app URL
    local app_url=$(${HETZNER_AI_CLI} app get "$app_name" --json | jq -r '.url' || echo "")
    if [ -n "$app_url" ]; then
...
scale_app() {
    ...
    ${HETZNER_AI_CLI} app scale "$app_name" --replicas "$replicas"
...
status_app() {
    ...
    ${HETZNER_AI_CLI} app get "$app_name"
}
...
stop_app() {
    ...
    ${HETZNER_AI_CLI} app stop "$app_name"
...
delete_app() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ${HETZNER_AI_CLI} app delete "$app_name"
...
# Show usage
usage() {
    echo "HETZNER AI Deploy Inference Deployment"
    echo ""
