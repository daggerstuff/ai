#!/bin/bash
# HETZNER Object Storage Dataset Sync Script
# Syncs training datasets to HETZNER Object Storage for AI Training
#
# Dataset Sources:
...
check_hetzner_ai_cli() {
    if ! command -v ${HETZNER_AI_CLI} &> /dev/null; then
        log_error "${HETZNER_AI_CLI} CLI not found. Please install it first:"
...
check_auth() {
    log_info "Checking HETZNER authentication..."
    if ! ${HETZNER_AI_CLI} me &> /dev/null; then
        log_error "Not authenticated to HETZNER. Please run: ${HETZNER_AI_CLI} login"
        exit 1
    fi
    log_success "Authenticated to HETZNER AI Platform"
}

ensure_bucket() {
    local bucket_name=$1
    log_info "Checking bucket: $bucket_name"
    if ${HETZNER_AI_CLI} bucket list "$bucket_name" &> /dev/null; then
        log_success "Bucket '$bucket_name' exists"
    else
        log_warn "Bucket '$bucket_name' may need to be created via HETZNER Control Panel"
...
upload_datasets() {
    log_info "Uploading datasets to HETZNER Object Storage..."
    log_info "Target bucket: $DATA_BUCKET@$HETZNER_REGION"
    ...
    if [ -d "$AI_DIR/data/acquired_datasets" ]; then
        ...
        ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$AI_DIR/data/acquired_datasets/" --prefix "acquired/" || log_warn "Failed to upload acquired datasets"
    fi
    ...
    if [ -d "$AI_DIR/data/lightning_h100" ]; then
        ...
        ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$AI_DIR/data/lightning_h100/" --prefix "lightning/" || log_warn "Failed to upload lightning data"
    fi
    ...
    if [ -d "$AI_DIR/data/tim_fletcher_voice" ]; then
        ...
        ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$AI_DIR/data/tim_fletcher_voice/" --prefix "voice/" || log_warn "Failed to upload voice data"
    fi
    ...
    if [ -d "$AI_DIR/pipelines/pixel_voice" ]; then
        ...
        if [ -d "$AI_DIR/pipelines/pixel_voice/data" ]; then
            ...
            ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$AI_DIR/pipelines/pixel_voice/data/" --prefix "pixel_voice/" || log_warn "Failed to upload pixel voice data"
        fi
    fi
    ...
    if [ -d "$AI_DIR/pipelines/edge_case_pipeline_standalone" ]; then
        ...
        for data_file in "$AI_DIR/pipelines/edge_case_pipeline_standalone"/*.json*; do
            if [ -f "$data_file" ]; then
                ...
                ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$data_file" --prefix "edge_cases/" || log_warn "Failed to upload $data_file"
            fi
        done
    fi
    ...
    if [ -d "$AI_DIR/pipelines/dual_persona_training" ]; then
        ...
        ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$AI_DIR/pipelines/dual_persona_training/" --prefix "dual_persona/" || log_warn "Failed to upload dual persona data"
    fi
    ...
    if [ -f "$AI_DIR/data/dataset_registry.json" ]; then
        ...
        ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$AI_DIR/data/dataset_registry.json" --prefix "config/" || log_warn "Failed to upload registry"
    fi
    ...
    if [ -f "$AI_DIR/lightning/moe_training_config.json" ]; then
        ...
        ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$AI_DIR/lightning/moe_training_config.json" --prefix "config/" || log_warn "Failed to upload MoE config"
    fi
    ...
    if [ -d "$GDRIVE_MOUNT" ] && [ "$(ls -A "$GDRIVE_MOUNT" 2>/dev/null)" ]; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            if [ -d "$GDRIVE_MOUNT/cot_reasoning" ]; then
                ...
                ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$GDRIVE_MOUNT/cot_reasoning/" --prefix "gdrive/cot_reasoning/" || log_warn "Failed to upload CoT reasoning"
            else
                ...
                for dataset in "CoT_Reasoning_Clinical_Diagnosis_Mental_Health" "CoT_Heartbreak_and_Breakups_downloaded.json" "CoT_Neurodivergent_vs_Neurotypical_Interactions_downloaded.json"; do
                    if [ -d "$GDRIVE_MOUNT/$dataset" ] || [ -f "$GDRIVE_MOUNT/$dataset" ]; then
                        ...
                        ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$GDRIVE_MOUNT/$dataset" --prefix "gdrive/cot_reasoning/" || log_warn "Failed to upload $dataset"
                    fi
                done
            fi
            ...
            if [ -d "$GDRIVE_MOUNT/professional_therapeutic" ]; then
                ...
                ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$GDRIVE_MOUNT/professional_therapeutic/" --prefix "gdrive/professional_therapeutic/" || log_warn "Failed to upload professional therapeutic"
            else
                # Fallback to legacy structure
                for dataset in "Psych8k" "mental_health_counseling_conversations" "therapist-sft-format" "SoulChat2.0"; do
                    if [ -d "$GDRIVE_MOUNT/$dataset" ]; then
                        log_info "  Uploading $dataset (legacy)..."
                        ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$GDRIVE_MOUNT/$dataset/" --prefix "gdrive/professional_therapeutic/" || log_warn "Failed to upload $dataset"
                    fi
                done
            fi
            ...
            if [ -d "$GDRIVE_MOUNT/priority" ]; then
                ...
                ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$GDRIVE_MOUNT/priority/" --prefix "gdrive/priority/" || log_warn "Failed to upload priority datasets"
            elif [ -d "$GDRIVE_MOUNT/datasets-wendy" ]; then
                log_info "  Uploading priority datasets (legacy: datasets-wendy)..."
                ${HETZNER_AI_CLI} data push "$DATA_BUCKET" "$GDRIVE_MOUNT/datasets-wendy/" --prefix "gdrive/priority/" || log_warn "Failed to upload priority datasets"
            fi
        else
    ...
    echo ""
    log_info "Dataset structure on HETZNER:"
    echo "  s3://$DATA_BUCKET/"
...
# Download datasets from HETZNER
download_datasets() {
    local target_dir="${1:-/data}"
    log_info "Downloading datasets from HETZNER Object Storage to $target_dir..."
    
    mkdir -p "$target_dir"
    
    ${HETZNER_AI_CLI} data pull "$DATA_BUCKET" "$target_dir/" || {
...
list_contents() {
    ...
    ${HETZNER_AI_CLI} bucket list "$DATA_BUCKET"
    
    echo ""
    log_info "Listing contents of $CHECKPOINT_BUCKET..."
    ${HETZNER_AI_CLI} bucket list "$CHECKPOINT_BUCKET" 2>/dev/null || log_warn "Checkpoint bucket may not exist yet"
}

# Download checkpoints
download_checkpoints() {
    local target_dir="${1:-/checkpoints}"
    log_info "Downloading checkpoints from HETZNER..."
    
    mkdir -p "$target_dir"
    
    ${HETZNER_AI_CLI} data pull "$CHECKPOINT_BUCKET" "$target_dir/" || {
...
upload_checkpoints() {
    ...
    log_info "Uploading checkpoints to HETZNER..."
    ...
    ${HETZNER_AI_CLI} data push "$CHECKPOINT_BUCKET" "$source_dir/" || {
...
# Show usage
usage() {
    echo "HETZNER Object Storage Dataset Sync Script"
    echo ""
    echo "Usage: $0 <command> [options]"
    echo ""
    echo "Commands:"
    echo "  upload              Upload datasets to HETZNER Object Storage"
    echo "  download [dir]      Download datasets from HETZNER (default: /data)"
