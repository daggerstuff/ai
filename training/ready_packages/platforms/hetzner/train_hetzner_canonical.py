#!/usr/bin/env python3
"""
HETZNER AI Training - Wayfarer-2-12B Supervised Fine-Tuning
...
- HETZNER Object Storage for data and checkpoints
- WandB integration for experiment tracking

Dataset Structure (on HETZNER Object Storage):
  /data/
...
logger = logging.getLogger("train_hetzner_canonical")
...
def setup_wandb(config: Dict, stage_name: str):
    ...
    try:
        ...
        run = wandb.init(
            ...
            name=f"hetzner-{stage_name}-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M')}",
            config=config,
            tags=["hetzner", stage_name, "wayfarer-2-12b"],
        )
...
def train_stage(
    ...
    training_args = TrainingArguments(
        ...
        run_name=f"{stage_name}-{datetime.now(timezone.utc).strftime('%Y%m%d')}",
...
def main():
    ...
    parser = argparse.ArgumentParser(description="HETZNER AI Training - Wayfarer-2-12B SFT")
    ...
    logger.info("=" * 60)
    logger.info("HETZNER AI Training - Wayfarer-2-12B Supervised Fine-Tuning")
...
def run_staged_training_pipeline(config, args, training_start_time):
    ...
    logger.info(f"Total duration: {datetime.now(timezone.utc) - training_start_time}")
    logger.info("=" * 60)
