...
class TestReadDvcMd5:
    def test_valid_dvc_file(self, tmp_path: Path) -> None:
        ...
        dvc.write_text(
            json.dumps({"outs": [{"md5": "abc123", "path": "train.jsonl", "size": 100}]})
        )
...
class TestLoadVersioned:
    ...
    def mock_repo(self, tmp_path: Path) -> Path:
        ...
        train_data = [{"messages": [{"role": "user", "content": "hi"}]}, {"messages": [{"role": "user", "content": "bye"}]}]
        ...
        for split, data in [("train", train_data), ("val", val_data), ("test", test_data)]:
            ...
            dvc_path.write_text(
                json.dumps({"outs": [{"md5": md5, "path": f"{split}.jsonl", "size": data_path.stat().st_size}]})
            )

        return tmp_path

    @mock.patch("subprocess.run")
    def test_load_train(self, mock_run: mock.Mock, mock_repo: Path) -> None:
    ...
    @mock.patch("subprocess.run")
    def test_missing_data_file_after_pull(self, mock_run: mock.Mock, mock_repo: Path) -> None:
    ...
    @mock.patch("subprocess.run")
    def test_git_checkout_called_with_correct_args(self, mock_run: mock.Mock, mock_repo: Path) -> None:
        load_versioned("val", version="dataset-v2.0.0", repo_root=mock_repo)
        # Check that git checkout was called with the right arguments.
    ...
    @mock.patch("subprocess.run")
    def test_dvc_pull_called(self, mock_run: mock.Mock, mock_repo: Path) -> None:
    ...
    @mock.patch("subprocess.run")
    def test_empty_jsonl(self, mock_run: mock.Mock, mock_repo: Path) -> None:
        """An empty JSONL file should return an empty list."""
        data_path = mock_repo / "data" / "curated" / "sft_chatml" / "train.jsonl"
        data_path.write_text("")  # Empty file.
    ...
    @mock.patch("subprocess.run")
    def test_subprocess_error_propagates(self, mock_run: mock.Mock, mock_repo: Path) -> None:
...
class TestListAvailableVersions:
    @mock.patch("subprocess.run")
    def test_lists_dataset_tags(self, mock_run: mock.Mock) -> None:
    ...
    @mock.patch("subprocess.run")
    def test_no_tags(self, mock_run: mock.Mock) -> None:
        mock_run.return_value = mock.Mock(stdout="")
        versions = list_available_versions()
        assert versions == []

    @mock.patch("subprocess.run")
    def test_filters_only_dataset_prefix(self, mock_run: mock.Mock) -> None:
        mock_run.return_value = mock.Mock(
            stdout="dataset-v1.0.0\nv0.9.0\nother-tag\ndataset-v2.0.0\n"
        )
        versions = list_available_versions()
        # git tag -l "dataset-v*" already filters, but test the function handles it.
