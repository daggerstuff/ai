        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    raw_content = ""
                    if "messages" in data:
                        raw_content = "\n".join([f"{m.get('role', 'user')}: {m.get('content', '')}" for m in data["messages"]])
                    elif "conversations" in data:
                        raw_content = "\n".join([f"{m.get('from', 'user')}: {m.get('value', '')}" for m in data["conversations"]])
                    elif "text" in data:
...
def main():
    ...
    storage = FileStorage(first_entry_file_name=prep_file, cache_path="./unified_cache", file_name_prefix="unified_eval", cache_type="jsonl")
