...
FIXED: TrajectoryGroup does not have a .reward attribute.
...
DATASET_PATH = "/home/vivi/dataset/final_training_dataset.jsonl"
...
async def main():
    ...
    print(f"Loaded {len(examples)} examples for RL")
    ...
    for step in range(MAX_RL_STEPS):
        ...
        result = await backend.train(
            model, train_groups, learning_rate=LEARNING_RATE, precalculate_logprobs=True
        )
        await model.log(
            train_groups,
        ...
        # FIXED: TrajectoryGroup has no .reward; iterate through trajectories
        total_reward = sum(
            t.reward for g in train_groups for t in g.trajectories
        )
