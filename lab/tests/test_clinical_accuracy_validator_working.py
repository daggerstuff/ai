...
class MockClinicalAccuracyValidator:
    ...
    def validate_clinical_accuracy(
        ...
        therapeutic_score = self._assess_therapeutic_appropriateness(
            response_lower, context
        )
        boundary_score = self._assess_professional_boundaries(response_lower, context)

        # Calculate overall accuracy
        overall_accuracy = (
            dsm5_score + evidence_score + therapeutic_score + boundary_score
        ) / 4
    ...
    def _assess_dsm5_compliance(self, response: str, context: Dict[str, Any]) -> float:
        ...
        if any(
            term in response
            for term in ["you have", "you are diagnosed", "you suffer from"]
        ):
            score -= 0.3
    ...
    def _assess_evidence_based_practice(
        ...
        if any(
            term in response for term in ["always works", "guaranteed", "never fails"]
        ):
            score -= 0.3
    ...
    def _assess_professional_boundaries(
        ...
        if any(
            term in response
            for term in ["therapeutic", "session", "treatment", "professional"]
        ):
            score += 0.1
    ...
    def _identify_violations(self, response: str, context: Dict[str, Any]) -> List[str]:
        ...
        if any(
            term in response_lower
            for term in ["friend", "personal", "be friends", "outside of therapy"]
        ):
        ...
        if any(
            term in response_lower
            for term in ["harm yourself", "end it all", "should harm", "no hope"]
        ):
    ...
    def _generate_recommendations(
        self, dsm5: float, evidence: float, therapeutic: float, boundary: float
    ) -> List[str]:
        ...
        if evidence < 0.7:
            recommendations.append(
                "Use more evidence-based interventions and techniques"
            )

        if therapeutic < 0.7:
            recommendations.append(
                "Enhance therapeutic rapport and collaborative approach"
            )
    ...
    def validate_treatment_plan(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        completeness_score = (len(required_components) - len(missing_components)) / len(
            required_components
        )
    ...
    def get_validation_statistics(self) -> Dict[str, Any]:
        ...
        avg_accuracy = (
            sum(v["clinical_accuracy"] for v in self.validation_history) / total
        )

        severity_counts = {}
        for severity in self.severity_levels:
            severity_counts[severity] = sum(
                1 for v in self.validation_history if v["severity"] == severity
            )
        ...
        return {
            ...
            "dsm5_compliance_rate": sum(
                1 for v in self.validation_history if v["dsm5_compliance"]
            )
            / total
            * 100,
            "evidence_based_rate": sum(
                1 for v in self.validation_history if v["evidence_based"]
            )
            / total
            * 100,
        }
...
class TestClinicalAccuracyValidator(unittest.TestCase):
    ...
    def test_excellent_clinical_responses(self):
        ...
        for response in self.test_responses["excellent"]:
            with self.subTest(response=response):
                result = self.validator.validate_clinical_accuracy(
                    response, self.test_context
                )
    ...
    def test_concerning_clinical_responses(self):
        ...
        for response in self.test_responses["concerning"]:
            with self.subTest(response=response):
                ...
                self.assertIn(
                    result["severity"], ["concerning", "unacceptable", "acceptable"]
                )
    ...
    def test_unacceptable_clinical_responses(self):
        ...
        for response in self.test_responses["unacceptable"]:
            with self.subTest(response=response):
                ...
                violation_found = any(
                    violation in expected_violations
                    for violation in result["violations"]
                )
                self.assertTrue(
                    violation_found,
                    f"Expected one of {expected_violations}, got {result['violations']}",
                )
    ...
    def test_dsm5_compliance_assessment(self):
        ...
        result = self.validator.validate_clinical_accuracy(
            good_response, self.test_context
        )
        self.assertTrue(result["dsm5_compliance"])
        ...
        result = self.validator.validate_clinical_accuracy(
            poor_response, self.test_context
        )
    ...
    def test_evidence_based_practice_assessment(self):
        ...
        evidence_response = (
            "Research shows that cognitive behavioral therapy is effective for anxiety."
        )
        result = self.validator.validate_clinical_accuracy(
            evidence_response, self.test_context
        )
        self.assertTrue(result["evidence_based"])

        # Non-evidence-based response
        non_evidence_response = (
            "This treatment always works and is guaranteed to cure you."
        )
        result = self.validator.validate_clinical_accuracy(
            non_evidence_response, self.test_context
        )
    ...
    def test_therapeutic_appropriateness_assessment(self):
        ...
        appropriate_response = (
            "I understand this is difficult. What do you think might be helpful?"
        )
        result = self.validator.validate_clinical_accuracy(
            appropriate_response, self.test_context
        )
        self.assertGreaterEqual(result["therapeutic_appropriateness"], 0.8)

        # Inappropriate therapeutic response
        inappropriate_response = (
            "You should just get over it. I would never feel that way."
        )
        result = self.validator.validate_clinical_accuracy(
            inappropriate_response, self.test_context
        )
    ...
    def test_professional_boundaries_assessment(self):
        ...
        result = self.validator.validate_clinical_accuracy(
            good_boundaries, self.test_context
        )
        ...
        result = self.validator.validate_clinical_accuracy(
            boundary_violation, self.test_context
        )
