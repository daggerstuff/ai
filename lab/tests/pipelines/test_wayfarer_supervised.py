...
@patch(
    "ai.pipelines.wayfarer_supervised.bias_privacy_hooks", new_callable=lambda: Mock()
)
def test_orchestration_e2e(
    mock_hooks, mock_finetune, mock_tokenizer, mock_loader, dummy_config
):
    ...
    mock_hooks.run_bias_privacy_pipeline.return_value = {
        "bias": {},
        "pii": [],
        "phi": [],
    }
...
def test_finetune_error_propagation(mock_finetune, dummy_config):
    ...
    ws.bias_privacy_hooks = Mock(
        run_bias_privacy_pipeline=lambda *a, **kw: {"bias": {}, "pii": [], "phi": []}
    )
