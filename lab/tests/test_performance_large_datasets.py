        ...
        for batch_start in range(0, num_conversations, batch_size):
            ...
            for i in range(batch_start, batch_end):
                ...
                conversation_json = json.dumps(
                    [{"human": human_msg}, {"assistant": assistant_msg}]
                )
...
class TestLargeDatasetLoading(PerformanceTestBase):
    ...
    def test_load_10k_conversations(self):
        ...
        try:
            ...
            self.assertLess(
                perf_metrics["execution_time"],
                10.0,
                "Loading should complete within 10 seconds",
            )
            self.assertLess(
                perf_metrics["memory_delta_mb"],
                500,
                "Memory usage should be under 500MB",
            )

            print(
                f"  ✅ Loaded 10K conversations in {perf_metrics['execution_time']:.2f}s"
            )
        ...
        try:
            ...
            def load_data_chunked():
                ...
                for chunk in pd.read_sql_query(
                    "SELECT * FROM conversations", conn, chunksize=chunk_size
                ):
                    chunks.append(chunk)
            ...
            print(
                f"  ✅ Loaded 50K conversations in {perf_metrics['execution_time']:.2f}s"
            )
            print(f"  ✅ Memory usage: {perf_metrics['memory_delta_mb']:.1f}MB")
    ...
    def test_streaming_data_processing(self):
        ...
        try:
            ...
            self.assertLess(
                perf_metrics["execution_time"],
                20.0,
                "Streaming processing should be efficient",
            )
            self.assertLess(
                perf_metrics["memory_delta_mb"], 300, "Streaming should use less memory"
            )

            print(
                f"  ✅ Processed 25K conversations in {perf_metrics['execution_time']:.2f}s"
            )
            print(f"  ✅ Memory usage: {perf_metrics['memory_delta_mb']:.1f}MB")
...
class TestAnalyticsPerformance(PerformanceTestBase):
    ...
    def test_aggregation_performance(self):
        ...
        large_df = pd.DataFrame(
            {
                ...
                "dataset_source": np.random.choice(
                    ["dataset_a", "dataset_b", "dataset_c"], 20000
                ),
                "tier": np.random.choice(
                    ["priority_1", "standard", "additional"], 20000
                ),
        ...
        def perform_aggregations():
            ...
            tier_stats = large_df.groupby("tier").agg(
                {"quality_score": ["mean", "count"], "word_count": "sum"}
            )
        ...
        # Performance assertions
        self.assertLess(
            perf_metrics["execution_time"], 5.0, "Aggregations should complete quickly"
        )
        self.assertLess(
            perf_metrics["memory_delta_mb"],
            200,
    ...
    def test_statistical_analysis_performance(self):
        ...
        # Performance assertions
        self.assertLess(
            perf_metrics["execution_time"], 3.0, "Statistical analysis should be fast"
        )
        self.assertLess(
            perf_metrics["memory_delta_mb"],
            150,
            "Statistical analysis should be memory efficient",
        )
    ...
    def test_machine_learning_performance(self):
        ...
        self.assertLess(
            perf_metrics["execution_time"],
            15.0,
            "ML training should complete within reasonable time",
        )
        self.assertLess(
            perf_metrics["memory_delta_mb"],
            300,
            "ML training should manage memory well",
        )

        print(
            f"  ✅ Trained ML model on 10K samples in {perf_metrics['execution_time']:.2f}s"
        )
        print(f"  ✅ Memory usage: {perf_metrics['memory_delta_mb']:.1f}MB")
...
class TestScalabilityLimits(PerformanceTestBase):
    ...
    def test_memory_scalability(self):
        ...
        for size in dataset_sizes:
            ...
            def process_dataset():
                ...
                result = df.groupby(df.index // 1000).agg(
                    {"word_count": "mean", "quality_score": ["mean", "std"]}
                )
                return result
        ...
        memory_growth_rate = (memory_usage[-1] - memory_usage[0]) / (
            dataset_sizes[-1] - dataset_sizes[0]
        )
        ...
        print(
            f"  ✅ Memory growth rate: {memory_growth_rate:.4f} MB per 1K conversations"
        )
        print(
            f"  ✅ Time growth rate: {time_growth_rate:.6f} seconds per 1K conversations"
        )
    ...
    def test_concurrent_processing_simulation(self):
        ...
        def simulate_concurrent_processing():
            ...
            for i in range(num_workers):
                worker = threading.Thread(
                    target=worker_function, args=(work_queue, result_queue)
                )
                worker.start()
        ...
        print(
            f"  ✅ Processed 20 tasks concurrently in {perf_metrics['execution_time']:.2f}s"
        )
        print(f"  ✅ Memory usage: {perf_metrics['memory_delta_mb']:.1f}MB")
