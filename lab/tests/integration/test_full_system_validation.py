...
class FullSystemValidator:
    ...
    async def _run_preflight_checks(self):
        ...
        logger.info(
            f"System Resources: {memory_gb:.1f}GB RAM, {cpu_count} CPUs, {disk_gb:.1f}GB Disk"
        )

        if memory_gb < 8:
            self.results.add_warning(
                f"Low memory: {memory_gb:.1f}GB (recommended: 16GB+)"
            )
    ...
    async def _validate_dataset_availability(self):
        ...
        logger.info(
            f"Found approximately {total_conversations:,} conversations in test data"
        )
                ...
                for endpoint, method in endpoints:
                    ...
                    try:
                        ...
                        self.results.performance_metrics[
                            f"api_{endpoint.replace('/', '_')}_ms"
                        ] = response_time

                        if response.status_code == 200:
                            logger.info(
                                f"API {method} {endpoint}: {response_time:.0f}ms - ✅"
                            )
                            self.results.add_success()
                        else:
                            self.results.add_error(
                                f"API {method} {endpoint}: HTTP {response.status_code}"
                            )
    ...
    async def _test_export_systems(self):
        ...
        try:
            ...
            for format_type in export_formats:
                ...
                self.results.performance_metrics[f"export_{format_type}_ms"] = (
                    export_time
                )
    ...
    async def _test_end_to_end_workflows(self):
        ...
        for workflow in workflows:
            try:
                ...
                self.results.performance_metrics[f"workflow_{workflow}_ms"] = (
                    workflow_time
                )
    ...
    async def _run_stress_tests(self):
        ...
        try:
            ...
            self.results.performance_metrics["stress_test_duration_seconds"] = (
                stress_duration
            )
            ...
            logger.info(
                f"Stress test: {successful_tasks}/{concurrent_tasks} tasks successful"
            )
            ...
            if successful_tasks == concurrent_tasks:
                ...
                self.results.add_warning(
                    f"Some stress tasks failed: {concurrent_tasks - successful_tasks}"
                )
    ...
    async def _test_disaster_recovery(self):
        ...
        try:
            ...
            if monitoring_available > 0:
                logger.info(
                    f"Monitoring system: {monitoring_available} configs available - ✅"
                )
                self.results.add_success()
            else:
    ...
    def save_results(self, output_path: Path = None):
        ...
        if not output_path:
            output_path = (
                project_root / "tests/results/full_system_validation_report.json"
            )
...
async def test_full_system_validation():
    ...
    assert (
        success_rate >= 80
    ), f"System validation failed: {success_rate:.1f}% success rate"
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(f"{'='*60}")
        print(
            f"Success Rate: {results.successful_operations}/{results.successful_operations + results.failed_operations} ({(results.successful_operations / (results.successful_operations + results.failed_operations) * 100) if (results.successful_operations + results.failed_operations) > 0 else 0:.1f}%)"
        )
