...
class TherapeuticPromptGenerator:
        ...
        for sentence in sentences[:3]:  # Focus on first few sentences
            ...
            if len(words) > 5:
                ...
                for word in words:
                    if any(
                        term in word.lower()
                        for term in ["trauma", "heal", "therap", "relation", "emotion"]
                    ):
                        context = " ".join(
                            words[max(0, words.index(word) - 2) : words.index(word) + 3]
                        )
    ...
    def generate_prompt(self, segment: Dict) -> str:
        ...
        templates = self.prompt_templates.get(
            style, self.prompt_templates["therapeutic"]
        )
...
def main():
    ...
    print(
        f"  Success rate: {total_stats['processed'] / total_stats['total'] * 100:.1f}%"
    )
