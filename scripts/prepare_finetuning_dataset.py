    ...
    def _replace_pattern(self, text: str, pattern: re.Pattern, placeholder: str) -> str:
        ...
        for match in matches:
            ...
            if match and isinstance(match, str):
                ...
                self._increment_stat(f"{placeholder.split('_')[0]}s_redacted")
...
class FineTuningDatasetPreparer:
    ...
    def __init__(
        ...
        logger.info(
            f"FineTuningDatasetPreparer initialized with "
        ...
        for pattern in patterns:
            for file_path in transcript_path.glob(pattern):
                try:
                    ...
                    logger.warning(
                        f"Failed to load transcript {file_path}: {e}"
                    )
    ...
    def create_training_examples(
        ...
        for transcript in transcripts:
            ...
            standard_examples = self._create_standard_examples(
                transcript, memories, reflections
            )
            examples.extend(standard_examples)
            
            # Create memory-specific examples
            if memories:
                memory_examples = self._create_memory_examples(
                    transcript, memories, reflections
                )
    ...
    def _create_standard_examples(
        ...
        if isinstance(data, list):
            for record in data:
                examples.extend(
                    self._create_examples_from_record(
                        record, memories, reflections, transcript
                    )
                )
            return examples
        
        # Handle single record format
        return self._create_examples_from_record(
            data, memories, reflections, transcript
        )
    ...
    def _create_memory_examples(
        ...
        if isinstance(data, list):
            for record in data:
                examples.extend(
                    self._create_memory_examples_from_record(
                        record, memories, reflections, transcript
                    )
                )
            return examples
        ...
        # Handle single record format
        return self._create_memory_examples_from_record(
            data, memories, reflections, transcript
        )
    ...
    def _detect_conversation_type(self, messages: list[Any]) -> str:
        ...
        if any(word in text for word in ["crisis", "emergency", "urgent"]):
            return "crisis"
        elif any(word in text for word in ["reflection", "insight", "learned"]):
            return "reflection"
        elif any(word in text for word in ["goal", "progress", "improvement"]):
            return "therapeutic"
        elif "greeting" in text or "hello" in text:
            return "onboarding"
    ...
    def anonymize_examples(
        ...
        logger.info(
            f"Anonymized {len(anonymized)} examples "
            f"(PII replacements: {self.anonymizer.get_stats()})"
        )
    ...
    def _compute_statistics(
        ...
        self.statistics.avg_memories_per_example = (
            total_memories / total if total > 0 else 0.0
        )
    ...
    def prepare(
        ...
        examples = self.create_training_examples(
            transcripts, memories, reflections
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Prepare fine-tuning dataset for Pixelated Empathy AI"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    if abs(total_ratio - 1.0) > 0.001:
        logger.error(
            f"Train/validation/test ratios must sum to 1.0, got {total_ratio}"
        )
        return 1
