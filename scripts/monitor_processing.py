    ...
    while True:
        ...
        if not is_running and progress["status"] == "completed":
            ...
            if config_file.exists():
                ...
                print(
                    f"   Processed Conversations: {stats.get('processed_conversations', 0)}"
                )
                ...
                print(
                    f"   Contextual Questions: {stats.get('contextual_questions', 0)}"
                )
                print(f"   Duplicates Removed: {stats.get('duplicates_removed', 0)}")

                print("\n🚀 Ready for Lightning.ai H100 Training!")
                print("📁 Dataset Location: /root/pixelated/data/unified_training/")
