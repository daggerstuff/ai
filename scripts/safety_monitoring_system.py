...
class SafetyMonitoringSystem:
    ...
    async def process_user_input(
        self, user_input: str, user_id: str = None
    ) -> Dict[str, Any]:
        """
        ...
        if (
            ...
            or crisis_assessment["crisis_level"] != "none"
        ):
    ...
    def _analyze_context_indicators(self, user_input: str) -> Dict[str, Any]:
        ...
        indicators["urgency_words"] = sum(
            1 for word in urgency_words if word in user_input_lower
        )
        ...
        indicators["negative_emotion_words"] = sum(
            1 for word in negative_emotions if word in user_input_lower
        )

        # Check for help-seeking behavior
        help_words = ["help", "support", "talk", "listen", "advice"]
        indicators["help_seeking"] = any(
            word in user_input_lower for word in help_words
        )

        # Check temporal indicators
        indicators["past_tense_harm"] = any(
            phrase in user_input_lower
            for phrase in ["hurt myself", "cut myself", "tried to"]
        )
        indicators["future_tense_harm"] = any(
            phrase in user_input_lower for phrase in ["going to", "will", "plan to"]
        )

        # Check for specific methods
        methods = ["pills", "knife", "gun", "rope", "bridge", "car", "train"]
        indicators["specific_methods"] = any(
            method in user_input_lower for method in methods
        )

        return indicators

    def _determine_incident_severity(
        self, crisis_assessment: Dict[str, Any]
    ) -> IncidentSeverity:
    ...
    async def _create_incident(
        ...
        incident = SafetyIncident(
            ...
            response_actions=self.response_protocols.get(
        ...
        logger.warning(
            f"Safety incident created: {incident_id} (Severity: {severity.value})"
        )
    ...
    async def _trigger_immediate_intervention(self, incident: SafetyIncident):
        ...
        logger.critical(
            f"IMMEDIATE INTERVENTION REQUIRED: Incident {incident.incident_id}"
        )
    ...
    def _generate_safety_response(
        self, crisis_assessment: Dict[str, Any], incident: Optional[SafetyIncident]
    ) -> Dict[str, Any]:
    ...
    def _update_metrics(
        self, crisis_assessment: Dict[str, Any], processing_time: float
    ):
        """Update safety monitoring metrics"""

        # Update response time average
        total_time = self.metrics.response_time_avg * (
            self.metrics.total_interactions - 1
        )
    ...
    def get_safety_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "detection_rate": (
                self.metrics.crisis_detections / self.metrics.total_interactions * 100
            )
    ...
    def get_recent_incidents(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        recent_incidents = [
            incident for incident in self.incidents if incident.timestamp >= cutoff_time
        ]
...
if __name__ == "__main__":
    ...
    async def test_safety_monitoring():
        ...
        for i, test_input in enumerate(test_inputs, 1):
            ...
            print(
                f"Response: {response['response_type']} (Crisis Level: {response['crisis_level']})"
            )

            if response.get("incident_created"):
                print(
                    f"Incident Created: {response['incident_id']} (Severity: {response['severity']})"
                )
        ...
        for incident in incidents:
            print(
                f"  {incident['incident_id']}: {incident['severity']} - {incident['crisis_level']}"
            )
