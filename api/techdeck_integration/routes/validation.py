...
def quality_check():
    ...
    try:
        ...
        quality_results = _perform_quality_checks(
            data, checks, dataset_context, options
        )

        # Log validation results
        request_logger.info(
            f"Quality validation completed with score: {quality_results.get('overall_score', 0)}"
        )
    ...
    try:
        ...
        if bias_results.get("bias_detected", False):
            request_logger.warning(
                f"Bias detected with score: {bias_results.get('bias_score', 0)}"
            )
        else:
            ...
            jsonify(
                {
                    ...
                    "data": {
                        ...
                        "recommendations": compliance_results.get(
                            "recommendations", []
                        ),
...
def schema_validation():
    ...
    try:
        ...
        validation_results = _validate_against_schema(
            data, schema, strict_mode, options
        )
            ...
            jsonify(
                {
                    ...
                    "data": {
                        ...
                        "schema_id": schema_id,
                        "validation_time_ms": validation_results.get(
...
def batch_validate():
    ...
    try:
        ...
        request_logger.info(
            f"Batch validating {len(items)} items with type: {validation_type}"
        )
            ...
            for item in batch:
                try:
                    if validation_type == "quality":
                        result = _perform_quality_checks(
                            item, ["completeness", "consistency"], None, options
                        )
                    elif validation_type == "bias":
                        result = _perform_bias_detection(
                            item, {}, "demographic", options
                        )
                    elif validation_type == "schema":
                        # Schema validation requires schema_id, use default
                        result = _validate_against_schema(
                            item, {"fields": []}, True, options
                        )
                    else:
                    ...
                    batch_results.append(
                        {
                            ...
                            "data": result,
                            "item_index": i
                            + batch_results.index({"success": True, "data": result}),
                        }
                    )
                except Exception as e:
                    batch_results.append(
        ...
        request_logger.info(
            f"Batch validation completed: {successful}/{len(results)} successful"
        )
...
def get_validation_history():
    ...
    try:
        ...
        request_logger.info(
            f"Retrieving validation history with filters: dataset={dataset_id}, type={validation_type}"
        )
        ...
        history = _get_validation_history(
            redis_client, dataset_id, validation_type, start_date, end_date, limit
        )

        # Log successful retrieval
        request_logger.info(
            f"Retrieved {len(history.get('validations', []))} validation records"
        )
...
def _perform_quality_checks(
    ...
    try:
        ...
        if "consistency" in checks:
            consistency_score, consistency_issues = _check_consistency(
                data, dataset_context
            )
            ...
            if consistency_score < 0.8:
                recommendations.append(
                    "Review data consistency issues and standardize formats"
                )
...
def _perform_compliance_check(
    data: Dict[str, Any], compliance_type: str, options: Dict[str, Any]
) -> Dict[str, Any]:
    """Perform compliance validation."""
    try:
        violations = []
...
def _validate_against_schema(
    ...
    try:
        ...
        for field_name, field_value in data.items():
            ...
            field_errors = _validate_field_against_schema(
        ...
        return {
            ...
            "warnings": [],
            "validation_time_ms": (datetime.utcnow() - start_time).total_seconds()
            * 1000,
        }
...
def _get_validation_history(
    ...
    try:
        ...
        if dataset_id:
            history["validations"] = [
                v for v in history["validations"] if v.get("dataset_id") == dataset_id
            ]
        ...
        if validation_type:
            history["validations"] = [
                ...
                if v.get("validation_type") == validation_type
            ]
...
def _check_completeness(
    data: Dict[str, Any], dataset_context: Optional[Dict[str, Any]]
) -> tuple:
    ...
    non_empty_fields = sum(
        1 for value in data.values() if value is not None and value != ""
    )
    ...
    if completeness_score < 0.8:
        issues.append(
            f"Data completeness is {completeness_score:.1%}, below recommended 80%"
        )
...
def _check_consistency(
    ...
    for key, value in data.items():
        if isinstance(value, str) and value.isdigit():
            issues.append(
                f"Field '{key}' contains numeric string, consider using integer type"
            )
