...
def liveness_check():
    ...
    try:
        ...
        return jsonify({"success": True, "data": liveness_status}), 200
...
def _perform_health_checks(services: list[str], redis_client: RedisClient | None, detailed: bool) -> dict[str, Any]:
    ...
    try:
        ...
        return {"services": services_status, "system_info": system_info}
...
def _check_readiness(redis_client: RedisClient | None) -> dict[str, Any]:
    ...
    try:
        ...
        checks["essential_services"] = {
            "ready": True,
            "message": "All essential services running",
        }
...
def _validate_system_config(section: str | None, config: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if section == "security" or not section:
            ...
            if "jwt_expiration_hours" in security_config and (
                not isinstance(security_config["jwt_expiration_hours"], int)
                or security_config["jwt_expiration_hours"] < 1
            ):
...
def _get_system_logs(log_level: str, start_date: datetime, end_date: datetime, limit: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {"logs": logs, "summary": summary, "total_count": len(logs)}
