...
class PipelineOrchestrator:
    ...
    async def execute_pipeline(
        ...
        request_logger.info(
            f"Starting pipeline execution {execution_id} for user {user_id}"
        )
        ...
        try:
            ...
            pipeline_input = self._prepare_pipeline_input(
                execution_id, config, dataset_info, user_id
            )

            # Execute six-stage pipeline
            results = await self._execute_six_stage_pipeline(
                execution_id, pipeline_input
            )

            request_logger.info(
                f"Pipeline execution completed successfully: {execution_id}"
            )
    ...
    async def _execute_six_stage_pipeline(
        ...
        try:
            ...
            stage1_result = await self._execute_stage_1_data_ingestion(
                execution_id, pipeline_input
            )
            results["stages"]["data_ingestion"] = stage1_result

            # Stage 2: Preprocessing
            stage2_result = await self._execute_stage_2_preprocessing(
                execution_id, stage1_result
            )
            results["stages"]["preprocessing"] = stage2_result

            # Stage 3: Bias Detection
            stage3_result = await self._execute_stage_3_bias_detection(
                execution_id, stage2_result
            )
            results["stages"]["bias_detection"] = stage3_result

            # Stage 4: Standardization
            stage4_result = await self._execute_stage_4_standardization(
                execution_id, stage3_result
            )
            results["stages"]["standardization"] = stage4_result

            # Stage 5: Validation
            stage5_result = await self._execute_stage_5_validation(
                execution_id, stage4_result
            )
            results["stages"]["validation"] = stage5_result

            # Stage 6: Output Generation
            stage6_result = await self._execute_stage_6_output_generation(
                execution_id, stage5_result
            )
            results["stages"]["output_generation"] = stage6_result

            # Calculate total duration
            start_time = datetime.fromisoformat(
                results["start_time"].replace("Z", "+00:00")
            )
            end_time = datetime.fromisoformat(
                stage6_result["end_time"].replace("Z", "+00:00")
            )
            ...
            raise PipelineExecutionError(
                f"Six-stage pipeline execution failed: {str(e)}"
            )
    ...
    async def _execute_stage_5_validation(
        ...
        try:
            ...
            # Publish progress update
            await self._publish_progress_update(
                execution_id, "validation", 100, "Validation completed successfully"
            )
    ...
    async def _publish_progress_update(
        self, execution_id: str, stage: str, progress_percent: float, message: str
    ):
        ...
        try:
            ...
            await self.redis_client.publish(
                self.pipeline_channel, json.dumps(message_obj.__dict__)
            )
    ...
    async def cancel_pipeline_execution(self, execution_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            await self.redis_client.publish(
                self.pipeline_channel, json.dumps(cancel_message.__dict__)
            )

            self.logger.info(
                f"Pipeline execution cancellation requested: {execution_id}"
            )
        ...
        try:
            ...
            if execution_data["status"] != "completed":
                raise ValidationError(
                    "Execution results not available in current status"
                )
            ...
            if not results_data:
                raise ResourceNotFoundError(
                    f"Execution results not found for {execution_id}"
                )
        ...
        try:
            ...
            if config.get("data_size_mb", 0) > 100:
                validation_result["warnings"].append(
                    "Large dataset size may impact performance"
                )
            ...
            self.logger.info(
                f"Pipeline configuration validation completed: {validation_result['is_valid']}"
            )
