...
class RedisClient:
    ...
    def __init__(
        ...
        try:
            ...
            if redis_url.startswith(("redis://", "rediss://")):
                # Standard Redis URL (including secure TLS)
    ...
    def _parse_redis_url(self, url: str) -> dict[str, Any]:
        ...
        if ":" in url:
            ...
            if len(parts) == 2:
                host, port = parts
                return {"host": host, "port": int(port), "password": None}
    ...
    def _test_connection(self) -> None:
        ...
        try:
            ...
            raise TechDeckRedisError(f"Redis connection test failed: {e}") from e
    ...
    def get(self, key: str, default: Any = None) -> Any:
        ...
        try:
            ...
            raise TechDeckRedisError(f"Redis connection error: {e}") from e
        except RedisError as e:
            self.logger.error(f"Redis error for key {key}: {e}")
            raise TechDeckRedisError(f"Redis operation failed: {e}") from e
    ...
    def publish_message(self, channel: str, message: Any) -> int:
        ...
        try:
            ...
            raise TechDeckRedisError(f"Redis publish failed: {e}") from e
...
def create_redis_client(config: TechDeckServiceConfig) -> RedisClient:
    ...
    """
    return RedisClient(redis_url=config.REDIS_URL, db=config.REDIS_DB, config=config)
