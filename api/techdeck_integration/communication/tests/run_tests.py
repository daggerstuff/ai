...
class TestRunner:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        logger.info(
            "Test execution completed in %.2f seconds", self.end_time - self.start_time
        )
    ...
    async def _run_test_file(self, test_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, cwd=str(self.test_directory)
            )
    ...
    def _analyze_performance(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "sub_50ms_compliance": all(
                t < 0.05 for t in execution_times
            ),  # 50ms threshold
            "performance_grade": self._calculate_performance_grade(execution_times),
        }

    def _analyze_security_compliance(
        ...
        for result in results:
            if (
                "security" in result["name"].lower()
                or "hipaa" in result["name"].lower()
            ):
        ...
        return {
            ...
            "security_compliance_rate": (security_tests - security_failures)
            / security_tests
            if security_tests > 0
            else 1.0,
            "hipaa_compliance_rate": (hipaa_tests - security_failures) / hipaa_tests
            if hipaa_tests > 0
            else 1.0,
    ...
    def print_summary(self, summary: Dict[str, Any]):
        ...
        if perf.get("status") == "analyzed":
            ...
            print(
                f"  Sub-50ms Compliance: {'✅' if perf['sub_50ms_compliance'] else '❌'}"
            )
        ...
        print(
            f"  Security Compliance Rate: {security.get('security_compliance_rate', 1.0):.1%}"
        )
        print(
            f"  HIPAA Compliance Rate: {security.get('hipaa_compliance_rate', 1.0):.1%}"
        )
        print(
            f"  Overall Compliance: {'✅ COMPLIANT' if security.get('overall_compliance', True) else '❌ NON-COMPLIANT'}"
        )
...
async def run_specific_test(
    test_name: str, test_directory: str = ".", verbose: bool = False
) -> Dict[str, Any]:
...
def main():
    ...
    parser.add_argument(
        "--performance", "-p", action="store_true", help="Run only performance tests"
    )
    ...
    async def run_tests():
        if args.specific_test:
            result = await run_specific_test(
                args.specific_test, args.test_dir, args.verbose
            )
            return result
        else:
            runner = TestRunner(
                args.test_dir, args.verbose, args.performance, args.security
            )
