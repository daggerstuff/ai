...
class ErrorRecoveryManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.logger.info(
            "ErrorRecoveryManager initialized with comprehensive recovery strategies"
        )
    ...
    async def _execute_retry_strategy(
        ...
        if error_classification["category"] == "timeout_errors" and attempt_number <= 2:
            ...
            error_classification["category"] == "validation_errors"
    ...
    def _get_next_recovery_strategy(
        ...
        strategy_progression = {
            ...
            RecoveryStrategy.FALLBACK: RecoveryStrategy.SKIP
            if config.skip_enabled
            else RecoveryStrategy.ROLLBACK,
    ...
    async def _wait_before_retry(
        self, attempt_number: int, config: RecoveryConfig
    ) -> None:
        ...
        self.logger.info(
            f"Waiting {delay:.1f} seconds before retry attempt {attempt_number + 1}"
        )
        await asyncio.sleep(delay)

    def _generate_recommendations(
        self, final_strategy: RecoveryStrategy, recovery_data: Optional[Dict[str, Any]]
    ) -> List[str]:
        ...
        if final_strategy == RecoveryStrategy.RETRY and recovery_data:
            ...
            recommendations.append(
                "Fallback solution applied - review for optimization"
            )
            if "mitigation_applied" in recovery_data:
                recommendations.append(
                    f"Applied mitigation: {recovery_data['mitigation_applied']}"
                )
    ...
    def _generate_recovery_suggestions(self, error: Exception) -> List[str]:
        ...
        if error_classification["recoverable"]:
            ...
            suggestions.append(
                f"Recommended strategy: {error_classification['recommended_strategy'].value}"
            )
        else:
    ...
    async def get_error_checkpoint(
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status = (
                "healthy"
                if all([config_healthy, rules_healthy, stages_healthy])
                else "degraded"
            )
