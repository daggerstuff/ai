...
class MockAdaptiveLearner:
    ...
    def _extract_learning_signals(
        ...
        signals = {
            ...
            "outcome_quality": (
                1.0
                if therapeutic_outcome == "positive"
                else 0.5 if therapeutic_outcome == "neutral" else 0.0
            ),
            "engagement_level": interaction_data.get("engagement_level", 0.5),
            "technique_effectiveness": interaction_data.get(
    ...
    def _apply_learning_algorithms(
        self, learning_signals: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _update_adaptation_strategies(
    ...
    def get_adaptation_recommendations(self, context: Dict[str, Any]) -> List[str]:
        ...
        if positive_outcomes / len(recent_interactions) > 0.7:
            recommendations.append(
                "Continue current therapeutic approach - showing positive results"
            )
        elif positive_outcomes / len(recent_interactions) < 0.3:
            recommendations.append(
                "Consider adjusting therapeutic techniques - low positive feedback"
            )

        # Check effectiveness trends
        avg_effectiveness = sum(
            interaction["learning_signals"]["effectiveness_score"]
            for interaction in recent_interactions
    ...
    def adapt_response_strategy(
        self, current_strategy: Dict[str, Any], learning_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if self.learning_history:
            ...
            avg_engagement = sum(
                l["learning_signals"]["engagement_level"] for l in recent_learning
            ) / len(recent_learning)
    ...
    def evaluate_learning_progress(self) -> Dict[str, Any]:
        ...
        if total_interactions >= 10:
            early_performance = (
                sum(
                    l["learning_signals"]["effectiveness_score"]
                    for l in self.learning_history[:5]
                )
                / 5
            )
            recent_performance = (
            ...
            learning_progress = max(
                0.0, (recent_performance - early_performance) / early_performance
            )
        else:
            learning_progress = (
                0.1 * total_interactions
            )  # Linear progress for small samples

        # Calculate adaptation effectiveness
        successful_adaptations = sum(
            1
            for l in self.learning_history
        ...
        if learning_progress < 0.1:
            recommendations.append(
                "Increase learning rate or adjust learning algorithms"
            )
            ...
            interaction = {
                "user_feedback": {
                    "rating": (
                        3
                        if outcome == "neutral"
                        else (5 if outcome == "positive" else 1)
                    )
                },
