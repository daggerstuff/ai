...
class TestDatabaseOperations(unittest.TestCase):
    ...
    def test_data_filtering(self):
        ...
        df = pd.read_sql_query(
            "SELECT * FROM conversations WHERE dataset_source = 'test_dataset'", conn
        )
        self.assertEqual(len(df), 2)

        # Test filtering by tier
        df = pd.read_sql_query(
            "SELECT * FROM conversations WHERE tier = 'priority_1'", conn
        )
        self.assertEqual(len(df), 2)
...
class TestAnalyticsCalculations(unittest.TestCase):
    ...
    def test_correlation_analysis(self):
        ...
        correlation = self.test_data["quality_score"].corr(
            self.test_data["engagement_score"]
        )
