...
class TestComprehensiveValidation(unittest.TestCase):
    ...
    def test_group_i_audit_results(self):
        ...
        audit_file = Path(
            "/home/vivi/pixelated/ai/group_i_comprehensive_audit_results.json"
        )
    ...
    def test_validation_completeness(self):
        ...
        for task_num in validation_tasks:
            result_file = Path(
                f"/home/vivi/pixelated/ai/task_{task_num}_*_validation_results.json"
            )
            matching_files = list(
                Path("/home/vivi/pixelated/ai").glob(
                    f"task_{task_num}_*_validation_results.json"
                )
            )
            self.assertGreater(
                len(matching_files), 0, f"No validation results for task {task_num}"
            )
