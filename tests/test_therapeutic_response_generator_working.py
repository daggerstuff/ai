...
class MockTherapeuticResponseGenerator:
    ...
    def generate_response(
        self, client_input: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Generate a therapeutic response based on client input and context."""
        if (
            not client_input
            or not isinstance(client_input, str)
            or str(client_input).strip() == ""
        ):
            return {
                "success": False,
                "response": None,
    ...
    def _analyze_client_input(self, input_text: str) -> Dict[str, Any]:
        ...
        emotions = []
        if any(
            word in input_lower for word in ["sad", "depressed", "down", "hopeless"]
        ):
            emotions.append("sadness")
        if any(
            word in input_lower for word in ["anxious", "worried", "nervous", "scared"]
        ):
            emotions.append("anxiety")
        if any(
            word in input_lower for word in ["angry", "mad", "frustrated", "irritated"]
        ):
        ...
        if any(
            word in input_lower
            for word in ["crisis", "emergency", "urgent", "immediate"]
        ):
            urgency = "high"
        ...
        return {
            ...
            "complexity": (
                ...
                else "medium" if len(input_text) > 50 else "low"
            ),
        }
    ...
    def _generate_contextual_response(
        ...
        if primary_technique in self.response_templates:
            ...
            if (
                "{behavior}" in base_response
                and "behavioral_change" in analysis["therapeutic_needs"]
            ):
        ...
        if modality == "cbt":
            ...
            base_response += (
                " I wonder what this might connect to in your past experiences."
            )
    ...
    def _assess_response_quality(
        self, response: str, analysis: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        empathy_score = min(
            1.0,
            sum(1 for indicator in empathy_indicators if indicator in response_lower)
            * 0.15,
        )
        ...
        safety_score -= sum(
            0.2 for indicator in unsafe_indicators if indicator in response_lower
        )
        ...
        return {
            ...
            "assessment_criteria": {
                ...
                "collaborative_approach": "we" in response_lower
                or "together" in response_lower,
    ...
    def adapt_to_client_style(
        self, client_communication_style: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        emotional_style = client_communication_style.get(
    ...
    def get_generation_statistics(self) -> Dict[str, Any]:
        ...
        avg_quality = (
            sum(
                gen["quality_assessment"]["overall_quality"]
...
class TestTherapeuticResponseGenerator(unittest.TestCase):
        ...
        for input_type, client_input in self.test_inputs.items():
            with self.subTest(input_type=input_type):
                result = self.generator.generate_response(
                    client_input, self.test_contexts["cbt_session"]
                )
    ...
    def test_invalid_input_handling(self):
        ...
        for invalid_input in invalid_inputs:
            with self.subTest(input=invalid_input):
                result = self.generator.generate_response(
                    invalid_input, self.test_contexts["cbt_session"]
                )
    ...
    def test_safety_assessment(self):
        ...
        result = self.generator.generate_response(
            safe_input, self.test_contexts["cbt_session"]
        )
    ...
    def test_complex_input_handling(self):
        ...
        result = self.generator.generate_response(
            complex_input, self.test_contexts["cbt_session"]
        )
...
class TestTherapeuticResponseGeneratorIntegration(unittest.TestCase):
    ...
    def test_complete_therapeutic_conversation_flow(self):
        ...
        conversation_flow = [
            ...
            (
                "I've been having trouble sleeping because of work stress.",
                self.test_contexts["cbt_session"],
            ),
            (
                "When I think about work, I immediately assume the worst will happen.",
                self.test_contexts["cbt_session"],
            ),
            (
                "I tried the thought challenging technique we discussed.",
                self.test_contexts["cbt_session"],
            ),
        ]
        ...
        self.assertTrue(
            len(responses[2]["techniques_used"] + responses[3]["techniques_used"]) > 0
        )

    def test_multi_modal_therapeutic_approach(self):
        """Test integration across multiple therapeutic modalities."""
        client_input = (
            "I'm struggling with intense emotions and don't know how to cope."
        )
        ...
        )  # Multiple techniques across modalities
