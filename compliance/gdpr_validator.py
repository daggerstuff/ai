...
class GDPRStorage:
    ...
    def _init_database(self):
        ...
        try:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_requests_subject ON data_subject_requests(data_subject_id)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_violations_timestamp ON gdpr_violations(timestamp)"
            )
...
class GDPRValidator:
    ...
    def validate_data_processing(
        ...
        if personal_data:
            ...
            if consent_id and not self._validate_consent(
                consent_id, data_subject_id, purpose
            ):
            ...
            self._record_processing_activity(
                data_subject_id, personal_data, purpose, legal_basis, consent_id
            )
    ...
    def process_data_subject_request(
        ...
        try:
            ...
            if request.request_type == DataSubjectRight.ACCESS:
                response_data = self._process_access_request(
                    request.data_subject_id, cursor
                )
            elif request.request_type == DataSubjectRight.ERASURE:
                response_data = self._process_erasure_request(
                    request.data_subject_id, cursor
                )
            elif request.request_type == DataSubjectRight.RECTIFICATION:
                response_data = self._process_rectification_request(
                    request.data_subject_id, cursor
                )
            elif request.request_type == DataSubjectRight.DATA_PORTABILITY:
                response_data = self._process_portability_request(
                    request.data_subject_id, cursor
                )
    ...
    def _validate_consent(
        self, consent_id: str, data_subject_id: str, purpose: ProcessingPurpose
    ) -> bool:
        """Validate consent"""
        try:
            conn = sqlite3.connect(self.storage.db_path)
    ...
    def _check_data_minimization(
        ...
        has_sensitive = any(
            category in sensitive_categories for category, _ in personal_data
        )

        if has_sensitive and purpose in [
    ...
    def _process_rectification_request(
        self, data_subject_id: str, cursor
    ) -> Dict[str, Any]:
    ...
    def generate_gdpr_compliance_report(self) -> GDPRComplianceReport:
        ...
        overall_score = (
            consent_compliance + data_protection_compliance + rights_fulfillment_rate
        ) / 3
    ...
    def _calculate_data_protection_compliance(self) -> float:
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM personal_data_records WHERE encrypted = TRUE"
            )
    ...
    def _calculate_rights_fulfillment_rate(self) -> float:
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM data_subject_requests WHERE status = 'completed'"
            )
        ...
        try:
            ...
            for row in cursor.fetchall():
                violation = GDPRViolation(
                    ...
                    breach_detected_date=datetime.fromisoformat(
    ...
    def _generate_gdpr_recommendations(
        self, score: float, violations: List[GDPRViolation]
    ) -> List[str]:
