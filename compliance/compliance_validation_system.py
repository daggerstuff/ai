...
class ComplianceValidationSystem:
    ...
    async def _assess_compliance_control(
        ...
        if control.category == ControlCategory.ACCESS_CONTROL:
            ...
            score, gaps, recs, evidence = await self._assess_security_management(
                control
            )
            ...
            score, gaps, recs, evidence = await self._assess_business_continuity(
                control
            )
        else:
    ...
    async def _assess_security_management(
        self, control: ComplianceControl
    ) -> Tuple[float, List[str], List[str], int]:
    ...
    async def _assess_incident_response(
        self, control: ComplianceControl
    ) -> Tuple[float, List[str], List[str], int]:
    ...
    async def _assess_business_continuity(
        self, control: ComplianceControl
    ) -> Tuple[float, List[str], List[str], int]:
    ...
    def _calculate_compliance_metrics(self) -> Dict[str, Any]:
        ...
        for standard in standard_scores:
            if standard_counts[standard] > 0:
                standard_scores[standard] = (
                    standard_scores[standard] / standard_counts[standard]
                )
        ...
        self.overall_compliance_score = (
            total_score / total_controls if total_controls > 0 else 0.0
        )
        ...
        return {
            ...
            "total_gaps": sum(
                len(result.gaps_identified) for result in self.assessment_results
            ),
            "total_evidence": sum(
                result.evidence_count for result in self.assessment_results
            ),
        }
    ...
    def _determine_production_readiness(self, metrics: Dict[str, Any]) -> bool:
        ...
        if (
            metrics["compliance_levels"]["non_compliant"]
            > criteria["max_non_compliant"]
        ):
            return False
    ...
    def _generate_compliance_report(
        self, execution_time: float, metrics: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        report = {
            ...
            "compliance_metrics": {
                ...
                "partially_compliant_controls": metrics["compliance_levels"][
                    "partially_compliant"
                ],
                "non_compliant_controls": metrics["compliance_levels"]["non_compliant"],
                "compliance_percentage": (
                    metrics["compliance_levels"]["compliant"]
            ...
            "production_requirements": {
                ...
                "current_status": {
                    ...
                    "all_standards_met": all(
                        score >= 85.0 for score in metrics["standard_scores"].values()
                    ),
                    "non_compliant_within_limit": metrics["compliance_levels"][
                        "non_compliant"
                    ]
                    <= 2,
                },
    ...
    def _generate_compliance_recommendations(
        ...
        if metrics["overall_score"] < 90.0:
            recommendations.append(
                f"Improve overall compliance score from {metrics['overall_score']:.1f}% to ≥90%"
            )
        ...
        for standard, score in metrics["standard_scores"].items():
            if score < 85.0:
                recommendations.append(
                    f"Improve {standard.upper()} compliance from {score:.1f}% to ≥85%"
                )

        # Check category-specific recommendations
        low_scoring_categories = [
            cat for cat, score in metrics["category_scores"].items() if score < 85.0
        ]
        if low_scoring_categories:
            recommendations.append(
                f"Focus on improving {', '.join(low_scoring_categories)} controls"
            )

        if metrics["total_gaps"] > 5:
            recommendations.append(
                f"Address {metrics['total_gaps']} identified compliance gaps"
            )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(f"{'=' * 60}")
        print(
            f"Overall Compliance Score: {report['compliance_assessment_summary']['overall_compliance_score']}%"
        )
        print(
            f"Certification Status: {report['compliance_assessment_summary']['certification_status']}"
        )
        print(
            f"Production Ready: {'YES' if report['compliance_assessment_summary']['production_ready'] else 'NO'}"
        )
