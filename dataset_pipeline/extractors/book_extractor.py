...
class BookExtractor:
    ...
    def extract_all(self):
        ...
        if not self.has_pdf or not self.has_epub:
            print("Warning: PyMuPDF (fitz), ebooklib, or bs4 not installed. Book extraction will be skipped.")
            return

        all_files = list(self.streamer.list_files(self.base_prefix))
        books = [f for f in all_files if f.endswith(".pdf") or f.endswith(".epub")]
