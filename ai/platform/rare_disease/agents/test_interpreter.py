...
class TestInterpreterAgent:
    ...
    def interpret(
                ...
                for match in matches:
                    ...
                    evidence.append(
                        Evidence(
                            source="lab",
                            content=(
                                f"Finding: {finding.test_name} = {finding.value}"
                                f" (abnormal={finding.is_abnormal})"
                            ),
                            weight=weight,
        ...
        for ev in evidence:
            if ev.disease_id:
                updated_scores[ev.disease_id] = (
                    updated_scores.get(ev.disease_id, 0.0) + ev.weight
                )
