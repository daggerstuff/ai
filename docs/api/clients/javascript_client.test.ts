...
import { PixelatedEmpathyAPI, PixelatedEmpathyAPIError, RateLimitError, type RequestOptions, } from "./javascript_client";
...
describe("PixelatedEmpathyAPI Rate Limiting", () => {
 ...
 api.maxRetries = 2;
 const httpRequestSpy = vi.spyOn(api, "httpRequest");
 httpRequestSpy.mockResolvedValue({ statusCode: 429, headers: { "retry-after": "0" }, body: "", });
...
describe("PixelatedEmpathyAPI Methods", () => {
 ...
 expect(makeRequestSpy).toHaveBeenCalledWith("GET", "/conversations", { params: { limit: 50, offset: 100, dataset: "test_dataset", }, });
 });
...
describe("PixelatedEmpathyAPI Method getConversations", () => {
 ...
 await api.getConversations({ limit: 50, offset: 10, dataset: "test_dataset", tier: "professional", minQuality: 0.8, });
 expect(calledEndpoint).toBe("/conversations");
 expect(calledOptions.params).toEqual({ limit: 50, offset: 10, dataset: "test_dataset", tier: "professional", min_quality: 0.8, });
 });
 ...
 expect(calledOptions.params).toEqual({ limit: 100, offset: 0, });
 });
 ...
 expect(calledOptions.params).toEqual({ limit: 100, offset: 0, min_quality: 0, });
 });
});
...
describe("PixelatedEmpathyAPI Method validateConversationQuality", () => {
 ...
 return { statusCode: 200, headers: {}, body: '{"success":true,"data":{"valid":true,"score":0.95}}', success: true, data: { valid: true, score: 0.95 }, };
 });
 const conversation = {
...
describe("PixelatedEmpathyAPI Method submitProcessingJob", () => {
 ...
 expect(calledOptions.data).toEqual({ dataset_name: "my-dataset", processing_type: "export", parameters: { format: "csv" }, });
 });
});
...
describe("PixelatedEmpathyAPI Method exportData", () => {
 ...
 expect(calledOptions.data).toEqual({ dataset: "my-dataset", format: "jsonl", });
 });
 ...
 expect(calledOptions.data).toEqual({ dataset: "my-dataset", format: "csv", tier: "premium", min_quality: 0.9, });
...
describe("PixelatedEmpathyAPI Method searchConversations", () => {
 ...
 expect(calledOptions.data).toEqual({ query: "anxiety", filters: {}, limit: 100, offset: 0, });
 ...
 await api.searchConversations("depression", { filters: { tier: "professional" }, limit: 10, offset: 20, });
 expect(calledOptions.data).toEqual({ query: "depression", filters: { tier: "professional" }, limit: 10, offset: 20, });
 });
});
describe("PixelatedEmpathyAPI Method getQualityMetrics", () => {
 ...
 await api.getQualityMetrics({ dataset: "test_dataset", tier: "professional", });
 expect(calledEndpoint).toBe("/quality/metrics");
 expect(calledOptions.params).toEqual({ dataset: "test_dataset", tier: "professional", });
 });
...
describe("PixelatedEmpathyAPI response parsing helpers", () => {
 ...
 expect(safeParseResponse(jsonStr)).toEqual({ success: true, data: { success: true, data: { id: 1 } }, });
 });
...
describe("PixelatedEmpathyAPI Method httpRequest timeout", () => {
 ...
 try {
 await expect(
 api.httpRequest("http://test.com", { method: "GET", headers: {}, timeout: 10 }),
...
describe("PixelatedEmpathyAPI Method safeParseResponse edge cases", () => {
 ...
 it("should return error object if parsed JSON is an array", () => {
 const api = new PixelatedEmpathyAPI("test_key");
 const jsonStr = '[1, 2, 3]';
 expect(api.safeParseResponse(jsonStr)).toEqual({ success: false, message: "Invalid JSON response", });
 });
...
describe("PixelatedEmpathyAPI Method makeRequest", () => {
 ...
 let calledUrl = "";
 let calledOptions: Record<string, unknown> = {};
 const httpRequestSpy = vi.spyOn(api, "httpRequest").mockImplementation(async (url: string, options?: RequestOptions) => {
 calledUrl = url;
 calledOptions = options ?? {};
 return { statusCode: 200, headers: {}, body: '{"success": true}' };
 });
 
 await api.makeRequest("GET", "/endpoint?initial=true", { params: { valid: "value", num: 42, undef: undefined, nul: null } });
 
 expect(calledUrl).toBe("https://api.pixelatedempathy.com/v1/endpoint?initial=true&valid=value&num=42");
