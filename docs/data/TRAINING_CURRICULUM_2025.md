...
This curriculum implements a **3-phase training approach** optimized for late
2025 best practices:

1. **Phase A: Continued Pretraining** - Domain-adaptive text pretraining
2. **Phase B: Multi-Stage SFT Curriculum** - Supervised fine-tuning with
   curriculum learning
3. **Phase C: Preference Alignment** - DPO/ORPO/SimPO/KTO preference
   optimization
...
Domain-adaptive pretraining on large-scale mental health and therapeutic text to
build foundational knowledge.

### Datasets

- **Video Transcripts**: Tim Fletcher, Doc Snipes, Heidi Priebe, Crappy
  Childhood Fairy transcripts
...
**Output Checkpoint**:
`s3://pixelated-checkpoints/foundation/stage1_foundation/`
...
**Output Checkpoint**:
`s3://pixelated-checkpoints/foundation/stage2_therapeutic_expertise/`
...
- **Context Length**: 4096 tokens
- **Focus**: Crisis intervention, edge case handling, unrestricted content
  robustness
- **Safety Note**: PII stripping enforced, but content intensity retained

**Output Checkpoint**:
`s3://pixelated-checkpoints/foundation/stage3_edge_stress_test/`
...
**Output Checkpoint**:
`s3://pixelated-checkpoints/foundation/stage4_voice_persona/`
...
**Output Checkpoint**:
`s3://pixelated-checkpoints/foundation/stage5_long_running/`
...
**Output Checkpoint**:
`s3://pixelated-checkpoints/foundation/stage6_specialized/`
...
- Generic instruction-following / structured-output corpora derived from
  Nemotron HF datasets (mirrored via `scripts/ingest_nemotron_datasets.py` into
  `external/nemotron/...`).
...
- Nemotron-derived generic reasoning/instruction: 100% for this stage (kept
...
**Recommended**: **ORPO (Odds Ratio Preference Optimization)** or **SimPO
...
1. **Roleplay/Simulator Outputs**: Generate preference pairs from roleplay
   scenarios
2. **Adversarial Prompts**: Use edge case generator to create challenging
   prompts
3. **Human Feedback**: If available, use human preference annotations
4. **Synthetic Preferences**: Generate preferences using reward model or
   LLM-as-judge
...
- **Test Set**: Hard holdout families (long_running_therapy, edge_case_crisis,
  sarcasm, voice_persona)
- **Metrics**:
  - Therapeutic quality
  - Crisis handling
  - Voice consistency
