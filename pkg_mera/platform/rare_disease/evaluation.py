...
class DiagnosisArenaAdapter:
    ...
    def compute(self) -> DiagnosisArenaMetrics:
        ...
        r1 = sum(1 for h, g in zip(self._hits, self._ground_truth, strict=False) if h and h[0] == g)
        r5 = sum(1 for h, g in zip(self._hits, self._ground_truth, strict=False) if g in h[:5])
        r10 = sum(1 for h, g in zip(self._hits, self._ground_truth, strict=False) if g in h[:10])
    ...
    def _compute_mrr(self) -> float:
        recips = []
        for h, g in zip(self._hits, self._ground_truth, strict=False):
