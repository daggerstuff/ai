...
class ConversationEffectivenessPredictor:
    ...
    def _extract_effectiveness_features(self, conversations: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, conv in conversations.iterrows():
            ...
            positive_words = len(
                re.findall(
                    r"\b(good|great|excellent|wonderful|amazing|helpful|thank|appreciate|love|happy|joy)\b",
                    text.lower(),
                )
            )
            negative_words = len(
                re.findall(
                    r"\b(bad|terrible|awful|horrible|hate|angry|frustrated|disappointed|sad|upset)\b",
                    text.lower(),
                )
            )
            empathy_words = len(
                re.findall(
                    r"\b(understand|feel|sorry|empathize|support|care|concern|comfort)\b",
                    text.lower(),
                )
            )
            ...
            professional_terms = len(
                re.findall(
                    r"\b(analysis|evaluation|assessment|methodology|implementation|optimization|strategy|solution)\b",
                    text.lower(),
                )
            )
    ...
    def _calculate_effectiveness_scores(self, conversations: pd.DataFrame) -> pd.Series:
        ...
        for _, conv in conversations.iterrows():
            ...
            professional_terms = len(
                re.findall(
                    r"\b(analysis|evaluation|assessment|methodology|implementation|solution)\b",
                    text.lower(),
                )
            )
    ...
    def _analyze_effectiveness_patterns(
        ...
        for feature in key_features:
            if feature in modeling_data_with_pred.columns:
                ...
                q2_mask = (modeling_data_with_pred[feature] > quartiles[0.25]) & (
                    modeling_data_with_pred[feature] <= quartiles[0.5]
                )
                q3_mask = (modeling_data_with_pred[feature] > quartiles[0.5]) & (
                    modeling_data_with_pred[feature] <= quartiles[0.75]
                )
    ...
    def _generate_effectiveness_insights(
        ...
        if stats["mean_effectiveness"] < 50:
            insights.append(
                f"⚠️ Average effectiveness is low ({stats['mean_effectiveness']:.1f}) - significant improvement needed"
            )
        elif stats["mean_effectiveness"] > 70:
            insights.append(
                f"✅ Good average effectiveness ({stats['mean_effectiveness']:.1f}) - maintain current quality"
            )
    ...
    def _generate_effectiveness_recommendations(self, effectiveness_analysis: dict[str, Any]) -> list[str]:
        ...
        if "empathy_density" in feature_eff:
            ...
            if empathy_corr > 0.2:
                recommendations.append(
                    "❤️ Increase empathetic language - strong positive correlation with effectiveness"
                )
        ...
        if "dialogue_turns" in feature_eff:
            ...
            if turns_corr > 0.1:
                recommendations.append(
                    "💬 Encourage more dialogue turns - interactive conversations are more effective"
                )
    ...
    def _create_effectiveness_visualizations(
        ...
        fig.suptitle(
            "Conversation Effectiveness Prediction Analysis",
            fontsize=16,
            fontweight="bold",
        )
