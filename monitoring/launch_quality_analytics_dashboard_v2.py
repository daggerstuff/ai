    ...
    def check_dependencies(self) -> bool:
            ...
            try:
                if package == "sqlite3":
                    ...
                    if spec:
                        ...
                        raise ImportError("sqlite3 not built-in")
                    continue
                ...
                if min_version and hasattr(module, "__version__"):
                    ...
                    if self._compare_versions(current_version, min_version) < 0:
                        version_issues.append(
                            f"{package} (current: {current_version}, "
        ...
        if version_issues:
            ...
            logger.info(
                ...
                f"{' '.join(p.split()[0] for p in version_issues)}"
            )
            return False
    ...
    def validate_database(self) -> bool:
        ...
        try:
            ...
            if missing_conv_columns:
                logger.error(
                    f"❌ Missing columns in conversations table: "
                    f"{', '.join(missing_conv_columns)}"
                )
                conn.close()
                return False
            ...
            if missing_qual_columns:
                logger.error(
                    f"❌ Missing columns in conversation_quality table: "
                    f"{', '.join(missing_qual_columns)}"
                )
                conn.close()
                return False
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM conversation_quality WHERE "
                "overall_quality IS NOT NULL"
            )
        ...
        try:
            with open(self.dashboard_file, "r") as f:
                ...
                if missing_components:
                    logger.error(
                        f"❌ Missing components in dashboard file: "
    ...
    def launch_dashboard(self, port: int = 8501, host: str = "localhost") -> bool:
        ...
        try:
            ...
            if self.dashboard_process.poll() is None:
                logger.info(
                    f"✅ Dashboard launched successfully! "
