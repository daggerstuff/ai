...
class QualityAnalyticsDashboardLauncher:
    ...
    def check_dependencies(self) -> bool:
            ...
            try:
                ...
                if min_version and hasattr(module, "__version__"):
                    ...
                    if self._compare_versions(current_version, min_version) < 0:
                        ...
                        logger.warning(
                            f"  ⚠️ {package}: Version {current_version} < {min_version}"
                        )
                    else:
        ...
        if version_issues:
            ...
            logger.info(
                f"   pip install --upgrade {' '.join(p.split()[0] for p in version_issues)}"
            )
            return False
    ...
    def validate_database(self) -> bool:
        ...
        try:
            ...
            missing_conv_columns = [
                col for col in required_conv_columns if col not in conv_columns
            ]
            if missing_conv_columns:
                logger.error(
                    f"❌ Missing columns in conversations table: {', '.join(missing_conv_columns)}"
                )
                conn.close()
                return False
            ...
            required_qual_columns = [
                "conversation_id",
                "overall_quality",
                "therapeutic_accuracy",
            ]

            missing_qual_columns = [
                col for col in required_qual_columns if col not in qual_columns
            ]
            if missing_qual_columns:
                logger.error(
                    f"❌ Missing columns in conversation_quality table: {', '.join(missing_qual_columns)}"
                )
                conn.close()
                return False
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM conversation_quality WHERE overall_quality IS NOT NULL"
            )
            ...
            logger.info(
                f"✅ Database validated: {quality_count:,} quality records found"
            )
            conn.close()
            return True
        ...
        try:
            with open(self.dashboard_file, "r") as f:
                ...
                if missing_components:
                    logger.error(
                        f"❌ Missing components in dashboard file: {', '.join(missing_components)}"
                    )
                    return False
    ...
    def launch_dashboard(self, port: int = 8501, host: str = "localhost") -> bool:
        ...
        try:
            ...
            if self.dashboard_process.poll() is None:
                logger.info(
                    f"✅ Dashboard launched successfully! PID: {self.dashboard_process.pid}"
                )
    ...
    def _stop_dashboard(self):
        ...
        if self.dashboard_process and self.dashboard_process.poll() is None:
            ...
            try:
                ...
                logger.warning(
                    "⚠️ Dashboard didn't stop gracefully, forcing termination"
                )
