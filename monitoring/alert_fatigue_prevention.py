...
class AlertFatiguePreventionSystem:
    ...
    def __init__(
        self, db_path: str = "/home/vivi/pixelated/ai/monitoring/alert_fatigue.db"
    ):
    ...
    async def _find_or_create_group(
        self, fingerprint: AlertFingerprint, alert_data: Dict[str, Any]
    ) -> AlertGroup:
        ...
        for group in self.active_groups.values():
            if (
                ...
                time_window = timedelta(
                    minutes=self.grouping_config["time_window_minutes"]
                )
                if datetime.utcnow() - group.last_seen <= time_window:
    ...
    def _calculate_similarity(
        self, fp1: AlertFingerprint, fp2: AlertFingerprint
    ) -> float:
    ...
    async def _apply_fatigue_rules(
        self, group: AlertGroup, alert_data: Dict[str, Any]
    ) -> List[str]:
        ...
        for rule in sorted_rules:
            ...
            if await self._evaluate_rule_conditions(rule, group, alert_data):
                ...
                logger.info(
                    f"Applied fatigue rule '{rule.name}' to group {group.group_id}"
                )
    ...
    async def _evaluate_rule_conditions(
        self, rule: FatigueRule, group: AlertGroup, alert_data: Dict[str, Any]
    ) -> bool:
    ...
    async def _update_group_state(self, group: AlertGroup, actions: List[str]):
        ...
        group.metadata.update(
            {"last_actions": actions, "last_updated": datetime.utcnow().isoformat()}
        )
    ...
    async def _log_rule_application(
        self, rule: FatigueRule, group: AlertGroup, actions: List[str]
    ):
    ...
    async def get_group_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "group_statistics": [
                {"state": s[0], "groups": s[1], "total_alerts": s[2]} for s in stats
            ],
            "top_sources": [
                {"source": s[0], "groups": s[1], "alerts": s[2]} for s in top_sources
            ],
            "suppression_actions": [
                {"rule": s[0], "action": s[1], "count": s[2]} for s in suppressions
            ],
