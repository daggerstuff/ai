...
class EvaluationGate:
    ...
    def evaluate(self, metric_value: float) -> Tuple[bool, str]:
        ...
        if self.gate_type == GateType.MINIMUM_THRESHOLD:
            ...
            reason = (
                f"Value {metric_value} {'meets' if passed else 'fails'} "
                f"minimum threshold {self.threshold_value}"
            )
        elif self.gate_type == GateType.MAXIMUM_THRESHOLD:
            passed = metric_value <= self.threshold_value
            reason = (
                f"Value {metric_value} {'meets' if passed else 'fails'} "
                f"maximum threshold {self.threshold_value}"
            )
        elif self.gate_type == GateType.RANGE_THRESHOLD:
            if self.secondary_threshold is not None:
                passed = (
                    self.threshold_value <= metric_value <= self.secondary_threshold
                )
                reason = (
                    f"Value {metric_value} {'is in' if passed else 'is out of'} "
                    f"range [{self.threshold_value}, {self.secondary_threshold}]"
                )
            else:
                passed = False
                reason = "Range threshold requires secondary threshold value, got None"
        elif self.gate_type == GateType.COMPARISON_THRESHOLD:
            # For comparison, threshold_value is typically 0 for
            # "no worse than baseline"
            # Positive values mean it can be that much worse
            passed = metric_value <= self.threshold_value
            reason = (
                f"Value {metric_value} {'meets' if passed else 'fails'} "
                f"comparison threshold {self.threshold_value}"
            )
        else:
            passed = False
...
class GateConfiguration:
    ...
    )  # Gates that are always critical
    metadata: Optional[Dict[str, Any]] = None
...
class HealthCheckManager:
    ...
    def __init__(self, service_name: str = "pixelated-empathy-ai"):
        ...
        self.logger.info(
            f"HealthCheckManager initialized for service: {self.service_name}"
        )

    def register_component(
        self, name: str, component: Any, component_type: Optional[str] = None
    ):
        ...
        self.logger.info(
            "Registered component for health monitoring: %s (type: %s)",
            name,
            component_type or "generic",
        )
        ...
        try:
            ...
            for i in range(gpu_count):
                try:
                    ...
                    memory_percent = (
                        (gpu_memory / gpu_memory_total) * 100
    ...
    def _check_memory_status(self) -> ComponentHealth:
        ...
        try:
            ...
            process_memory_percent = (
                process_memory_info.rss / psutil.virtual_memory().total * 100
            )
        ...
        try:
            ...
            for model_name in models:
                try:
                    ...
                    model_issues.append(
                        f"Model {model_name} check failed: {model_error}"
                    )
    ...
    def perform_health_check(self) -> HealthCheckResult:
            ...
            try:
                ...
                if component_health.status == ComponentStatus.FAILED:
                    critical_issues.append(
                        f"{check_name}: "
                        f"{component_health.last_error or 'Component failed'}"
                    )
                    self.logger.error(
                        "Health check failed for %s: %s",
                        check_name,
                        component_health.last_error,
                    )
                ...
                if component_health.component_type:
                    if (
                        component_health.component_type == "gpu"
                        and component_health.health_score < 0.8
                    ):
                        recommendations.append(
                            "Consider GPU maintenance or replacement"
                        )
                    elif (
                        component_health.component_type == "memory"
                        and component_health.health_score < 0.7
                    ):
                        recommendations.append(
                            "Check memory usage and consider optimization"
                        )
                    elif (
                        component_health.component_type == "disk"
                        and component_health.health_score < 0.6
                    ):
        ...
        if component_results:
            ...
            overall_score = (
                sum(weighted_scores) / total_weight if total_weight > 0 else 1.0
            )
        ...
        for comp_health in component_results.values():
            ...
            component_health_summary[status_key] = (
                component_health_summary.get(status_key, 0) + 1
            )
        ...
        self.logger.debug(
            "Health check completed with status: %s, score: %.3f",
    ...
    def initiate_graceful_shutdown(
        self, reason: str = "manual_shutdown", force: bool = False
    ) -> ShutdownResult:
        ...
        self.logger.info(
            "Initiating graceful shutdown for %s (reason: %s)...",
            self.service_name,
            reason,
        )
        ...
        try:
            ...
            if not drained_successfully:
                warning_messages.append(
                    "Some requests may not have completed before shutdown"
                )
            shutdown_components.append("request_draining")

            # 3. Call registered shutdown callbacks in order
            self.logger.info(
                f"Calling {len(self.shutdown_callbacks)} shutdown callbacks..."
            )
            ...
            try:
                ...
                if model_manager:
                    ...
                    if unload_result.success:
                        ...
                        self.logger.info(
                            f"Successfully unloaded {unload_result.models_unloaded} "
                            f"models"
                        )
                    else:
                        error_msg = (
                            f"Failed to unload models: {unload_result.error_message}"
                        )
            ...
            try:
                ...
                if closed_connections:
                    ...
                    warning_messages.append(
                        "Some database connections may not have closed properly"
                    )
            except Exception as e:
            ...
            try:
                ...
                if metrics_exported:
                    ...
                    warning_messages.append(
                        "Some metrics may not have exported completely"
                    )
            except Exception as e:
            ...
            try:
                self.logger.info(
                    "Graceful shutdown sequence starting for %s", self.service_name
                )
                ...
                if notification_result.success:
                    ...
                    warning_messages.append(
                        f"Monitoring notification had issues: "
            ...
            try:
                ...
                if cleanup_performed:
                    ...
                    warning_messages.append(
                        "Final cleanup may not have completed fully"
                    )
            except Exception as e:
            ...
            if success:
                ...
                self.logger.warning(
                    f"⚠️ Graceful shutdown completed with "
                    f"{len(failed_components)} failed components in "
                    f"{duration:.2f} seconds"
                )
            ...
            self.logger.info(
                "Graceful shutdown of %s completed in %.2f seconds",
                self.service_name,
                duration,
            )
            return ShutdownResult(
            ...
            try:
                ...
                emergency_error_msg = (
                    f"Emergency cleanup failed: {str(emergency_error)}"
                )
    ...
    def _drain_existing_requests(self, timeout_seconds: int = 30) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Draining existing requests with timeout of {timeout_seconds} seconds"
            )

            # Simulate draining
            time.sleep(
                min(2, timeout_seconds / 10)
            )  # Don't actually wait long in testing
    ...
    def _execute_shutdown_callbacks(self) -> Dict[str, List[str]]:
        ...
        for i, callback in enumerate(self.shutdown_callbacks):
            try:
                self.logger.info(
                    "Executing shutdown callback %d/%d",
                    i + 1,
                    len(self.shutdown_callbacks),
                )
                callback()
                successful_callbacks.append(f"callback_{i}")
                self.logger.info("Shutdown callback %d completed successfully", i + 1)
            except Exception as e:
    ...
    def _flush_logs(self) -> bool:
        ...
        try:
            ...
            self.logger.error("Error flushing logs: %s", e)
            return False
    ...
    def _notify_monitoring_systems(self, reason: str) -> Any:
        ...
        try:
            ...
            self.logger.info(
                f"Notified monitoring systems of shutdown (reason: {reason})"
            )
            ...
            try:
                ...
                for thread in threading.enumerate():
                    if thread != main_thread and thread.is_alive():
                        self.logger.info(
                            "Thread %s still alive, waiting...", thread.name
                        )
    ...
    def get_component_health(self, component_name: str) -> Optional[ComponentHealth]:
        ...
        if (
            ...
            and component_name in self.last_health_check.components
        ):
...
class HealthCheckMiddleware:
    ...
    def health_check_endpoint(self):
        ...
        if health_result.status == HealthStatus.HEALTHY:
            ...
            status_code = (
                200  # Degraded is still considered "healthy" but with warnings
            )
        else:
    ...
    def readiness_probe_endpoint(self):
        ...
        response_data = {
            ...
            "status": health_result.status.value,
            "timestamp": health_result.timestamp,
            "reason": "Service is ready to serve requests"
            if is_ready
...
def integrate_health_checks_with_fastapi(app):
    ...
    async def health_check():
        ...
        status_code, response_data = HealthCheckMiddleware(
            health_manager
...
def integrate_with_asgi_server(server_type: str = "uvicorn"):
    ...
    def shutdown_handler(signum, frame):
        ...
        if result.success:
            ...
            logger.error(
                f"Server shutdown completed with errors: {result.error_messages}"
            )
...
def test_health_check_system():
    ...
    for component_name, component_health in health_result.components.items():
        print(
            ...
            f"(score: {component_health.health_score:.2f})"
        )
    ...
    # Note: We won't actually call initiate_graceful_shutdown here as it
    # would shut down the test
