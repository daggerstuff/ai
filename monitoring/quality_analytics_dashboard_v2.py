...
class QualityAnalyticsDashboard:
    ...
    def calculate_quality_analytics(self, df: pd.DataFrame) -> QualityAnalytics:
        ...
        try:
            ...
            recommendations = self._generate_recommendations(
                df, average_quality, tier_performance, component_performance
            )
        ...
        try:
            ...
            if avg_quality < 0.5:
                recommendations.append(
                    "🚨 CRITICAL: Overall quality is critically low (< 0.5). Immediate intervention required."
                )
            elif avg_quality < 0.6:
                recommendations.append(
                    "⚠️ WARNING: Overall quality is below acceptable threshold (< 0.6). Review data sources and processing."
                )
            ...
            for tier, metrics in tier_performance.items():
                ...
                if avg_tier_quality < 0.5:
                    recommendations.append(
                        f"🔴 CRITICAL: Tier '{tier}' quality ({avg_tier_quality:.3f}) is critically low with {count:,} conversations. Urgent review needed."
                    )
                elif avg_tier_quality < 0.6:
                    recommendations.append(
                        f"🟡 WARNING: Tier '{tier}' quality ({avg_tier_quality:.3f}) needs improvement ({count:,} conversations)."
                    )
                elif count < 100:
                    recommendations.append(
                        f"📊 INFO: Tier '{tier}' has limited data ({count} conversations). Consider expanding dataset."
                    )
            ...
            if total_conversations < 1000:
                recommendations.append(
                    f"📈 SCALE: Dataset is relatively small ({total_conversations:,} conversations). Consider expanding for better insights."
                )
            ...
            if len(df) > 7:  # Need at least a week of data
                ...
                if len(recent_week) > 0 and len(older_week) > 0:
                    ...
                    if change < -0.05:
                        recommendations.append(
                            f"📉 TREND: Quality declining over past week ({change:.3f}). Investigate recent changes."
                        )
                    elif change > 0.05:
                        recommendations.append(
                            f"📈 TREND: Quality improving over past week (+{change:.3f}). Continue current practices."
                        )
    ...
    def run_streamlit_dashboard(self):
        ...
        # Main header
        st.markdown(
            '<h1 class="main-header">📊 Quality Analytics Dashboard</h1>',
            unsafe_allow_html=True,
        )
        ...
        with st.spinner("🔄 Loading quality data..."):
            try:
                ...
                if df.empty:
                    st.error(
                        "❌ No quality data available with current filters. Please adjust your filters or ensure quality validation has been run."
                    )
                    st.stop()
