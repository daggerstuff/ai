...
class QualityDistributionAnalyzer:
    ...
    def __init__(self, db_path: str = "database/conversations.db"):
        ...
        # Analysis categories
        self.analysis_categories = ["tier", "dataset_source", "processing_status"]
    ...
    def _detect_distribution_type(self, values: list[float]) -> str:
        ...
        try:
            ...
            try:
                _shapiro_stat, shapiro_p = stats.shapiro(test_values)
                if shapiro_p > 0.05:
                    return "normal"
            except:
                pass
    ...
    def create_distribution_visualizations(
        ...
        try:
            ...
            for metric, metric_distributions in distributions.items():
                ...
                fig.suptitle(
                    f"{metric.replace('_', ' ').title()} Distribution Analysis",
                    fontsize=16,
                    fontweight="bold",
                )
                ...
                for i, (category, analysis) in enumerate(metric_distributions.items()):
                    ...
                    ax.hist(
                        analysis.values,
                        bins=min(30, len(analysis.values) // 2),
                        alpha=0.7,
                        density=True,
                        edgecolor="black",
                    )
...
def main():
    ...
    # Export report
    analyzer.export_distribution_report(distributions, visualizations)
