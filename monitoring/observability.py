    ...
    def redact_text(self, text: str) -> str:
        ...
        for pattern_name, pattern in self.redaction_patterns.items():
            redacted_text = re.sub(
                pattern,
                f"{self.placeholder}_{pattern_name.upper()}",
                redacted_text,
                flags=re.IGNORECASE,
            )
    ...
    def redact_dict(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, str):
                ...
                redacted_data[key] = [
                    self.redact_text(item) if isinstance(item, str) else item
                    for item in value
                ]
            else:
...
class Logger:
    ...
    def log(
        ...
        **extra_fields,
        ...
        redacted_context = (
            self.redaction_engine.redact_dict(context) if context else None
        )
        redacted_extra = (
            self.redaction_engine.redact_dict(extra_fields) if extra_fields else None
        )
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        if torch.cuda.is_available():
            ...
            for i in range(torch.cuda.device_count()):
                metrics[f"gpu_{i}_memory_gb"] = torch.cuda.memory_allocated(i) / (
                    1024**3
                )
                ...
                metrics[f"gpu_{i}_utilization"] = (
                    torch.cuda.utilization(i) if torch.cuda.utilization else 0
                )
...
class Tracer:
    ...
    def end_span(self, span: Span, status: str = "success"):
        ...
        if (
            span.trace_id in self.active_spans
            and self.active_spans[span.trace_id] == span.span_id
        ):
...
class ObservabilityManager:
    ...
    def log_request(
        ...
        if processing_time_ms is not None:
            self.metrics_collector.record_histogram(
                "request_latency_ms", processing_time_ms
            )
    ...
    def log_model_inference(
        ...
        self.metrics_collector.record_histogram(
            "model_inference_latency_ms", processing_time_ms
        )
        ...
        if success:
            ...
            if error_type:
                self.metrics_collector.increment_counter(
                    f"model_inferences_error_{error_type}"
                )
    ...
    def get_service_metrics(self) -> Dict[str, Any]:
        ...
        requests_processed = self.metrics_collector.counters.get(
            "requests_processed", 0
        )
        request_errors = self.metrics_collector.counters.get("request_errors", 0)
        error_rate = (
            request_errors / requests_processed if requests_processed > 0 else 0.0
        )
    ...
    def end_traced_request(
        self, span: Span, status: str = "success", error_message: Optional[str] = None
    ):
...
def test_observability():
    ...
    # Test tracing
    span = observability.create_traced_request(
        "test_operation", "user_123", "test_model"
    )
