...
class QualityAnalyticsDashboard:
    ...
    def load_quality_data(self, force_refresh: bool = False) -> pd.DataFrame:
        ...
        if (
            ...
            and (current_time - self._last_cache_time).seconds < self.cache_duration
        ):
    ...
    def _generate_recommendations(
        ...
        if lowest_component[1] < 0.6:
            recommendations.append(
                f"📊 Focus on improving {lowest_component[0].replace('_', ' ')} (score: {lowest_component[1]:.3f})"
            )
        ...
        if dataset_quality[worst_dataset] < 0.5:
            recommendations.append(
                f"📁 Dataset '{worst_dataset}' has poor quality ({dataset_quality[worst_dataset]:.3f}). Consider data cleaning."
            )
    ...
    def run_dashboard(self):
        ...
        with col4:
            ...
            st.metric("Quality Anomalies", anomaly_count, delta=None)
