...
class TextPatternExtractor:
    ...
    def __init__(self):
        self.common_patterns = [
            ...
            (
                r"timeout\s+(?:after\s+)?(\d+(?:\.\d+)?)\s*(ms|s|seconds?|minutes?)",
                "timeout_duration",
            ),
            # Resource patterns
            ...
            # Service patterns
            (
                r"service\s+([a-zA-Z0-9_-]+)\s+(?:is\s+)?(?:down|unavailable|failed)",
                "failed_service",
            ),
            (
                r"([a-zA-Z0-9_-]+)\s+service\s+(?:is\s+)?(?:down|unavailable|failed)",
                "failed_service",
            ),
            # Database patterns
            (
                r"database\s+connection\s+(?:to\s+)?([a-zA-Z0-9_-]+)\s+failed",
                "db_connection_target",
            ),
            (
                r"query\s+timeout\s+(?:after\s+)?(\d+(?:\.\d+)?)\s*(ms|s|seconds?)",
                "query_timeout",
            ),
            # Queue patterns
            (
                r"queue\s+([a-zA-Z0-9_-]+)\s+(?:has\s+)?(\d+)\s+(?:pending\s+)?(?:items?|messages?)",
                "queue_info",
            ),
    ...
    def extract_patterns(self, text: str) -> Dict[str, Any]:
        ...
        if numbers:
            patterns["numeric_values"] = [
                float(n) for n in numbers[:5]
            ]  # Limit to first 5
...
class IntelligentGroupingEngine:
    ...
    async def initialize(self, historical_alerts: List[Dict[str, Any]]):
        ...
        if historical_alerts:
            ...
            logger.info(
                f"Initialized grouping engine with {len(historical_alerts)} historical alerts"
            )

    async def suggest_groups(
        self, alerts: List[Dict[str, Any]], algorithm: str = "hybrid_approach"
    ) -> List[List[int]]:
        ...
        logger.info(
            f"Grouped {len(alerts)} alerts into {len(groups)} groups using {algorithm}"
        )
        return groups
    ...
    def _calculate_categorical_similarity(
        self, features: List[AlertFeatures]
    ) -> np.ndarray:
            ...
            if len(group) >= 2:  # Only keep groups with multiple alerts
                ...
                for existing_group in filtered_groups:
                    if self._should_merge_groups(
                        [features[group[0]]], [features[existing_group[0]]]
                    ):
    ...
    def _should_merge_groups(
        self, group1_features: List[AlertFeatures], group2_features: List[AlertFeatures]
    ) -> bool:
        ...
        for key in set(f1.categorical_features.keys()) | set(
            f2.categorical_features.keys()
        ):
            cat_total += 1
            if (
                ...
                first_timestamp = alert_times[
                    alert_times.index(
                        (
                            current_group[0],
                            next(
                                t for idx, t in alert_times if idx == current_group[0]
                            ),
                        )
                    )
                ][1]
    ...
    async def _hybrid_approach(
        ...
        for group in similarity_groups:
            ...
            pattern_subgroups = await self._pattern_matching(
                group_alerts, group_features
            )
        ...
        for group in refined_groups:
            ...
            temporal_subgroups = await self._temporal_clustering(
                group_alerts, group_features
            )
