...
class QualityDistributionComparator:
    ...
    def compare_across_time_periods(
        self, df: pd.DataFrame, metric: str, period_type: str = "month"
    ) -> ComparativeDistributionAnalysis:
    ...
    def _generate_correlation_summary(self, correlations: list[dict[str, Any]]) -> list[str]:
        ...
        if strongest_pos:
            summary.append(
                f"🔗 Strongest positive correlation: {strongest_pos['component1']} ↔ {strongest_pos['component2']} (r={strongest_pos['correlation']:.3f})"
            )
        ...
        if strongest_neg:
            summary.append(
                f"🔗 Strongest negative correlation: {strongest_neg['component1']} ↔ {strongest_neg['component2']} (r={strongest_neg['correlation']:.3f})"
            )
