...
class TierDistributionOptimizer:
    ...
    def _analyze_individual_tier(
        ...
        try:
            ...
            dataset_distribution = (
                tier_subset["dataset_source"].value_counts().to_dict()
            )

            # Processing efficiency
            total_conversations = len(tier_subset)
            processed_conversations = len(
                tier_subset[tier_subset["processing_status"] == "processed"]
            )
            ...
            # Characteristics
            characteristics = self._identify_tier_characteristics(
                tier_subset, tier_name
            )
    ...
    def _calculate_tier_quality_metrics(
        ...
        try:
            ...
            completeness_score = len(tier_data[tier_data["word_count"] > 0]) / len(
                tier_data
            )
    ...
    def _identify_tier_characteristics(
        ...
        try:
            ...
            processing_rate = len(
                tier_data[tier_data["processing_status"] == "processed"]
            ) / len(tier_data)
    ...
    def generate_tier_optimization(
        ...
        try:
            ...
            resource_allocation = self._calculate_resource_allocation(
                tier_analyses, optimal_distribution
            )
    ...
    def _calculate_optimal_distribution(
        ...
        try:
            ...
            for tier, analysis in tier_analyses.items():
                ...
                if target_key:
                    optimal_dist[tier] = (
                        self.optimization_config["target_distributions"][target_key]
                        * 100
                    )
                else:
            ...
            if total_optimal > 0:
                optimal_dist = {
                    tier: (pct / total_optimal) * 100
                    for tier, pct in optimal_dist.items()
                }
                ...
                if abs(difference) > 5:  # 5% threshold
                    if difference > 0:
                        recommendations.append(
                            f"Increase {tier} representation by {difference:.1f}% to reach optimal distribution"
                        )
                    else:
                        recommendations.append(
                            f"Reduce {tier} representation by {abs(difference):.1f}% to optimize balance"
                        )

            # General recommendations
            if len(recommendations) > 3:
                recommendations.append(
                    "Consider gradual rebalancing over multiple phases to minimize disruption"
                )

            if not recommendations:
                recommendations.append(
                    "Current tier distribution is well-balanced - maintain current ratios"
                )
    ...
    def _generate_quality_improvements(
        ...
        try:
            for tier, analysis in tier_analyses.items():
                ...
                for target_tier, target_threshold in self.optimization_config[
                    "quality_thresholds"
                ].items():
                    if target_tier.lower() in tier.lower():
                ...
                if overall_quality < threshold:
                    ...
                    improvements.append(
                        f"Improve {tier} quality by {gap:.2f} points to meet threshold"
                    )

                # Processing efficiency improvements
                if analysis.processing_efficiency < 90:
                    improvements.append(
                        f"Address processing issues in {tier} (current: {analysis.processing_efficiency:.1f}%)"
                    )

                # Content improvements
                if analysis.avg_word_count < 50:
                    improvements.append(
                        f"Enhance content depth in {tier} - average word count is low"
                    )

            # General improvements
            if len(improvements) == 0:
                improvements.append(
                    "All tiers meet quality standards - focus on maintaining excellence"
                )
    ...
    def _calculate_resource_allocation(
        ...
        try:
            ...
            for tier, analysis in tier_analyses.items():
                ...
                quality_effort = (
                    max(0, 0.8 - analysis.quality_metrics.get("overall_quality", 0.8))
                    * 100
                )
            ...
            return {
                ...
                "optimization_timeline_weeks": max(
                    4,
                    len(
                        [
                            e
                            for e in effort_allocation.values()
                            if e["priority"] == "high"
                        ]
                    )
                    * 2,
        ...
        try:
            ...
            return {
                ...
                "distribution_balance_score": float(
                    1.0 - np.std(list(optimal_dist.values())) / 100
                ),
                "overall_system_improvement": float(
                    (quality_improvement + processing_improvement) / 2
                ),
            }
    ...
    def create_tier_visualizations(
        ...
        try:
            ...
            fig.suptitle(
                "Tier Distribution Analysis & Optimization",
                fontsize=16,
                fontweight="bold",
            )
            ...
            optimal_values = [
                optimization.optimal_distribution.get(tier, 0) for tier in tiers
            ]
            ...
            ax.set_xticks(x)
            ax.set_xticklabels(
                [t.replace("_", " ").title() for t in tiers], rotation=45, ha="right"
            )
            ax.legend()
            ...
            ax = axes[0, 1]
            quality_scores = [
                analysis.quality_metrics.get("overall_quality", 0)
                for analysis in tier_analyses.values()
            ]
            tier_names = list(tier_analyses.keys())

            bars = ax.bar(
                range(len(tier_names)), quality_scores, alpha=0.7, color="green"
            )
            ...
            ax = axes[1, 1]
            processing_rates = [
                analysis.processing_efficiency for analysis in tier_analyses.values()
            ]

            bars = ax.bar(
                range(len(tier_names)), processing_rates, alpha=0.7, color="orange"
            )
            ...
            ax.axhline(
                y=90, color="red", linestyle="--", alpha=0.7, label="Target (90%)"
            )
            ax.legend()
            ax.grid(True, alpha=0.3)
    ...
    def export_tier_optimization_report(
        ...
        try:
            ...
            export_data = {
                ...
                "executive_summary": self._create_tier_executive_summary(
                    tier_analyses, optimization
                ),
            }
    ...
    def _create_tier_executive_summary(
        ...
        try:
            ...
            most_represented = max(
                optimization.current_distribution.items(), key=lambda x: x[1]
            )
            ...
            return {
                ...
                "timeline_weeks": optimization.resource_allocation.get(
                    "optimization_timeline_weeks", 8
                ),
            }
