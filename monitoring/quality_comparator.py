...
class QualityComparator:
    ...
    def load_comparison_data(
        self, days_back: int = 90, force_refresh: bool = False
    ) -> pd.DataFrame:
        ...
        if (
            ...
            and (current_time - self._cache_timestamps[cache_key]).seconds
            < self.cache_duration
        ):
    ...
    def compare_tiers(
        self, df: pd.DataFrame, metric: str = "overall_quality"
        ...
        for i, tier1 in enumerate(tiers):
            for tier2 in tiers[i + 1 :]:
                ...
                if (
                    len(tier1_data) >= self.min_sample_size
                    and len(tier2_data) >= self.min_sample_size
                ):
    ...
    def compare_datasets(
        ...
        for i, dataset1 in enumerate(datasets):
            for dataset2 in datasets[i + 1 :]:
                ...
                if (
                    len(dataset1_data) >= self.min_sample_size
                    and len(dataset2_data) >= self.min_sample_size
                ):
                    comparison = self._perform_comparison(
    ...
    def compare_components(self, df: pd.DataFrame) -> List[QualityComparison]:
        ...
        available_components = [
            comp for comp in self.quality_components if comp in df.columns
        ]
        ...
        for i, comp1 in enumerate(available_components):
            for comp2 in available_components[i + 1 :]:
                ...
                if (
                    len(comp1_data) >= self.min_sample_size
                    and len(comp2_data) >= self.min_sample_size
                ):
                    comparison = self._perform_comparison(
    ...
    def _generate_comparison_recommendations(
        ...
        if abs(mean_diff) > 0.05:  # 5% threshold
            ...
            recommendations.append(
                f"📊 {better_name} outperforms {worse_name} by {abs(mean_diff):.3f} points"
            )

        # Statistical significance
        significant_tests = [
            test for test in statistical_tests if test.get("significant", False)
        ]
        if significant_tests:
            recommendations.append(
                "📈 Difference is statistically significant - results are reliable"
            )
        else:
            recommendations.append(
                "⚠️ Difference lacks statistical significance - collect more data"
            )
        ...
        if effect_size >= self.effect_size_thresholds["large"]:
            ...
            recommendations.append(
                f"📊 Medium effect size ({effect_size:.3f}) indicates moderate practical difference"
            )
        elif effect_size >= self.effect_size_thresholds["small"]:
            recommendations.append(
                f"📈 Small effect size ({effect_size:.3f}) indicates limited practical difference"
            )
        else:
            recommendations.append(
                f"📊 Negligible effect size ({effect_size:.3f}) - difference may not be meaningful"
            )
        ...
        if comparison_type == "tier":
            if mean_diff > 0.1:
                recommendations.append(
                    "🎯 Consider applying best practices from higher-performing tier"
                )
        elif comparison_type == "dataset":
            if mean_diff > 0.1:
                recommendations.append(
                    "📁 Review data quality and processing methods for lower-performing dataset"
                )
    ...
    def _generate_benchmark_recommendations(
        ...
        if performance_gap > 0.05:  # Above benchmark
            recommendations.append(
                f"✅ Performance exceeds benchmark by {performance_gap:.3f} points"
            )
            recommendations.append(
                "🏆 Consider sharing best practices with underperforming groups"
            )
        elif performance_gap < -0.05:  # Below benchmark
            recommendations.append(
                f"⚠️ Performance below benchmark by {abs(performance_gap):.3f} points"
            )
            recommendations.append(
                "📈 Implement improvement initiatives to reach industry standards"
            )
        else:
