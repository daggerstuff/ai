...
class ConversationContentAnalyzer:
    ...
    def _analyze_individual_conversation(
        ...
        try:
            ...
            therapeutic_indicators = self._identify_therapeutic_indicators(
                all_text.lower()
            )
    ...
    def _analyze_sentiment_indicators(self, text: str) -> Dict[str, int]:
        ...
        for sentiment_type, keywords in self.analysis_config[
            "sentiment_keywords"
        ].items():
    ...
    def _calculate_content_quality_score(
        ...
        try:
            ...
            sentiment_score = min(
                1.0, sum(sentiment_indicators.values()) / 10
            )  # Normalize to 10 indicators
    ...
    def _generate_content_insights(
        self, dataset_name: str, content_analyses: List[ContentAnalysis]
    ) -> ContentInsights:
        ...
        try:
            ...
            avg_vocabulary_richness = np.mean(
                [a.vocabulary_richness for a in content_analyses]
            )
            avg_quality_score = np.mean(
                [a.content_quality_score for a in content_analyses]
            )
            ...
            content_patterns = {
                ...
                "word_count_distribution": {
                    ...
                    "medium": len(
                        [a for a in content_analyses if 50 <= a.word_count < 200]
                    ),
            ...
            common_themes = [
                keyword for keyword, count in keyword_counts.most_common(15)
            ]
            ...
            vocabulary_analysis = {
                "richness_distribution": {
                    "low": len(
                        [a for a in content_analyses if a.vocabulary_richness < 0.3]
                    ),
                    "medium": len(
                        [
                            a
                            for a in content_analyses
                            if 0.3 <= a.vocabulary_richness < 0.6
                        ]
                    ),
                    "high": len(
                        [a for a in content_analyses if a.vocabulary_richness >= 0.6]
                    ),
                },
            ...
            quality_distribution = {
                "low": len(
                    [a for a in content_analyses if a.content_quality_score < 0.4]
                ),
                "medium": len(
                    [
                        a
                        for a in content_analyses
                        if 0.4 <= a.content_quality_score < 0.7
                    ]
                ),
                "high": len(
                    [a for a in content_analyses if a.content_quality_score >= 0.7]
                ),
            }
        ...
        try:
            ...
            if avg_words < 50:
                recommendations.append(
                    "Consider enhancing conversation depth - average word count is low"
                )
            elif avg_words > 500:
                recommendations.append(
                    "Monitor for overly verbose conversations that may reduce engagement"
                )
            ...
            if avg_richness < 0.3:
                recommendations.append(
                    "Improve vocabulary diversity to enhance conversation quality"
                )
            elif avg_richness > 0.8:
                recommendations.append(
                    "Excellent vocabulary diversity - maintain current standards"
                )

            # Quality distribution recommendations
            low_quality_pct = (
                quality_distribution.get("low", 0) / total_analyzed
            ) * 100
            if low_quality_pct > 30:
                recommendations.append(
                    "High percentage of low-quality conversations - review content standards"
                )

            high_quality_pct = (
                quality_distribution.get("high", 0) / total_analyzed
            ) * 100
            if high_quality_pct > 60:
                recommendations.append(
                    "Strong content quality - consider using as training examples"
                )

            # General recommendations
            if total_analyzed < 100:
                recommendations.append(
                    "Increase sample size for more comprehensive content analysis"
                )
    ...
    def export_content_analysis_report(
        self, content_insights: Dict[str, ContentInsights]
    ) -> str:
    ...
    def _create_summary_statistics(
        ...
        try:
            ...
            total_conversations = sum(
                insights.total_conversations_analyzed
            ...
            for insights in content_insights.values():
                if "average_quality_score" in insights.content_patterns:
                    all_quality_scores.append(
                        insights.content_patterns["average_quality_score"]
                    )
...
def main():
    ...
    # Display summary
    total_analyzed = sum(
        insights.total_conversations_analyzed for insights in content_insights.values()
    )
    ...
    if content_insights:
        ...
        print(
            f"   - Conversations analyzed: {sample_insights.total_conversations_analyzed}"
        )
        print(
            f"   - Average word count: {sample_insights.content_patterns.get('average_word_count', 0):.1f}"
        )
        print(
            f"   - Average quality score: {sample_insights.content_patterns.get('average_quality_score', 0):.3f}"
        )
