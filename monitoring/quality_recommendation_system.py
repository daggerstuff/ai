...
class QualityRecommendationSystem:
            ...
            for metric in self.quality_metrics:
                ...
                if metric == "safety_score" and pass_rate < 95:
                    analysis["critical_issues"].append(
                        f"Safety validation below critical threshold ({pass_rate:.1f}%)"
                    )
                elif pass_rate < 70:
                    analysis["critical_issues"].append(
    ...
    def _generate_recommendations(
        ...
        try:
            ...
            for metric, performance in quality_analysis.get(
                "metric_performance", {}
            ).items():
                status = performance["status"]

                if status in ["critical", "low"]:
                    recommendation = self._create_metric_recommendation(
            ...
            recommendations.sort(
                key=lambda x: (
                    ...
                    {"immediate": 4, "short_term": 3, "long_term": 2, "strategic": 1}[
                        x.category
                    ],
                ),
                reverse=True,
    ...
    ) -> Optional[QualityRecommendation]:
    ...
    def _calculate_resource_allocation(
        ...
        return {
            ...
            "timeline_distribution": {
                "immediate": len(
                    [r for r in recommendations if r.category == "immediate"]
                ),
                "short_term": len(
                    [r for r in recommendations if r.category == "short_term"]
                ),
                "long_term": len(
                    [r for r in recommendations if r.category == "long_term"]
                ),
                "strategic": len(
                    [r for r in recommendations if r.category == "strategic"]
                ),
            },
        }
    ...
    def export_recommendation_plan(self, plan: RecommendationPlan) -> str:
        ...
        try:
            ...
            report_file = (
                self.output_dir / f"quality_recommendation_plan_{timestamp}.json"
            )
            ...
            export_data = {
                ...
                "executive_summary": {
                    "critical_recommendations": len(
                        [r for r in plan.recommendations if r.priority == "critical"]
                    ),
                    "high_priority_recommendations": len(
                        [r for r in plan.recommendations if r.priority == "high"]
                    ),
                    "estimated_timeline_days": (
                        max([r.timeline_days for r in plan.recommendations])
                        if plan.recommendations
                        else 0
                    ),
                    "total_affected_metrics": len(
                        set(
                            [
                                m
                                for r in plan.recommendations
                                for m in r.affected_metrics
                            ]
                        )
                    ),
                },
...
def main():
    ...
    print(
        f"   - Critical priority: {len([r for r in plan.recommendations if r.priority == 'critical'])}"
    )
    print(
        f"   - High priority: {len([r for r in plan.recommendations if r.priority == 'high'])}"
    )
    ...
    for category, items in plan.implementation_roadmap.items():
        if items:
            print(
                f"   {category.replace('_', ' ').title()}: {len(items)} recommendations"
            )
