            ...
            if config_dir.exists():
                ...
                for config_file in config_dir.glob("*.yaml"):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to logout from profile {profile_name}: {e}"
                        )
...
@click.option(
    "--confirm-password", prompt=True, hide_input=True, help="Confirm new password"
)
@click.pass_context
def change_password(
    ctx,
...
def history(ctx, profile: Optional[str], days: int):
    ...
    try:
        ...
        for event in events:
            ...
            click.echo(
                f"  User Agent: {user_agent[:50]}{'...' if len(user_agent) > 50 else ''}"
            )
            click.echo()
...
def disable_mfa(ctx, profile: Optional[str]):
    ...
    try:
        ...
        if not click.confirm(
            "⚠️  Are you sure you want to disable MFA? This will reduce your account security."
        ):
