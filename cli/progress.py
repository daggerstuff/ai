...
class ProgressTracker:
    ...
    def track_operation(
        self, operation_name: str, total_steps: int, operation_id: Optional[str] = None
    ) -> str:
    ...
    def update_step(
        ...
        with self._lock:
            ...
            if step is None:
                step = ProgressStep(
                    name=step_name, description=description or step_name
                )
    ...
    def display_progress_report(self, operation_id: str):
        ...
        if not report:
            self.console.print(
                f"[red]No progress report found for operation: {operation_id}[/red]"
            )
            return
    ...
    def track_file_processing(
        self, files: list, operation_name: str = "File Processing"
    ) -> str:
        """Track file processing with progress bar"""
        operation_id = self.track_operation(operation_name, len(files))

        with self.create_progress_bar(len(files), f"Processing {len(files)} files") as (
            progress,
            task_id,
        ):
    ...
    def track_pipeline_execution(
        ...
        for i, step_config in enumerate(steps):
            ...
            self.update_step(
                operation_id, step_name, ProgressStatus.RUNNING, step_description
            )
    ...
    def display_live_progress(self, operation_id: str):
        ...
        def update_display():
            while True:
                ...
                layout.split_column(
                    Layout(header, size=3), Layout(steps_table, size=10)
                )
