...
class DeploymentConfig:
    ...
    ab_test_ratio: float = (
        50.0  # For A/B testing, percentage to new version (50/50 split)
    )
    shadow_sampling_rate: float = 0.1  # For shadow deployments, sample rate (0-1)
    evaluation_metrics: List[str] = field(
        default_factory=lambda: ["latency", "error_rate", "user_satisfaction"]
    )
    success_criteria: Dict[str, Any] = field(
        default_factory=dict
    )  # Metric thresholds for success
    rollback_criteria: Dict[str, Any] = field(
        default_factory=dict
...
class DeploymentManager:
            ...
            try:
                ...
                for model_name, rules_data in config.get("traffic_rules", {}).items():
                    self.traffic_rules[model_name] = [
                        self._dict_to_traffic_rule(rule_data)
                        for rule_data in rules_data
                    ]
                self.logger.info(
                    f"Loaded {len(self.deployments)} deployments from config"
                )
    ...
    def _save_config(self):
        ...
        if self.config_path:
            try:
                config_data = {
                    "deployments": {
                        dep_id: self._deployment_to_dict(dep)
                        for dep_id, dep in self.deployments.items()
                    },
                    "traffic_rules": {
                        model_name: [self._traffic_rule_to_dict(rule) for rule in rules]
                        for model_name, rules in self.traffic_rules.items()
                    },
                    "experiment_results": {
                        exp_id: self._experiment_result_to_dict(exp)
    ...
    def _dict_to_deployment(self, data: Dict[str, Any]) -> Deployment:
        ...
        config = DeploymentConfig(
            ...
            evaluation_metrics=config_data.get(
                "evaluation_metrics", ["latency", "error_rate"]
            ),
    ...
    def _deployment_to_dict(self, deployment: Deployment) -> Dict[str, Any]:
        ...
        return {
            ...
            "new_version": {
                "version_id": deployment.new_version.version_id
                if deployment.new_version
                else None,
                "model_name": deployment.new_version.model_name
                if deployment.new_version
                else None,
                "model_path": deployment.new_version.model_path
                if deployment.new_version
                else None,
                "model_type": deployment.new_version.model_type
                if deployment.new_version
                else None,
                "created_at": deployment.new_version.created_at
                if deployment.new_version
                else None,
                "hash_checksum": deployment.new_version.hash_checksum
                if deployment.new_version
                else None,
                "size_bytes": deployment.new_version.size_bytes
                if deployment.new_version
                else None,
                "performance_baseline": deployment.new_version.performance_baseline
                if deployment.new_version
                else None,
                "metadata": deployment.new_version.metadata
                if deployment.new_version
                else None,
            }
            if deployment.new_version
    ...
    def create_deployment(
        ...
        self.logger.info(
            f"Created deployment {deployment_id} for model {model_name} using strategy {strategy.value}"
        )
    ...
    def start_deployment(self, deployment_id: str) -> bool:
        ...
        self.logger.info(
            f"Started deployment {deployment_id} for model {deployment.model_name}"
        )
        return True
    ...
    def _setup_traffic_routing(self, deployment: Deployment):
        ...
        if deployment.strategy == DeploymentStrategy.AB_TEST:
            ...
            treatment_rule = TrafficRoutingRule(
                ...
                model_name=model_name,
                condition=f"user_id_hash >= {(100 - deployment.config.ab_test_ratio) / 100}",
                target_version=deployment.new_version.version_id
                if deployment.new_version
                else deployment.current_version.version_id,
                priority=2,
            )
    ...
    def advance_canary_deployment(self, deployment_id: str) -> bool:
        ...
        if deployment.config.traffic_percentage >= deployment.config.target_percentage:
            self.logger.info(
                f"Deployment {deployment_id} has reached target percentage"
            )
            return True
        ...
        # Log the advancement
        self.logger.info(
            f"Advanced canary deployment {deployment_id} from {old_percentage}% to {deployment.config.traffic_percentage}%"
        )
            ...
            if metric_name in metrics:
                ...
                if isinstance(threshold, dict):
                    ...
                    if not (min_val <= actual_value <= max_val):
                        success_violations.append(
                            f"{metric_name}: {actual_value} not in range [{min_val}, {max_val}]"
                        )
                else:
                    # Handle simple threshold (assuming lower is better for error rates, higher is better for others)
                    if (
                        "error" in metric_name.lower()
                        or "latency" in metric_name.lower()
                    ):
                        if actual_value > threshold:
                            success_violations.append(
                                f"{metric_name}: {actual_value} > {threshold}"
                            )
                    else:
                        if actual_value < threshold:
                            success_violations.append(
    ...
    def _trigger_alert(
        self, deployment: Deployment, alert_type: str, details: List[str]
    ):
        ...
        self.logger.warning(
            f"Alert triggered for deployment {deployment.deployment_id}: {alert_type} - {details}"
        )
    ...
    def rollback_deployment(self, deployment_id: str) -> bool:
        ...
        if deployment.status in [DeploymentStatus.ROLLED_BACK, DeploymentStatus.FAILED]:
            self.logger.error(
                f"Deployment {deployment_id} cannot be rolled back (already {deployment.status.value})"
            )
            return False
        ...
        self.logger.info(
            f"Rolled back deployment {deployment_id} from {old_status.value} to rolled_back"
        )
        self._save_config()
        return True
        ...
        try:
            ...
            if "user_id_hash" in condition:
                ...
                if user_id:
                    ...
                    user_hash = (
                        int(hashlib.md5(user_id.encode()).hexdigest(), 16) % 1000000
                    )
    ...
    def complete_experiment(
        self, deployment_id: str, results: ExperimentResult
    ) -> bool:
    ...
    def get_deployment_status(self, deployment_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "new_version": deployment.new_version.version_id
            if deployment.new_version
            else None,
            "created_at": deployment.created_at,
            "started_at": deployment.started_at,
            ...
            "last_updated": deployment.last_updated,
            "alerts": len(
                [a for a in deployment.alerts if not a.get("resolved", False)]
            ),
...
def test_deployment_manager():
    ...
    test_version = DeploymentVersion(
        ...
        performance_baseline={"latency_ms": 150.0, "throughput_reqs_per_sec": 10.0},
    )
    ...
    evaluation_result = deployment_manager.evaluate_deployment(
        direct_deployment_id, test_metrics
    )
    ...
    for dep in all_deployments:
        print(
            f"  - {dep['deployment_id']}: {dep['status']} ({dep['traffic_percentage']}%)"
        )
