...
class DeploymentManager:
    ...
    def _deployment_to_dict(self, deployment: Deployment) -> Dict[str, Any]:
        ...
        return {
            ...
            "new_version": (
                {
                    "version_id": (
                        deployment.new_version.version_id
                        if deployment.new_version
                        else None
                    ),
                    "model_name": (
                        deployment.new_version.model_name
                        if deployment.new_version
                        else None
                    ),
                    "model_path": (
                        deployment.new_version.model_path
                        if deployment.new_version
                        else None
                    ),
                    "model_type": (
                        deployment.new_version.model_type
                        if deployment.new_version
                        else None
                    ),
                    "created_at": (
                        deployment.new_version.created_at
                        if deployment.new_version
                        else None
                    ),
                    "hash_checksum": (
                        deployment.new_version.hash_checksum
                        if deployment.new_version
                        else None
                    ),
                    "size_bytes": (
                        deployment.new_version.size_bytes
                        if deployment.new_version
                        else None
                    ),
                    "performance_baseline": (
                        deployment.new_version.performance_baseline
                        if deployment.new_version
                        else None
                    ),
                    "metadata": (
                        deployment.new_version.metadata
                        if deployment.new_version
                        else None
                    ),
                }
    ...
    def get_deployment_status(self, deployment_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "new_version": (
                deployment.new_version.version_id if deployment.new_version else None
            ),
            "created_at": deployment.created_at,
            "started_at": deployment.started_at,
            "completed_at": deployment.completed_at,
