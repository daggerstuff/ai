...
from .state import ConvergenceStatus
from .types import (
    DifferentialEntry,
    DiseaseRarity,
    OrchestrationResult,
        ...
        for disease_id, score in sorted(matrix.items(), key=lambda x: x[1], reverse=True)[:20]:
            differential.append(
                DifferentialEntry(
                    ...
                    organ_systems=[],
                    rarity=DiseaseRarity.RARE if score < THRESHOLD_LOW_PROBABILITY else DiseaseRarity.UNCOMMON,
