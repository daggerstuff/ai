...
# Threshold constants for therapeutic simulation
...
THRESHOLD_MAX_TURNS = 50
THRESHOLD_SKILL_WEAK = 0.3
THRESHOLD_SKILL_STRONG = 0.7
THRESHOLD_CRISESCORE = 0.6
THRESHOLD_MAX_RESPONSE_LENGTH = 50
THRESHOLD_MAX_QUESTIONS = 2
...
class TherapeuticSimulationEngine:
    ...
    def _identify_therapist_mistakes(  # noqa: PLR0912
        self,
        therapist_input: str,
        client_profile: DifficultClientProfile,
        state: SimulationState,
        ...
        # Universal mistakes
        if len(therapist_input.split()) > THRESHOLD_MAX_RESPONSE_LENGTH:
            mistakes.append("Response too long - may overwhelm client")

        if "?" in therapist_input and therapist_input.count("?") > THRESHOLD_MAX_QUESTIONS:
        ...
        if client_profile.personality_type == ClientPersonality.RESISTANT:
            ...
            if state.emotional_intensity > THRESHOLD_HIGH_TRUST and "homework" in input_lower:
            ...
            if state.crisis_risk_level > THRESHOLD_CRISESCORE and "positive" in input_lower:
    ...
    def _assess_intervention_appropriateness(
        ...
        if state.crisis_risk_level > THRESHOLD_CRISESCORE:
        ...
        if state.resistance_level > THRESHOLD_HIGH_RESISTANCE:
    ...
    def _predict_client_response_impact(
        self,
        therapist_input: str,
        client_profile: DifficultClientProfile,
        ...
        if client_profile.personality_type == ClientPersonality.BORDERLINE_TRAITS and (
            "time is up" in input_lower or "end" in input_lower
        ):
    ...
    def _update_simulation_state(
        ...
        if therapist_analysis["therapeutic_quality"] > THRESHOLD_HIGH_THERAPEUTIC:
        ...
        if client_profile.personality_type == ClientPersonality.SUICIDAL_IDEATION and (
            state.trust_level > THRESHOLD_MODERATE_TRUST and "safe" in state.last_therapist_intervention.lower()
        ):
            crisis_change -= 0.1
        ...
        if (
            ...
            and state.resistance_level < THRESHOLD_MODERATE_RESISTANCE
            and therapist_analysis["therapeutic_quality"] > THRESHOLD_VERY_HIGH_TRUST
        ):
    ...
    async def _call_therapeutic_ai(
        ...
        Current State: {mood}, resistance={resistance:.1f}, trust={trust:.1f}
    ...
    def _generate_rule_based_response(
        ...
        if state.breakthrough_opportunity and state.trust_level > THRESHOLD_HIGH_TRUST:
        ...
        elif state.resistance_level > THRESHOLD_HIGH_RESISTANCE or therapist_analysis["mistakes"]:
        ...
        elif state.crisis_risk_level > THRESHOLD_CRISESCORE:
            # Crisis response
    ...
    def _generate_supervisor_feedback(
        ...
        for skill, score in therapist_analysis["skills_demonstrated"].items():
            ...
            if score > THRESHOLD_SKILL_STRONG:
                feedback["notes"].append(f"Strong {skill.replace('_', ' ')} demonstrated")
            elif score < THRESHOLD_SKILL_WEAK:
        ...
        if state.trust_level > THRESHOLD_MODERATE_TRUST:
    ...
    def _check_phase_transitions(self, state: SimulationState, _client_profile: DifficultClientProfile):
        ...
        if state.session_phase == "opening" and state.trust_level > THRESHOLD_LOW_TRUST:
            state.session_phase = "exploration"
        elif state.session_phase == "exploration" and state.trust_level > THRESHOLD_MODERATE_TRUST:
            state.session_phase = "working"
        elif state.session_phase == "working" and state.trust_level > THRESHOLD_VERY_HIGH_TRUST:
    ...
    def _calculate_next_challenge_level(self, state: SimulationState, client_profile: DifficultClientProfile) -> float:
        ...
        if len(state.red_flags_triggered) > THRESHOLD_SKILL_WEAK:
            base_challenge -= 0.1  # Reduce challenge if struggling

        if state.trust_level > THRESHOLD_HIGH_TRUST:
