...
DEFAULT_WAVE1_PACKAGE_ROOT = (
    ...
    / ".agent/internal/research/training_corpus_wave1_seed_build_2026-04-09"
)
DEFAULT_DELTA_PACKAGE_ROOT = (
    Path(__file__).resolve().parents[2]
    / ".agent/internal/research/training_corpus_release_delta_over_wave1_2026-04-09"
)
...
def _build_merged_once(output_dir: Path, config: MergePackageConfig) -> CorpusBuildResult:
    ...
    if overlap_ids:
        raise ValueError(
            "Component packages contain overlapping entries: "
            f"{', '.join(overlap_ids[:10])}"
        )
        ...
        for artifact_name, artifact_path in sorted(build_result.artifacts.items()):
            ...
            if comparison_path is None or not artifact_path.exists() or not comparison_path.exists():
                ...
                compared_files.append(
                    {"artifact": artifact_name, "match": False, "reason": "missing_artifact"}
                )
                continue
    ...
    for row in rows:
        ...
        sources.append(
            CorpusSource(
                ...
                allowed_lanes=tuple(
                    lane for lane in row.get("allowed_lanes", []) if isinstance(lane, str)
                ),
