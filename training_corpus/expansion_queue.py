...
DEFAULT_EXPANSION_QUEUE_PATH = (
    Path(__file__).resolve().parent / "assets" / "wave5_expansion_packets.json"
)
DEFAULT_EXPANSION_PACKET_PATH = DEFAULT_EXPANSION_QUEUE_PATH
DEFAULT_EXPANSION_DRAFT_QUEUE_PATH = (
    ...
    **kwargs: Any,
    ...
    return SeedPackQueueConfig(
        ...
        include_statuses=tuple(
            item for item in include_statuses if item in _QUEUE_ALLOWED_STATUSES
        )
...
def build_expansion_packet_queue(source_registry: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for source in source_registry:
        ...
        normalized_batch_size = recommended_batch_size if isinstance(recommended_batch_size, int) and recommended_batch_size > 0 else None
        ...
        for index, extraction_target in enumerate(extraction_targets, start=1):
            ...
            packet = {
                ...
                "source_evidence": _clone_json(source.get("evidence") if isinstance(source.get("evidence"), dict) else {}),
            }
    ...
    packets.sort(key=lambda packet: (_PRIORITY_ORDER.get(packet["priority"], 99), packet["source_key"], packet["packet_id"]))
    return {
...
def build_draft_queue_from_packets(packet_queue: dict[str, Any]) -> tuple[ExpansionQueueEntry, ...]:
    ...
    for packet in packets:
        ...
        if artifact_kind not in _QUEUE_ALLOWED_KINDS:
            artifact_kind = _infer_artifact_kind(str(packet.get("extraction_target") or ""), str(packet.get("mode") or ""))
    ...
    for packet in packets:
        ...
        ticket = tickets_by_source.setdefault(
            source_key,
            {
                ...
                "priority": packet.get("priority"),
                "allowed_lanes": [],
                "row_targets": _clone_json(packet.get("row_targets") if isinstance(packet.get("row_targets"), dict) else {}),
                "fields_to_mine": [],
                "extraction_focus": [],
                "discard_zones": [],
            },
