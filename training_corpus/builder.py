    ...
    def _build_once(self, config: CorpusBuildConfig) -> CorpusBuildResult:
        ...
        governance_report = build_governance_report(
            tuple(
                source
                for source in sources
                if source.inventory_decision == "keep" and source.default_lane is not None
            ),
            entry_tuple,
        )
        if config.enforce_release_gates and (
            ...
            raise ValueError(
                "Training corpus release gates failed: "
                f"governance={governance_report} continuity={continuity_report}"
            )
