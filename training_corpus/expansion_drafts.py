...
DEFAULT_WAVE5_EXPANSION_DRAFT_PACK_PATH = (
    Path(__file__).resolve().parent / "assets" / "wave5_expansion_draft_pack.json"
)
...
def _artifact_targets(ticket: dict[str, Any]) -> dict[str, int]:
    ...
    fields_to_mine = {
        item.lower()
        for item in ticket.get("fields_to_mine", [])
...
def _draft_pack_markdown(draft_pack: dict[str, Any]) -> str:
    ...
    for card in draft_pack.get("authoring_cards", []):
        lines.append(
            f"- `{card['card_id']}` | {card['artifact_type']} x{card['target_count']} | "
            f"{card['source_key']}"
        )
