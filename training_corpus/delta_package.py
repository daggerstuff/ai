...
def build_release_candidate_delta_corpus(
    ...
    *,
    ...
    resolved_catalog = (
        dict(resolved_config.catalog)
        if resolved_config.catalog is not None
        else build_local_source_catalog()
    )
    ...
    resolved_artifact_root = (
        resolved_config.artifact_root or output_dir.parent / f"inputs_{output_dir.name}"
    )
