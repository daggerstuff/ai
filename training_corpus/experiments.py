    ...
    catalog = {
        ...
        "foundation_helios": PreparedSource(
            ...
            records=_prepare_helios(
                base_dir
                / "ai/datasets/training_v3/stage1_foundation/heliosbrahma_mental_health_chatbot_dataset.jsonl"
            ),
        ),
        ...
        "evaluator_psychology": PreparedSource(
            ...
            records=_prepare_psychology_knowledge(
                base_dir / "ai/data/psychology_knowledge_base_optimized.json"
            ),
        ),
...
def _experiment_variants() -> tuple[ExperimentVariant, ...]:
    ...
    return (
        ...
        ExperimentVariant("G", "G1.1", "no dedicated long-running lane", {**common_base, "edge_policy": 40, **benchmark_base}),
        ExperimentVariant(
            "G",
            "G1.2",
        ...
        ExperimentVariant("H", "H1.1", "no persona texture lane", {"foundation_amod": 80, "foundation_helios": 40, "specialist_fadodr": 80, "edge_policy": 40, **benchmark_base}),
        ExperimentVariant(
            "H",
            "H1.2",
...
def _variant_metrics(build_result: CorpusBuildResult) -> dict[str, Any]:
    ...
    high_risk_persona_entries = [
        entry for entry in entries if str(entry.attributes.get("identity_risk", "")) == "high"
    ]
    simulation_rubric_entries = [
        entry for entry in simulation_entries if isinstance(entry.attributes.get("rubric_items"), list) and entry.attributes.get("rubric_items")
    ]
...
def _report_markdown(report: dict[str, Any]) -> str:
    ...
    for family in sorted(report["families"]):
        ...
        lines.append(
            f"- {family}: {winner['variant_id']} (score {winner['score']}) - {winner['description']}"
        )
    lines.extend(
        [
            "",
...
def _message_record(
    ...
    *,
    ...
    if _contains_pii(*parts):
        return None
    return {"messages": messages, "metadata": {key: value for key, value in metadata.items() if value is not None}, "lane": lane}
