...
class QualityImprovementReporter:
    ...
    def _generate_executive_summary(
        ...
        summary.append(
            f"📊 Quality Improvement Status: {total_active} active interventions, {total_completed} completed"
        )
        ...
        if overall_impact["total_interventions"] > 0:
            ...
            if overall_impact["successful_interventions"] > 0:
                summary.append(
                    f"✅ {overall_impact['successful_interventions']} interventions achieved their improvement targets"
                )
        ...
        if active_interventions:
            ...
            for intervention in active_interventions:
                intervention_types[intervention.intervention_type] = (
                    intervention_types.get(intervention.intervention_type, 0) + 1
                )

            most_common_type = max(intervention_types.items(), key=lambda x: x[1])
            summary.append(
                f"🔄 Most common active intervention type: {most_common_type[0]} ({most_common_type[1]} interventions)"
            )
    ...
    def _generate_action_items(
        ...
        if active_interventions:
            ...
            if overdue_interventions:
                actions.append(
                    f"⏰ Review {len(overdue_interventions)} long-running interventions for completion or adjustment"
                )
        ...
        if analyses:
            ...
            if poor_components:
                actions.append(
                    f"🔧 Focus additional resources on underperforming components: {', '.join(poor_components)}"
                )
