...
class ConversationQualityPatternAnalyzer:
    ...
    def _analyze_response_quality(self, conversations: pd.DataFrame) -> dict[str, Any]:
        ...
        for _, conv in conversations.iterrows():
            ...
            positive_words = len(
                re.findall(
                    r"\b(good|great|excellent|wonderful|amazing|helpful|thank|appreciate)\b",
                    text.lower(),
                )
            )
            negative_words = len(
                re.findall(
                    r"\b(bad|terrible|awful|horrible|hate|angry|frustrated|disappointed)\b",
                    text.lower(),
                )
            )
    ...
    def _analyze_conversation_flow(self, conversations: pd.DataFrame) -> dict[str, Any]:
        ...
        return {
            ...
            "flow_by_dataset": flow_df.groupby("dataset")["flow_score"]
            .agg(["mean", "median", "std", "count"])
            .to_dict("index"),
            "flow_patterns": {
    ...
    def _generate_quality_insights(self, quality_results: dict[str, Any]) -> list[str]:
        ...
        if flow_quality["overall_flow"]["mean_flow_score"] < 45:
            insights.append(
                "🌊 Conversation flow needs improvement - add more transition words and reduce topic shifts"
            )
